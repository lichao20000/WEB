...
import com.ai.itms.sms.socket.SMGPSocket;
...
public class SMGP3 {
	 ...
	 */
	private static Logger log = LoggerFactory.getLogger(SMGP3.class);
	 ...
	 * 返回值：
	 * 
	 * ERROR_SOCKET_CREATE socket创建失败 ERROR_CONNECT socket连接失败 ERROR_SOCKET_WRITE
	 * socket写失败 ERROR_SOCKET_READ socket读失败 ERROR_RSP_TIMEOUT 网关超时应答
	 * ERROR_CONNECT_MOD 链接登陆方式错 0 socket操作成功的socket号
	 * 
	 * 函数参数： param1: 网关IP param2: 网关端口 param3: SP的用户名（非SP_ID） param4: SP的密码 param5:
	 * 连接方式 0：发送短消息（send mode）； 1：接收短消息（receive mode）； 2：收发短消息（transmit mode）；
	 * 
	 * 
	 */
	...
	/**
	 * 返回值： ERROR_SOCKET_WRITE socket写失败 ERROR_SOCKET_READ socket读失败 0 得到MO消息 1
	 * 得到状态报告 2 无消息返回 3 返回非DELIVER消息
	 * 
	 * 函数参数： param1: socket号，由SGIP_Connect函数返回. param2: 返回DELIVER消息包。
	 * 
	 */
				...
				if (available >= 12) {
					...
					case FinalDef.CMD_SMGP_DELIVER: {
						...
						if (smgp_deliver.cIsReport == 1) {
							...
							smgp_status.parseResponseBody(
									Common.getText(smgp_deliver.ucMsgLength, smgp_deliver.sMsgContent));
							ret = 1;
						} else {
							ret = 0;
						}
	...
	/**
	 * 发送链路检测包 返回值： 0 成功 小于0 失败，失败后会主动关闭Socket套接字。
	 * 
	 * 函数参数 param1: socket号，由SMGP_connect函数返回.
	 * 
	 */
	...
	/**
	 * 发送DELIVER响应包 返回值： 0 成功 小于0 失败
	 * 
	 * 函数参数 param1： 收到mo对应的MSGID. param2: lSerial_ID，由接收到的DELIVER对应的序列号. param3:
	 * 请求返回结果
	 * 
	 */
	...
	/**
	 * 返回值： ERROR_SOCKET_WRITE socket写失败 ERROR_SOCKET_READ socket读失败
	 * ERROR_RSP_TIMEOUT 网关超时应答 ERROR_OK socket success
	 * 
	 * 函数参数： param1: socket号，是SMGP_connect函数的返回值； param2:
	 * 用户填写需要发送TSMGP_SUBMIT包；（具体格式参考头文件） param3: 查看网关的应答结果；
	 * 
	 */
