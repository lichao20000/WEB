...
/**
 * unsigned char cMsgType; // 1.3: 短消息子类型（0＝取消订阅，1＝订阅请求，2＝点播，3＝订阅） 1 // 2.0:
 * 短消息类型（1=取消订阅，2=订阅请求，3=点播，4=订阅，5=交互式操作，6=查询，7=点对点消息，8=电子商务，9=代码方式） // 3.0:
 * 短消息类型（0=MO消息，6=MT消息，7=点对点消息） 2 unsigned char cNeedReport; // 是否要求返回状态报告 //
 * 0：不要求 1：要求 unsigned char cPriority; // 发送优先级（ 1.3: 0-9； 2.0: 0-3； 3.0: 0-3） 3
 * unsigned char sServiceID[MAX_SMGP_SERVICEID_LEN]; // 服务类型 13 unsigned char
 * sFeeType[MAX_SMGP_FEETYPE_LEN]; // 资费类型 15 // 1.3:
 * （00：免费，01：按条收费，02：包月，03：封顶） // 2.0: （0：包月，1：按条，2：流量） // 3.0:
 * （00：免费，01：按条收费，02：包月，03：封顶）
 * 
 * unsigned char sFeeCode[MAX_SMGP_FEECODE_LEN]; // 资费代码(分) 21 unsigned char
 * sFixedFee[MAX_SMGP_FIXEDFEE_LEN]; // 包月费(封顶费)071051(单位为分) 27
 * 
 * 
 * unsigned char ucMsgFormat; // 短消息格式(参见短消息格式表) 28 unsigned char
 * sValidTime[MAX_SMGP_TIME_LEN]; // 有效时间，遵循SMPP3.3 45 unsigned char
 * sAtTime[MAX_SMGP_TIME_LEN]; // 定时发送时间，遵循SMPP3.3 62 unsigned char
 * sSrcTermID[MAX_SMGP_TERMINALID_LEN]; // 短消息发送用户号码 83 unsigned char
 * sChargeTermID[MAX_SMGP_TERMINALID_LEN]; // 计费用户号码 104 unsigned char
 * cDestTermIDCount; // 短消息接收号码总数(小于100) 105 unsigned char
 * sDestTermID[MAX_SMGP_TERMINALID_LEN*MAX_USER_NUM]; //
 * 短消息接收号码(连续存储DestTermIDCoutn个) unsigned char ucMsgLength; // 短消息长度 unsigned
 * char sMsgContent[MAX_SMGP_MSGCONTENT_LEN]; // 短消息内容 unsigned char
 * sReserve[MAX_SMGP_RESERVE_LEN]; // 保留
 */

public class TSMGP_SUBMIT implements SMGPRequestBody {

	public static Logger log = LoggerFactory.getLogger(TSMGP_SUBMIT.class);
	...
	public byte[] getRequestBody() {
		...
		int length = FinalDef.MAX_SMGP_RESERVE_LEN + b.length + FinalDef.MAX_SMGP_TERMINALID_LEN * 2
				+ FinalDef.MAX_SMGP_TIME_LEN * 2 + FinalDef.MAX_SMGP_FIXEDFEE_LEN + FinalDef.MAX_SMGP_FEECODE_LEN
				+ FinalDef.MAX_SMGP_FEETYPE_LEN + FinalDef.MAX_SMGP_SERVICEID_LEN + 6 + tsmgpTlv.getBytes().length;
		...
		log.warn("接收方:" + sDestTermID + ",业务代码:" + sServiceID);
		...
		log.debug("tlv.length包长度：" + tlv.length);
		cur += tlv.length;
