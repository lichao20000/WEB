...
import com.linkage.liposs.buss.dao.securitygw.AttackReportDAO;
 ...
 * 
 */
public class AttackReportAction extends ActionSupport
{
	private static Logger log = LoggerFactory.getLogger(AttackReportAction.class);
	/**
	 * 
	 */
	 ...
	 * 病毒事件统计 数据来源对象
	 */
	private AttackReportDAO attrDAO;
	...
	public String getTableString(List<Map> resultData){
		...
		sb.append("<td class=\"tr_yellow\">攻击方式</td>");
		...
		if(resultData != null && resultData.size() != 0){
			...
			while(it.hasNext()){
				...
				sb.append("<td class=\"tr_white\">").append(map.get("attackname")).append("</td>");
				sb.append("<td class=\"tr_white\">").append(map.get("attacktimes")).append("</td>");
				sb.append("</tr>");
			}
		}else{
			sb.append("<tr>").append("<td colspan=4 class=\"tr_white\">").append("<div align=\"center\">暂时无攻击事件数据.</div>").append("</td>").append("</tr>");
		}
		sb.append("</table>");
	 ...
	 * @return
	 */
	public String getAttackWeekTable(){
		DateTimeUtil dateTime = new DateTimeUtil(nowDateString);
		long endtime = dateTime.getLongTime();
		// 先向后推7天
		...
		result = attrDAO.getAttackReportData(deviceid, starttime, endtime,1);
		ajax = getTableString(result);
		return cst.AJAX;
	}
	/**
	 ...
	 * @return
	 */
	public String getAttackMonthTable(){
		DateTimeUtil dateTime = new DateTimeUtil(nowDateString);
		// 结束时间
		long endtime = dateTime.getLongTime();
		dateTime.getNextMonth(-1);
		long starttime = dateTime.getLongTime();
		result = attrDAO.getAttackReportData(deviceid, starttime, endtime,2);
		ajax = getTableString(result);
	...
	public String getPreView(){
		...
		result = attrDAO.getAttackReportData(deviceid, starttime, endtime,0);
		startTime = attrDAO.getStartTime();
		endTime = attrDAO.getEndTime();
		chartUtil.setXDate(new Date(starttime*1000),new Date(endtime*1000));
		chartUtil.setXAxisUnit(DateTickUnit.HOUR, 4, "H");
		chart = chartUtil.createXYStackedBarP("攻击事件统计", "时间", "次数", result, "attacktimes", "stime", "attackname", ChartYType.Hour, false);
		return "small";
	}
	/**
	 * 
	...
	public String getAttackDay(){
		...
		chart = chartUtil.createXYStackedBarP("攻击事件统计(" + startTime + " 到 " + endTime + ")", "时间", "次数", result, "attacktimes", "stime", "attackname", ChartYType.Hour, true);
		return "day";
	}
	/**
	 * 
	...
	public String getAttackWeek(){
		...
		chart = chartUtil.createXYStackedBarP("攻击事件统计(" + startTime + " 到 " + endTime + ")", "时间", "次数", result, "attacktimes", "stime", "attackname", ChartYType.Day, true);
		return "week";
	}
	/**
	 * 
	...
	public void setAttrDAO(AttackReportDAO attrDAO)
	{
	 ...
	 * @return
	 */
	public String getAttackAdvQuery(){
	    //算出两个时间点之间的秒数
		...
		result = attrDAO.getAttackReportData(deviceid, adv_stime, adv_etime,0);
		startTime = attrDAO.getStartTime();
		endTime = attrDAO.getEndTime();
		...
		result = attrDAO.getAttackReportData(deviceid, adv_stime, adv_etime,1);
		startTime = attrDAO.getStartTime();
		endTime = attrDAO.getEndTime();
		chartUtil.setXDate(new Date(adv_stime*1000),new Date(adv_etime*1000));
		chartUtil.setXAxisUnit(DateTickUnit.DAY, 2, "dd");
	 ...
	 * @return
	 */
	public String getAttackAdvTable(){
	  //算出两个时间点之间的秒数
	    long second = adv_stime - adv_etime;
	    //如果时间是24小时之内,则直接查询小时表
	    if(second == 0){
		//如果两个时间选择的是同一个时间,如：2008-5-6 2008-5-6，则结束时间为起始时间+86400s
		adv_etime = adv_stime + 86400;
