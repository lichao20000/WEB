...
/**
 * 增加高级查询及修改报表显示(基于zhaof的代码修改)
 * 
 * @author suixz(5253)
 * @version 1.0
 * @category securitygw
 */
public class PBMailTopN extends ActionSupport
{
	/**
	 * serialVersionUID
	 */
	private static final long serialVersionUID = -9192343425426651L;
	private static final Logger LOG = LoggerFactory.getLogger(PBMailTopN.class);
	...
	public String execute() throws Exception
	{
		...
		dataDay = pbTopN.getMailTopNDay(deviceid, date, topN);
		int total = 0;
		for (Map m : dataDay)
			{
		...
		dataWeek = pbTopN.getMailTopNWeek(deviceid, date, topN);
		int totalOfWeek = 0;
		for (Map m : dataWeek)
			{
		...
		dataMonth = pbTopN.getMailTopNMonth(deviceid, date, topN);
		int totalOfMon = 0;
		for (Map m : dataMonth)
			{
		...
		imgUrlDay = "/securitygw/PBMailTopNAction!getMailTopNDay.action" + s;
		imgUrlWeek = "/securitygw/PBMailTopNAction!getMailTopNWeek.action" + s;
		imgUrlMonth = "/securitygw/PBMailTopNAction!getMailTopNMonth.action" + s;
		return SUCCESS;
	}
