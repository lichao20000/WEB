 ...
 * 
 */
public class AttackReportDAO
{
	private static Logger log = LoggerFactory.getLogger(AttackReportDAO.class);
	 ...
	 * @return
	 */
	public List<Map> getAttackReportData(String deviceid, long starttime, long endtime, int type)
	{
		...
		String sqlTemp = "select " + com.linkage.litms.common.util.DbUtil.getNullFunction("attackname", "'未知攻击类型'") + " attackname,sum(attacktimes) attacktimes,stime,etime from ? a left join sgw_attack_info b on a.attacktype=b.attacktype where deviceid='"
				+ deviceid + "' and stime >=" + starttime + " and etime<=" + endtime + " group by attackname,stime,etime";
		sqlTemp = getAllSQLList(deviceid, starttime, endtime,type == 0 ? Calendar.DAY_OF_MONTH : Calendar.MONTH, 1, type == 0 ? "sgw_attack_hour_" : "sgw_attack_day_", type == 0 ? "yyyy_MM_dd" : "yyyy_MM",sqlTemp);
		...
		if (dateTimeStart.getYear() == dateTimeEnd.getYear()
				&& dateTimeStart.getMonth() == dateTimeEnd.getMonth())
			{
				...
				sql = "select nvl(attackname,'未知攻击类型') attackname,sum(attacktimes) attacktimes,stime,etime from " + tablename + " a left join sgw_attack_info b on a.attacktype=b.attacktype where deviceid='"
						+ deviceid + "' and stime >=" + starttime + " and etime<=" + endtime + " group by attackname,stime,etime";
			}
		else
			if (dateTimeStart.getYear() == dateTimeEnd.getYear())
				{// 如果是同年不同月
					...
					sql = "select nvl(attackname,'未知攻击类型') attackname,sum(attacktimes) attacktimes,stime,etime from " + tablename
							+ " a left join sgw_attack_info b on a.attacktype=b.attacktype where deviceid='" + deviceid + "' and stime >="
							+ starttime + " and etime<=" + endtime + " group by attackname,stime,etime";
					...
					sql += " union select nvl(attackname,'未知攻击类型') attackname,sum(attacktimes) attacktimes,stime,etime from " + tablename
							+ " a left join sgw_attack_info b on a.attacktype=b.attacktype where deviceid='" + deviceid + "' and stime >="
							+ starttime + " and etime<=" + endtime + " group by attackname,stime,etime";
				}
			else
	...
	public String getTableName(int year,int month,int day,int type){
		String tablenamePrex = "sgw_attack_hour_";
		if(type == 1 || type == 2){
			tablenamePrex = "sgw_attack_day_";
		}
