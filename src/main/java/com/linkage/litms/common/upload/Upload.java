...
public class Upload 
{
		...
		do
		{
			...
			if(flag3 && s4.length() > 0)
			{
				...
				if(m_maxFileSize > 0L && (long)((m_endData - m_startData) + 1) > m_maxFileSize)
					throw new SecurityException(String.valueOf((new StringBuffer("Size exceeded for this file : ")).append(s4).append(" (1105).")));
			...
			if(flag3)
			{
				com.linkage.litms.common.upload.File file = new com.linkage.litms.common.upload.File();
				file.setParent(this);
		...
		throws UploadException, IOException, ServletException
	{
		...
		if(s2 == null)
			m_response.setHeader("Content-Disposition", String.valueOf((new StringBuffer(String.valueOf(m_contentDisposition))).append(" filename=").append(getFileName(s))));
		else
		if(s2.length() == 0)
			m_response.setHeader("Content-Disposition", m_contentDisposition);
		else
			m_response.setHeader("Content-Disposition", String.valueOf((new StringBuffer(String.valueOf(m_contentDisposition))).append(" filename=").append(s2)));
		while((long)k < l) 
		{
		...
		throws SQLException, IOException, ServletException
	{
		...
		if(s2.length() == 0)
			...
			m_response.setHeader("Content-Disposition", "attachment; filename=".concat(String.valueOf(s2)));
	...
	private String getDataFieldValue(String s, String s1)
	{
		...
		boolean flag1 = false;
		boolean flag2 = false;
		s2 = String.valueOf((new StringBuffer(String.valueOf(s1))).append("=").append('"'));
		i = s.indexOf(s2);
		if(i > 0)
		{
			int j = i + s2.length();
		...
		if(i == 0)
		{
			if(isVirtual(s1))
			{
				...
				if(s1.endsWith(s3))
					...
					s1 = String.valueOf((new StringBuffer(String.valueOf(s1))).append(s3).append(s2));
				return s1;
			}
			if(flag)
			{
