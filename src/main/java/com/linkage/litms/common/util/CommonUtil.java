...
public class CommonUtil
{
	...
	public static String getSQLByDay(String start, String end, long st, long et,
			String sel, String whe, String order, String time, String tab_name)
	{
				...
				if (flg)
					{
						sql += "union all " + sel + " " + table + " " + whe + " and "
								+ time + "<=" + edt.getLongTime() + order + " ";
					}
				else
					{
						sql = sel + " " + table + " " + whe + " and " + time + ">="
								+ sdt.getLongTime() + " and " + time + "<="
								+ edt.getLongTime() + order + " ";
					}
			}
		// 清除变量
		tabMap = null;
	...
	/**
	 * 根据开始时间、结束时间、表名称、条件拼接sql语句(以周分隔表e.g. tab_servicedata_2008_54)
	 * 报表中经常使用到跨表查询，如果表不存在又没有做判断就容易出错，这个通用类就是为了解决之中的错误 
	 * 注：start、end和st、et只能使用一种
	 * 
	 * @author benyp(5260) benyp@lianchuang.com
	 * @param start:开始时间:默认null
	 * @param end:结束时间：默认null
	 ...
	 * @param whe:查询的条件,如果条件没有就用where
	 *            1=1
	 * @param order:order 的参数,如果没有则为""
	 * @param group:group by 的参数,如果没有则为""
	 * @param time:条件中时间参数
	 * @param tab_name:表名(以什么形式开头)
	 *            eg:sql=getSQLByDay("2008-02-01","2008-03-17","select
	 *            name,time,id from "," where 1=1 and
	 ...
	 * @return sql:返回的sql
	 */
	public static String getSQLByWeek(String start, String end, long st, long et,
			String sel, String whe, String order,String group, String time, String tab_name)
	{
		tab_name=tab_name.endsWith("_")?tab_name:tab_name+"_";
		Map tabMap = getExistTables(tab_name); // 获取所有存在的table名称
		if(tabMap.isEmpty()) return "";//无数据返回
		DateTimeUtil sdt , edt; // 开始时间、结束时间
		boolean type = (st == 0 && et == 0) ? true : false;// 时间是否是字符串格式，true：是字符串格式，false：是长整形格式
		sdt = type ? (new DateTimeUtil(start)) : (new DateTimeUtil(st));
		edt = type ? (new DateTimeUtil(end)) : (new DateTimeUtil(et));
		ArrayList<String> list=new ArrayList<String>();
		int sy = sdt.getYear();//开始年份
		int ey = edt.getYear();//结束年份
		int sw=sdt.getWeekOfYear();
		int ew=edt.getWeekOfYear();
		String table="";
		if(sy==ey){//同一年
			for(int i=sw;i<=ew;i++){
				table=tab_name+sy+"_"+i;
				if(tabMap.containsKey(table)){
					list.add(table);
				}
			}
		}else{//不同年份
			DateTimeUtil tdt=new DateTimeUtil(sw+"-12-31");
			for(int i=sw;i<=tdt.getWeekOfYear();i++){
				table=tab_name+sy+"_"+i;
				if(tabMap.containsKey(table)) list.add(table);
			}
			for(int i=0;i<=ew;i++){
				table=tab_name+ey+"_"+i;
				if(tabMap.containsKey(table)) list.add(table);
			}
		}
		...
		for(int i=0;i<n;i++){
			if(flg){
				sql+=" union all "+sel + " from " + list.get(i) + " " + whe + " and " + time + ">="
				+ sdt.getLongTime() + " and " + time + "<=" + edt.getLongTime()
				+ group + " ";
			}else{
				sql=sel + " from " + list.get(i) + " " + whe + " and " + time + ">="
				+ sdt.getLongTime() + " and " + time + "<=" + edt.getLongTime()
				+ group + " ";
				flg=true;
			}
