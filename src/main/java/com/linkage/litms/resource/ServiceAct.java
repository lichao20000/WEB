...
public class ServiceAct
{
	...
	public String getHtmlDeviceByVendor(HttpServletRequest request)
	{
		...
		else
		{
			...
			{
				if("2".equals(gw_type_Str)){
					pSQL.setSQL("select count(a.device_id) as num,a.vendor_id,a.city_id from tab_gw_device a,tab_egwcustomer b ,tab_devicetype_info t where a.device_status = 1 and a.devicetype_id=t.devicetype_id  and a.device_id = b.device_id and a.cpe_allocatedstatus = 1 and gw_type=? group by a.vendor_id,a.city_id");
				}else{
					pSQL.setSQL("select count(a.device_id) as num,a.vendor_id,a.city_id from tab_gw_device a,tab_hgwcustomer b ,tab_devicetype_info t where a.device_status = 1 and a.devicetype_id=t.devicetype_id  and a.device_id = b.device_id and a.cpe_allocatedstatus = 1 and gw_type=? group by a.vendor_id,a.city_id");
				}
			}else {
		...
		if (Global.HBDX.equals(Global.instAreaShortName)) {
			cursor = DataSetBean.getCursor(pSQL.getSQL(), "proxool.xml-ctc-group-check");
		}else {
			cursor = DataSetBean.getCursor(pSQL.getSQL(), DataSourceTypeCfgPropertiesManager
					.getInstance().getConfigItem(this.getClass().getName()));
		}
		...
		sbTable.append(getSubTotal(cityList, isPrt, total, gw_type, userCity,isBind));
		// Clear Resouce
		listServiceId.clear();
		listServiceId = null;
		mapServiceCityNum.clear();
	...
	public String getHtmlDeviceByEdition(HttpServletRequest request)
	{
		...
		String is_esurfing=StringUtil.getStringValue(request.getParameter("is_esurfing"));//天翼网关更改的地方  jiangkun 
		...
		//hb_lt增加三级属地过滤
		if(Global.HBLT.equals(Global.instAreaShortName) && 4 == gw_type)
		{
		...
		if (no_query == null || no_query.trim().equalsIgnoreCase(""))
		{
			...
			{
				sqlCount.append(" select count(a.device_id) as num, a.devicetype_id from tab_gw_device a,tab_devicetype_info t ");
				
				//查询是否天翼网关，是否支持测速，是否支持千兆 需要加上关联表tab_device_version_attribute
				if (!StringUtil.IsEmpty(is_esurfing)||  !StringUtil.IsEmpty(gbbroadband) || !StringUtil.IsEmpty(is_speedTest)) {
				...
				if ("1".equals(is_ableUser)) {
					if (1 == gw_type) {
						sqlCount.append(" ,tab_hgwcustomer b ,(select distinct user_id from hgwcust_serv_info where serv_type_id in (10,11,14)) d ");
					}else {
						sqlCount.append(" ,tab_egwcustomer b ,(select distinct user_id from egwcust_serv_info where serv_type_id in (10,11,14)) d ");
					}
				}
				...
				sqlCount.append(" and a.device_status = 1 and a.customer_id is not null and a.gw_type="+ gw_type + " and 1=1");
				
			}
			else if(Global.JLDX.equals(Global.instAreaShortName)&&sqlid.equals("1"))
			{
				...
				{
					sqlCount.append(" select count(a.device_id) as num, a.devicetype_id from tab_gw_device a,tab_devicetype_info t,tab_hgwcustomer b ,hgwcust_serv_info c "
							+" where a.devicetype_id=t.devicetype_id and a.device_id = b.device_id and b.user_id = c.user_id "
							+"and c.serv_type_id = 10 and a.device_status = 1 and a.gw_type="
							+ gw_type + " and 1=1");
					sqlCount.append(" and a.cpe_allocatedstatus = 1");
				}else {
					sqlCount.append(" select count(a.device_id) as num, a.devicetype_id from tab_gw_device a,tab_devicetype_info t where a.devicetype_id=t.devicetype_id and a.device_status = 1 and a.gw_type="
							+ gw_type + " and 1=1");
				}
			}
				...
				if(1 == gw_type)
				{
					sqlCount.append(" select count(a.device_id) as num, a.devicetype_id from tab_gw_device a,tab_devicetype_info t,tab_hgwcustomer b where a.devicetype_id=t.devicetype_id and a.device_id = b.device_id and a.device_status = 1 and a.gw_type="
							+ gw_type + " and 1=1 and a.cpe_allocatedstatus = 1  and a.customer_id is not null "  );
				}
				else
				{
					sqlCount.append(" select count(a.device_id) as num, a.devicetype_id from tab_gw_device a,tab_devicetype_info t,tab_egwcustomer b where a.devicetype_id=t.devicetype_id and a.device_id = b.device_id and a.device_status = 1 and a.gw_type="
							+ gw_type + " and 1=1 and a.cpe_allocatedstatus = 1  and a.customer_id is not null "  );
				}
			...
			if(Global.JXDX.equals(Global.instAreaShortName))
			{
				...
				{
					sqlCount.append(" and a.complete_time>"+startTime+"");
				}
				if (!StringUtil.IsEmpty(endTime))
				{
					sqlCount.append(" and a.complete_time<"+endTime+"");
				}
			}
			...
			psqlCount.getSQL();
			if (Global.HBDX.equals(Global.instAreaShortName)) {
				cursor = DataSetBean.getCursor(sqlCount.toString(),"proxool.xml-ctc-group-check");
			}else {
			...
			Map mapService = getEditionMap(vendorId, modelId, deviceTypeId, edtionIdStr,
					rela_dev_type_id, access_style_relay_id, userCityID,gw_type,is_esurfing, gbbroadband, is_speedTest, ip_type);
			if (null == mapService)
			{
				return "<TR bgcolor='#FFFFFF'><TD align='center'class=column>系统中暂无任何业务!</TD></TR>";
			}
			...
			if(4 == gw_type){
				...
				if (Global.HBLT.equals(Global.instAreaShortName))
				{
					sbTable.append("<TH nowrap>厂商</TH><TH>设备型号</TH><TH>硬件版本</TH><TH>软件版本</TH><TH>是否千兆</TH><TH>版本特性</TH><TH>是否审核</TH>");
				}
			...
			sbTable.append("<TH nowrap>小计</TH></TR>");//展示了表第一行
				getNextCityList(cityList);
			// 初始化地市小计
			Map cityMap = CityDAO.getCityIdCityNameMap();
			Iterator cityIt = cityMap.keySet().iterator();
			...
			else if(Global.JXDX.equals(Global.instAreaShortName))
			{
				...
				sql.append(" select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a,tab_devicetype_info t ");
				
				//查询是否天翼网关，是否支持测速，是否支持千兆 需要加上关联表tab_device_version_attribute
				if (!StringUtil.IsEmpty(is_esurfing)||  !StringUtil.IsEmpty(gbbroadband) || !StringUtil.IsEmpty(is_speedTest)) {
					sql.append(" ,tab_device_version_attribute c ");
				}
				...
				if ("1".equals(is_ableUser)) {
					if (1 == gw_type) {
						sql.append(" ,tab_hgwcustomer b ,(select distinct user_id from hgwcust_serv_info where serv_type_id in (10,11,14)) d ");
					}else {
						sql.append(" ,tab_egwcustomer b ,(select distinct user_id from egwcust_serv_info where serv_type_id in (10,11,14)) d ");
					}
				}
				
				sql.append(" where a.devicetype_id=t.devicetype_id ");
				
				//查询是否天翼网关，是否支持测速，是否支持千兆 关联条件 
				if (!StringUtil.IsEmpty(is_esurfing)||  !StringUtil.IsEmpty(gbbroadband) || !StringUtil.IsEmpty(is_speedTest)) {
					sql.append(" and t.devicetype_id=c.devicetype_id ");
				}
				
				if (!StringUtil.IsEmpty(recentStartTime) || !StringUtil.IsEmpty(recentEndTime)) {
					sql.append(" and a.device_id=e.device_id ");
				}
				
				//查询是否有效用户 是：关联条件
				if ("1".equals(is_ableUser)) {
					sql.append(" and a.device_id = b.device_id and b.user_id = d.user_id ");
				}
				sql.append(" and a.device_status = 1 and a.customer_id is not null and a.gw_type=? and 1=1");
			...
			else if(Global.JLDX.equals(Global.instAreaShortName)&&sqlid.equals("1"))
			{
				...
				if ("0".equals(use_state))
				{
					sql1 = "select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a ,tab_devicetype_info t where a.devicetype_id=t.devicetype_id "
							+"and a.device_status = 1 and a.gw_type=? ";
					
					sql1=sql1+" and a.cpe_allocatedstatus = 0";
				}
				else if ("1".equals(use_state))
				{
					sql1 = "select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a, tab_hgwcustomer b ,hgwcust_serv_info c ,tab_devicetype_info t"
							+" where a.devicetype_id=t.devicetype_id and a.device_id = b.device_id and b.user_id = c.user_id and c.serv_type_id = 10 "
							+"and a.device_status = 1 and a.gw_type=? ";
					
					sql1=sql1+" and a.cpe_allocatedstatus = 1";
				}else {
					sql1 = "select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a ,tab_devicetype_info t where a.devicetype_id=t.devicetype_id "
							+"and a.device_status = 1 and a.gw_type=? ";
				}
			...
			else
			{
				if (Global.NXDX.equals(Global.instAreaShortName))
				{
					if(1 == gw_type)
					{
						sql1 = "select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a,tab_devicetype_info t,tab_hgwcustomer b where a.device_id=b.device_id " +
								" and a.devicetype_id=t.devicetype_id and a.device_status = 1 and a.gw_type=? and a.customer_id is not null ";
					}
					else
					{
						sql1 = "select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a,tab_devicetype_info t,tab_egwcustomer b where a.device_id=b.device_id " +
								" and a.devicetype_id=t.devicetype_id and a.device_status = 1 and a.gw_type=? and a.customer_id is not null ";
					}
				}
				else
			...
			if (Global.NXDX.equals(Global.instAreaShortName)) 
			{
				sql.append(" and a.cpe_allocatedstatus=1");
			}
			...
			if(Global.JXDX.equals(Global.instAreaShortName))
			{
				//是否支持Ipv6
				if (!StringUtil.IsEmpty(ip_type)) {
					sql.append(" and t.ip_type = "+ip_type+" ");
				}
				...
				if (!StringUtil.IsEmpty(gbbroadband)) {
					sql.append(" and c.gbbroadband="+gbbroadband+" ");
				}
				if (!StringUtil.IsEmpty(is_speedTest)) {
					sql.append(" and c.is_speedTest="+is_speedTest+" ");
				}
				
			    //最近上报时间
				if (!StringUtil.IsEmpty(recentStartTime)) {
					sql.append(" and  e.last_time>"+recentStartTime+" ");
				}
				if (!StringUtil.IsEmpty(recentEndTime)) {
					sql.append(" and  e.last_time<"+recentEndTime+" ");
				}
				...
				if ("1".equals(is_ableUser)) {
					...
					//否：加上cpe_allocatedstatus = 0
					sql.append(" and a.cpe_allocatedstatus = 0 ");
				}
			}
			...
			psql.getSQL();
			if (Global.HBDX.equals(Global.instAreaShortName)) {
				cursor = DataSetBean.getCursor(sql.toString(),"proxool.xml-ctc-group-check");
			}else {
			...
			while (itVender.hasNext())
			{
				...
				{
					size = size
							+ ((List) valueMap.get(itDevicetype.next())).size();
				}
				mapServiceSize.put(key, size);
			}
			// 开始遍历属地,输出统计数据
			String isPrt = request.getParameter("isPrt");
			logger.warn("isPrt :  "+isPrt);
			String cityIdhblt = "";
			...
			while (it.hasNext())
			{
				...
				while (itDevicetype.hasNext())
				{
					...
					for (int i = 0; i < devicetypeList.size(); i++)
					{
						...
						if ("1".equals(isPrt))
						{
							if(Global.HBLT.equals(Global.instAreaShortName) && 4 == gw_type)//河北联通，且机顶盒
							{
								tmpRow=getEditionStatePrintHblt(devicetypeStr, "1",
										cityList, mapServiceCityNum, gw_type, userCityID,cityIdhblt,cityNextIdhblt);
							}
							else{
								tmpRow=getEditionStatePrint(devicetypeStr, "1",
						...
						else
						{
							if(Global.HBLT.equals(Global.instAreaShortName) && 4 == gw_type)
							{
								...
								tmpRow=getVendorStatRowHtml(devicetypeStr, "1",
										cityList, mapServiceCityNum, gw_type, userCityID,
										isBind);
							}
						}
					...
					if ((sbTableModel.length() > 0) && (!"".equals(sbTableModel)) && (sbTableModel != null)) {
			            sbTableVendor.append("<TD nowrap rowspan='" + modelSize + 
			              "' class=column align='right'>" + keyModel + "</TD>").append(sbTableModel);}
					 sbTableModel = null;
				}
				if ((sbTableVendor.length() > 0) && (!"".equals(sbTableVendor)) && (sbTableVendor != null)) {
			          sbTable.append("<TR ><TD nowrap rowspan='" + vendorSize + 
			            "' class=column align='right'>" + this.vendorMap.get(key) + "</TD>").append(sbTableVendor);
			        }
			        sbTableVendor = null;
			}
			String num = "";
			...
			if(Global.HBLT.equals(Global.instAreaShortName) && 4 == gw_type)
			{
				...
				num = getSubTotalHB(cityList, isPrt, total, gw_type, userCityID,cityIdhblt,cityNextIdhblt)
						.toString();
			}
			else
			{
				// total为 同一个地方所有版本的个数
				num = getSubTotal(cityList, isPrt, total, gw_type, userCityID,isBind)
									.toString();
			}
			// 输出小计信息
			if(4 == gw_type){
				...
				if(Global.HBLT.equals(Global.instAreaShortName))
				{
					sbTable.append("<tr><td nowrap align='right' class=column colspan=7>小计</td>");
				}
				else if(Global.SXLT.equals(Global.instAreaShortName))
				{
					sbTable.append("<tr><td nowrap align='right' class=column colspan=4>小计</td>");
	...
	public String getHtmlDeviceByEditionNXDXE8C(HttpServletRequest request)
	{
		HttpSession session = request.getSession();
		UserRes curUser = (UserRes) session.getAttribute("curUser");
		String userCityID = curUser.getCityId();
		...
		StringBuffer sqlCount = new StringBuffer();
		if (Global.NXDX.equals(Global.instAreaShortName)) {
			sqlCount.append("select count(a.device_id) as num, a.devicetype_id from tab_gw_device a,tab_devicetype_info t,tab_hgwcustomer b,tab_device_e8c_remould c "
					+ " where a.devicetype_id=t.devicetype_id and a.device_id = b.device_id and a.device_serialnumber=c.device_serialnumber "
					+ " and a.device_status = 1 and a.gw_type="
					+ gw_type + " and 1=1 and a.cpe_allocatedstatus = 1  and a.customer_id is not null "  );
		}
		if (!StringUtil.IsEmpty(userCityID) && !"00".equals(userCityID))
		...
		String rela_dev_type_id = request.getParameter("rela_dev_type_id");
		String access_style_relay_id = request.getParameter("access_style_relay_id");
		Map mapService = getEditionMapNXDXE8C(vendorId, modelId, deviceTypeId, edtionIdStr,
		...
		if (Global.NXDX.equals(Global.instAreaShortName))
		{
			sql1 = "select count(a.device_id) as num,a.devicetype_id,a.city_id from tab_gw_device a,tab_devicetype_info t,tab_hgwcustomer b ,tab_device_e8c_remould c where a.device_id=b.device_id " +
					" and a.devicetype_id=t.devicetype_id and a.device_serialnumber=c.device_serialnumber and a.device_status = 1 and a.gw_type=? and a.customer_id is not null"
					+ " and  a.vendor_id in ('2','10') and  a.device_model_id in ('13','34','100')";
		}
		pSQL.setSQL(sql1);
		pSQL.setInt(1, gw_type);
		...
		String num = "";
		num = getSubTotal(cityList, isPrt, total, gw_type, userCityID,isBind).toString();
		sbTable.append("<tr><td nowrap align='right' class=column colspan=5>小计</td>");
		sbTable.append(num);
		mapServiceCityNum.clear();
		mapServiceCityNum = null;
	...
	private StringBuffer getSubTotal(List cityList, String isPrt, long total,
			int gw_type, String userCityID,String isBind)
	{
		StringBuffer sbTable = new StringBuffer();
		long totalNum = 0;
		...
		{ 
			...
			else
			{
				...
				for (int i = 0; i < childList.size(); i++)
				{
					...
					num += StringUtil.getLongValue(cityTotalMap,(String)childList.get(i),0);
				}
				childList = null;
			}
			...
			else
			{
				...
				else
				{
					sbTable.append("<TD bgcolor=#ffffff align=center><a href=javascript:onclick=detail('"
							+ city + "','-1'," + gw_type + ",'"+isBind+"') >" + num_str + "</a></TD>");
				}
			}
			...
			else
			{
				...
				else
				{
					sbTable.append("<td bgcolor=#ffffff align=center><a href=javascript:onclick=detail('-1','-1',"
							+ gw_type + ",'"+isBind+"')>" + totalNum + "</a></td></tr>");
				}
			}
		}
		return sbTable;
	...
	private StringBuffer getSubTotalHB(List cityList, String isPrt, long total,
			int gw_type, String userCityID, String cityIdhblt, String cityNextIdhblt)
	{
		StringBuffer sbTable = new StringBuffer();
		long totalNum = 0;
		Iterator it = cityList.iterator();
		...
		{ 
			String city = (String) it.next();
			long num = 0;
			if(("-1".equals(cityIdhblt) || "00".equals(cityIdhblt)) && ("-1".equals(cityNextIdhblt) || "00".equals(cityNextIdhblt)))
			{
				if("00".equals(city))
				{
					num = Long.parseLong((String) cityTotalMap.get(city));
			...
			Map mapServiceCityNum, int gw_type, String userCityID)
	{
		...
		if ("1".equals(type))
		{
			...
			String versionFeature = "";
			String gigabit_port = "";
			String is_check ="";//是否审核，河北联通家庭网关
			if(Global.HBLT.equals(Global.instAreaShortName) && 1 == gw_type)
			{//厂商/型号/硬件/软件/千兆/特性/是否审核
				if("0".equals(tmp[5]))
				{
					versionFeature = "普通";
				}
				else if ("1".equals(tmp[5])) 
				{
					versionFeature = "测速";
				}
				else if ("2".equals(tmp[5]))
				{
					versionFeature = "全路由";
				}
				if("1".equals(tmp[4]))
				...
				if("-1".equals(tmp[6])){
					is_check = "未测试";
				}else if("0".equals(tmp[6])){
					is_check = "已测试，未审核";
				}else if("1".equals(tmp[6])){
					is_check = "已审核";
				}
				sb.append("<TD nowrap class=column align='right'>" + tmp[2] + "</TD>");
				sb.append("<TD nowrap class=column align='right'>" + tmp[3] + "</TD>");
				//是否千兆
				sb.append("<TD nowrap class=column align='right'>" + gigabit_port + "</TD>");
				//版本特性
				sb.append("<TD nowrap class=column align='right'>" + versionFeature + "</TD>");
				//是否审核  HBLT-RMS-20190102-LH-043
				sb.append("<TD nowrap class=column align='right'>" + is_check + "</TD>");
				logger.warn("是否审核："+is_check);
			}
			else 
			{
				if (tmp != null && tmp.length > 2)
				{
					...
					if (tmp.length > 4)
					{
						...
						// 是否规范
						if(4 != gw_type && !Global.SXLT.equals(Global.instAreaShortName)){
							sb.append("<TD nowrap class=column align='right'>" + tmp[3] + "</TD>");
						}
					}
					else
	...
	private String getEditionStatePrintHblt(String vendor_id, String type, List cityList,
			Map mapServiceCityNum, int gw_type, String userCityID, String cityIdhblt, String cityNextIdhblt)
	{
		Iterator it = cityList.iterator();
		String num = null;
		StringBuffer sb = new StringBuffer();
	...
	{
		logger.debug("getVendorStatRowHtml start ");
		Iterator it = cityList.iterator();
		String num = null;
		StringBuffer sb = new StringBuffer();
...
//			message = (String) versionMap.get(vendor_id);
			...
			//  [vendor_id,device_model,hardwareversion,gigabit_port,banbentexing]
			String[] tmp = message.split("\\|",-1);
			String versionFeature = "";
			String gigabit_port = "";
			String is_check = "";
		...
		sb.append("</TR>");
		logger.debug("getVendorStatRowHtml end!");
		return sb.toString();
	}
	 ...
	 * @return
	 */
	private String getVendorStatRowHtmlHblt(String vendor_id, String type, List cityList,
			Map mapServiceCityNum, int gw_type, String userCityID, String isBind, String cityIdhblt, String cityNextIdhblt)
	{
...
//			message = (String) versionMap.get(vendor_id);
	...
	public Map getEditionMap(String vendorId, String modelId, String deviceTypeId,
			String edtionIdStr, String rela_dev_type_id, String access_style_relay_id,
			String userCityID,int gw_type,String is_esurfing,String gbbroadband,String is_speedTest,String ip_type)
	{
		StringBuffer bf = new StringBuffer();
		if(4 == gw_type){
			bf.append(" select a.vendor_id,b.device_model,a.devicetype_id from "
		...
		else if(Global.JXDX.equals(Global.instAreaShortName))
		{
			if (!StringUtil.IsEmpty(is_esurfing)||  !StringUtil.IsEmpty(gbbroadband) || !StringUtil.IsEmpty(is_speedTest)) {
				bf.append(" select a.vendor_id,b.device_model,a.devicetype_id from ");
				bf.append(" tab_devicetype_info a,gw_device_model b,tab_device_version_attribute c where a.device_model_id=b.device_model_id and a.devicetype_id=c.devicetype_id ");
			}else {
				bf.append(" select a.vendor_id,b.device_model,a.devicetype_id from ");
				bf.append(" tab_devicetype_info a,gw_device_model b where a.device_model_id=b.device_model_id ");
			}
		...
		if(Global.JXDX.equals(Global.instAreaShortName))
		{
			//是否ipv6
			if (!StringUtil.IsEmpty(ip_type)) {
				bf.append(" and a.ip_type = "+ip_type+" ");
			}
			//查询是否天翼网关，是否支持测速，是否支持千兆 关联条件 
			if (!StringUtil.IsEmpty(is_esurfing)) {
			...
			if (!StringUtil.IsEmpty(gbbroadband)) {
				bf.append(" and c.gbbroadband="+gbbroadband+" ");
			}
			if (!StringUtil.IsEmpty(is_speedTest)) {
				bf.append(" and c.is_speedTest="+is_speedTest+" ");
			}
		...
		psql.getSQL();
		if (Global.HBDX.equals(Global.instAreaShortName)) {
			cursor = DataSetBean.getCursor(bf.toString(),"proxool.xml-ctc-group-check");
		}else {
			cursor = DataSetBean.getCursor(bf.toString());
		}
		fields = cursor.getNext();
	...
	public Map getEditionMapNXDXE8C(String vendorId, String modelId, String deviceTypeId,
			String edtionIdStr, String rela_dev_type_id, String access_style_relay_id,
			String userCityID,int gw_type)
	{
		StringBuffer bf = new StringBuffer();
		bf.append(" select a.vendor_id,b.device_model,a.devicetype_id from "
					+ "tab_devicetype_info a,gw_device_model b where a.device_model_id=b.device_model_id and a.vendor_id in ('2','10') and  b.device_model_id in ('13','34','100')");
		if (null != vendorId && !"null".equals(vendorId) && !"-1".equals(vendorId))
		{
			bf.append(" and a.vendor_id='" + vendorId + "' ");
		}
	...
	public List[] getVendorIdNameList(String gwType)
	{
		...
		if("4".equals(gwType)){
			cursor = DataSetBean.getCursor(m_VendorInfo_stb, DataSourceTypeCfgPropertiesManager
					.getInstance().getConfigItem(this.getClass().getName()));
		}else{
			cursor = DataSetBean.getCursor(m_VendorInfo, DataSourceTypeCfgPropertiesManager
					.getInstance().getConfigItem(this.getClass().getName()));
		}
	...
	private void getVersionMap(int gw_type)
	{
		...
		if(Global.HBLT.equals(Global.instAreaShortName) && 1 == gw_type)
		{	
			sql = "select f.*,e.gigabit_port,e.version_feature from (select d.* from "
					+"(select a.hardwareversion,a.is_normal,a.softwareversion,a.devicetype_id,a.is_check,b.device_model,b.vendor_id "
					+" from  tab_devicetype_info a inner join gw_device_model b on a.device_model_id=b.device_model_id) d "
					+" inner join tab_vendor c  on  d.vendor_id=c.vendor_id ) f left join tab_device_version_attribute e on f.devicetype_id = e.devicetype_id";
		}
		if (versionMap == null)
		{
			...
			while (fields != null)
			{
				String normalResult = "1".equals(fields.get("is_normal")) ? "是"
						: "否";
				String version = "";
				if(Global.HBLT.equals(Global.instAreaShortName) )
				{
					if(4 == gw_type)
					{
						version = (String) fields.get("vendor_id") + "|"
								...
								+ (String) fields.get("hardwareversion") + "(" + (String) fields.get("softwareversion") + ")";
					}else if (1 == gw_type) {
						//versionFeature 和gigabit_port存在为空的情况，需要处理
						versionFeature = (String) (StringUtil.IsEmpty((String) fields.get("version_feature"))?"0":(String) fields.get("version_feature"));
						String gigabit_port = (String) (StringUtil.IsEmpty((String)fields.get("gigabit_port"))?"0":(String) fields.get("gigabit_port"));
						version = (String) fields.get("vendor_id") + "|"
								+ (String) fields.get("device_model") + "|"
								...
								+ gigabit_port+"|"
								+ versionFeature+"|"
								+ (String) fields.get("is_check");  //河北联通增加“是否审核”一列 HBLT-RMS-20190102-LH-043
						}
	...
	public String getModelHtml(String vendorId, String modelId, String gw_type)
	{
		...
		{
			if("4".equals(gw_type)){
				PrepareSQL psql = new PrepareSQL(" select a.device_model_id,a.device_model "
						+ " from stb_gw_device_model a where a.vendor_id='" + vendorId + "'");
				psql.getSQL();
				cursor = DataSetBean.getCursor(" select a.device_model_id,a.device_model "
						+ " from stb_gw_device_model a where a.vendor_id='" + vendorId + "'");
			}else{
				PrepareSQL psql = new PrepareSQL(" select a.device_model_id,a.device_model "
						+ " from gw_device_model a where a.vendor_id='" + vendorId + "'");
				psql.getSQL();
				cursor = DataSetBean.getCursor(" select a.device_model_id,a.device_model "
						+ " from gw_device_model a where a.vendor_id='" + vendorId + "'");
				}
	...
	public String getDeviceTypeHtml(String modelId, String deviceTypeId,String gw_type)
	{
		...
		{
			if("4".equals(gw_type)){
				PrepareSQL psql = new PrepareSQL(" select a.devicetype_id,a.softwareversion"
						+ " from stb_tab_devicetype_info a where a.device_model_id='" + modelId
						+ "'");
				psql.getSQL();
				cursor = DataSetBean.getCursor(" select a.devicetype_id,a.softwareversion"
						+ " from stb_tab_devicetype_info a where a.device_model_id='" + modelId
						+ "'");
			}else{
				PrepareSQL psql = new PrepareSQL(" select a.devicetype_id,a.softwareversion"
						+ " from tab_devicetype_info a where a.device_model_id='" + modelId
						+ "'");
				psql.getSQL();
				cursor = DataSetBean.getCursor(" select a.devicetype_id,a.softwareversion"
						+ " from tab_devicetype_info a where a.device_model_id='" + modelId
	...
	public String getVendorHtmlNXDXE8C(String vendorId,String gw_type)
	{
		StringBuffer bf = new StringBuffer();
		PrepareSQL psql = new PrepareSQL("select vendor_id,vendor_name, vendor_add from tab_vendor where vendor_id in ('2','10') ");
		cursor = DataSetBean.getCursor(psql.getSQL());
		fields = cursor.getNext();
		if (null != fields)
		{
	...
	public String getModelHtmlNXDXE8C(String vendorId, String modelId, String gw_type)
	{
		StringBuffer bf = new StringBuffer();
		if (null == vendorId || "-1".equals(vendorId))
		{
		...
		{
			PrepareSQL psql = new PrepareSQL(" select a.device_model_id,a.device_model from gw_device_model a where a.vendor_id='" + vendorId + "' and device_model_id in ('13','34','100')");
			cursor = DataSetBean.getCursor(psql.getSQL());
			fields = cursor.getNext();
			if (null != fields)
			{
	...
	public String getDeviceTypeHtmlNXDXE8C(String modelId, String deviceTypeId,String gw_type)
	{
		StringBuffer bf = new StringBuffer();
		if (null == modelId || "-1".equals(modelId))
		{
		...
		{
			PrepareSQL psql = new PrepareSQL(" select a.devicetype_id,a.softwareversion from tab_devicetype_info a where a.device_model_id='"+modelId+"'");
			cursor = DataSetBean.getCursor(psql.getSQL());
			fields = cursor.getNext();
			if (null != fields)
			{
	...
	/*//河北联通，家庭网关界面修改
	public String getVersionFeature(Map field,String versionFeature)
	{
		//versionFeature 0 代表普通，1代表测速，2代表全路由
		logger.warn("start to getVersionFeature !");
		int vendor_id = 0;
		String vendor_id_string = (String) fields.get("vendor_id");
		if(!"".equals(vendor_id_string) || null != vendor_id_string)
		{
			vendor_id  = Integer.valueOf(vendor_id_string);
		}
		...
		{
			case 2: //华为
				if("HG8321R-GPON".equals(device_model))
				{
					if("V3R017C10S103".equals(softwareVersion))
					{
						versionFeature = "1";
					}
					else if ("V3R016C10S135".equals(softwareVersion) || "V3R016C10S138".equals(softwareVersion)) 
					{
						versionFeature = "2";
					}
					else 
				...
				else if("HG8347R-GPON".equals(device_model))
				{
					if("V3R016C10S115".equals(softwareVersion) || "V3R016C10S138".equals(softwareVersion))
					{
						versionFeature = "2";
					}
					else if("V3R017C10S103".equals(softwareVersion)) 
					{
						versionFeature = "1";
					}
				}else {
				...
				break;
			case 11://烽火
				if("HG261G".equals(device_model))
				{
					if("G00L1.04".equals(softwareVersion))
					{
						versionFeature = "1";
					}
					else if ("G00L1.03".equals(softwareVersion)) 
					{
						versionFeature = "2";
					}
					else 
				...
				else if("HG261GU".equals(device_model))
				{
					if("G00L2.00".equals(softwareVersion))
					{
						versionFeature = "2";
					}
					else if("G00L2.01".equals(softwareVersion) || "G00L2.02".equals(softwareVersion)) 
					{
						versionFeature = "1";
					}
					
				}
				else if ("HG260GS-U".equals(device_model)) 
				{
					if("G00L2.01".equals(softwareVersion))
					{
						versionFeature = "1";
					}
					else if ("G00L2.00".equals(softwareVersion)) 
					{
						versionFeature = "2";
					}
					else 
				...
				break;
			case  7://贝尔
				if("ALCATEL-I-120E".equals(device_model))
				{
					if ("3FE45290GFCB41".equals(softwareVersion) || "3FE56519AOCK46".equals(softwareVersion)) 
					{
						versionFeature = "2";
					}
					else 
				...
				else if("ALCL-I-120E".equals(device_model))
				{
					if("3FE45290GFCB41".equals(softwareVersion) || "3FE56519AOCK46".equals(softwareVersion))
					{
						versionFeature = "2";
					}
					else if("3FE45290GFCB35".equals(softwareVersion) || "3FE45290GFCB37".equals(softwareVersion)
							|| "3FE56519AOCK32".equals(softwareVersion)) 
					{
						versionFeature = "0";
					}
					else 
				...
				else if ("G-140W-UD".equals(device_model)) 
				{
					versionFeature = "1";
				}
				else if("ALCL-I-240W".equals(device_model))
				{
					if ("3FE46268GFEB01".equals(softwareVersion)) 
					{
						versionFeature = "2";
					}
					else if("3FE46268GFEB02".equals(softwareVersion))
					{
						versionFeature = "1";
					}
				}else {
				...
				break;
			case 10://中兴
				if("F407".equals(device_model) || "F407Za".equals(device_model) || "F627Za".equals(device_model))
				{
					versionFeature = "0";
				}
				else if("F607".equals(device_model))
				{
					if("V5.0.0P1T3".equals(softwareVersion))
					{
						versionFeature = "2";
					}
					else if("V5.0.0P1T5".equals(softwareVersion)) 
					{
						versionFeature = "1";
					}
					else 
				...
				else if("F607Za".equals(device_model))
				{
					if ("V6.0.0P1T3he".equals(softwareVersion)) 
					{
						versionFeature = "2";
					}
					else if("V6.0.0P1T5he".equals(softwareVersion))
					{
						versionFeature = "1";
					}
					else {
				...
				break;
			case 20://四川天邑
				if("TEWA-800G".equals(device_model) )
				{
					if("3.0.0".equals(softwareVersion) || "3.1".equals(softwareVersion) 
							|| "1.0".equals(softwareVersion))
					{
						versionFeature = "0";
					}
					else if("3.0.1".equals(softwareVersion) || "3.0.2".equals(softwareVersion)) 
					{
						versionFeature = "1";
					}
				}
				else if("TEWA-800E".equals(device_model))
				{
					if("3.0.1".equals(softwareVersion) || "3.0.2".equals(softwareVersion))
					{
						versionFeature = "1";
					}
					else if("3.0.0".equals(softwareVersion)) 
					{
						versionFeature = "0";
					}
				}else {
		...
		logger.warn("getVersionFeature  end : "+versionFeature);
