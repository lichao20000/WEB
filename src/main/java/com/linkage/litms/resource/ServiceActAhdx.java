 ...
 * 业务操作相关类 与tab_service tab_hgwcustomer相关
 */
public class ServiceActAhdx
{

	private static Logger logger = LoggerFactory.getLogger(ServiceAct.class);
	private Cursor cursor = null;
	private Map fields = null;
	private Cursor cursorEdition = null;
	private Map fieldsEdition = null;
	...
	/**
	 * 按版本统计设备数量ah_dx 
	 * e8b不统计，LAN上行的e8c和epon的不统计：设备型号有：TEWA-
	 * 300EA、HG226、HG220G-U、HG220、HG220GS、HG8120C-EPON、ZXA10 F460、ZTE
	 * F460、ZXA10F460、HG320、ZXV10 H618C、RG200O-CA； 统计总数大于1000台设备； 
	 * 是否规范版本 修改为‘是否统一版本‘,同时根据标记黄色部分新增一张中间表，导入记录报表id点后可以关联显示，方便后期维护；
	 * 设备最后一次更新上线时间大于6个月
	 * @param request
	 * @return
	 */
	public String getHtmlDeviceByEditionAHDX(HttpServletRequest request)
	{
		HttpSession session = request.getSession();
		UserRes curUser = (UserRes) session.getAttribute("curUser");
		...
		if (no_query == null || no_query.trim().equalsIgnoreCase(""))
		{
			...
			{
				sqlCount.append(" select count(a.device_id) as num,a.devicetype_id from tab_gw_device a,gw_device_model b,"
						+ " tab_devicetype_info t ,gw_devicestatus g where a.devicetype_id=t.devicetype_id and a.device_model_id=b.device_model_id "
						+ " and a.device_status = 1 and a.gw_type=1 and g.device_id = a.device_id and g.last_time > " + getSixMonthtTime()
						+ " and b.device_model not in"
						+ "('TEWA-300EA','HG226','HG220G-U','HG220','HG220GS','HG8120C-EPON','ZXA10 F460','ZTE F460','ZXA10F460','HG320','ZXV10 H618C','RG200O-CA')");
			}
			if (!StringUtil.IsEmpty(userCityID) && !"00".equals(userCityID))
			{
			...
			sqlCount.append(" group by a.devicetype_id having count(a.device_id) > 1000");
			...
			{
				sbTable.append("<TH nowrap>厂商</TH><TH>设备型号</TH><TH>版本达标率</TH><TH>软件版本</TH><TH>硬件版本</TH><TH style='word-break:keep-all'>是否统一版本</TH>");
			}
			// 在从数据库中 统计 业务属地对应的数据
			// 属地MAP 当前用户看到本身及其下属地市
			List cityList = new ArrayList();
			...
			{
				sql1.append(" select count(a.device_id) as num,a.devicetype_id ,a.city_id from tab_gw_device a,gw_device_model b,"
						+ " tab_devicetype_info t, gw_devicestatus g where a.devicetype_id=t.devicetype_id and t.rela_dev_type_id = 2 and a.device_model_id=b.device_model_id "
						+ " and a.device_status = 1 and a.gw_type=1 and g.device_id = a.device_id and g.last_time > " + getSixMonthtTime()
						+ " and b.device_model not in"
						+ "('TEWA-300EA','HG226','HG220G-U','HG220','HG220GS','HG8120C-EPON','ZXA10 F460','ZTE F460','ZXA10F460','HG320','ZXV10 H618C','RG200O-CA')");
				pSQL.setSQL(sql1.toString());
			...
			if(null != edtionIdStr && !edtionIdStr.isEmpty() && !"()".equals(edtionIdStr))
			{
				sql.append(" and t.devicetype_id in ");
				sql.append(edtionIdStr);
			}
			// 是否规范
			if (null != is_normal && !"null".equals(is_normal) && !"-1".equals(is_normal)
					&& !"true".equals(is_normal))
			...
			Map<String, String> mapEditionCount = new HashMap<String, String>();
			cursorEdition = getEditionCount(startTime, endTime);
			fieldsEdition = cursorEdition.getNext();
			while (fieldsEdition != null)
			{
				// 版本id+是否规范==>num 作为映射存入map中
				mapEditionCount.put(
						fieldsEdition.get("devicetype_id") + "_" + fieldsEdition.get("is_normal"),
						(String)fieldsEdition.get("num"));
				fieldsEdition = cursorEdition.getNext();
			}
			logger.warn("mapEditionCount is " + mapEditionCount);
			// 开始遍历属地,输出统计数据
			String isPrt = request.getParameter("isPrt");
			...
			while (it.hasNext())
			{
				...
				while (itDevicetype.hasNext())
				{
					...
					logger.warn("devicetypeList is " + devicetypeList);
					//版本达标率
					for (int i = 0; i < devicetypeList.size(); i++)
					{
						isnormalCount = isnormalCount + StringUtil.getIntValue(mapEditionCount, devicetypeList.get(i)+ "_" +"1");
						isnormalSum = isnormalSum + StringUtil.getIntValue(mapEditionCount, devicetypeList.get(i)+ "_" +"1")
										+ StringUtil.getIntValue(mapEditionCount, devicetypeList.get(i)+ "_" +"0");
					}
					logger.warn("isnormalCount is " + isnormalCount + " isnormalSum is" +isnormalSum);
					if(0 != isnormalSum)
					{
						percent = nf.format((float)isnormalCount/isnormalSum);
						logger.warn("percent is " + percent);
						//归零
						isnormalCount = 0;
						isnormalSum = 0;
					}
							...
							&& (sbTableModel != null))
					{
						sbTableVendor.append(
								"<TD nowrap rowspan='" + modelSize
										...
										+ "</TD>").append("<TD nowrap rowspan='" + modelSize
										+ "' class=column align='right'>" + percent
			...
			{
				sbTable.append("<tr><td nowrap align='right' class=column colspan=6>小计</td>");
			}
			sbTable.append(num);
			// Clear Resouce
			mapServiceCityNum.clear();
			...
			int gw_type, String userCityID, String cityIdhblt, String cityNextIdhblt)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if (("-1".equals(cityIdhblt) || "00".equals(cityIdhblt))
							&& ("-1".equals(cityNextIdhblt) || "00"
									.equals(cityNextIdhblt)))
					{
						sbTable.append("<td bgcolor=#ffffff align=center><a href=javascript:onclick=detail('-1','-1',"
								+ gw_type + ")>" + totalNum + "</a></td></tr>");
					}
			...
			String userCityID, int gw_type, String is_esurfing)
	{
		...
		else
		{
			bf.append(" select count(a.device_id) as num,a.devicetype_id,a.vendor_id,b.device_model from tab_gw_device a,gw_device_model b,"
					+ " tab_devicetype_info t , gw_devicestatus g where a.devicetype_id=t.devicetype_id and a.device_model_id=b.device_model_id "
					+ " and a.device_status = 1 and a.customer_id is not null and a.gw_type=1 and g.device_id = a.device_id and g.last_time > " + getSixMonthtTime()
					+ " and b.device_model not in"
					+ "('TEWA-300EA','HG226','HG220G-U','HG220','HG220GS','HG8120C-EPON','ZXA10 F460','ZTE F460','ZXA10F460','HG320','ZXV10 H618C','RG200O-CA')");
		}
		if(null != edtionIdStr && !edtionIdStr.isEmpty() && !"()".equals(edtionIdStr))
		{
			bf.append(" and t.devicetype_id in ");
			bf.append(edtionIdStr);
		}
		if (null != vendorId && !"null".equals(vendorId) && !"-1".equals(vendorId))
		{
			bf.append(" and a.vendor_id='" + vendorId + "' ");
	...
	private void getVersionMap(int gw_type)
	{
		...
		{
			sql = "select a.softwareversion,a.devicetype_id,b.device_model,c.vendor_id,a.hardwareversion,d.is_normal "
					+ " from tab_devicetype_info a, gw_device_model b,tab_vendor c ,tab_quality_temporary d"
					+ " where a.device_model_id=b.device_model_id and b.vendor_id=c.vendor_id and d.devicetype_id = a.devicetype_id";
		}
		if (versionMap == null)
		{
	 ...
	 * @return 
	 */
	public static long getSixMonthtTime()
	{
		Calendar cal = Calendar.getInstance();
	 ...
	 * @return cursor
	 */
	public Cursor getEditionCount(String startTime, String endTime)
	{
		StringBuffer sql = new StringBuffer();
		sql.append(" select count(a.device_id) as num,a.devicetype_id ,b.is_normal from tab_gw_device a,tab_quality_temporary b,gw_device_model c,"
				+ " tab_devicetype_info t, gw_devicestatus g where a.devicetype_id=t.devicetype_id and t.rela_dev_type_id = 2 and a.devicetype_id = b.devicetype_id "
				+ " and a.device_model_id=c.device_model_id and a.device_status = 1 and a.gw_type=1 and g.device_id = a.device_id and g.last_time > " + getSixMonthtTime());
		if (null != startTime && !"".equals(startTime))
		{
			sql.append(" and a.complete_time>");
			sql.append(startTime);
		...
		sql.append(" group by a.devicetype_id ,b.is_normal");
