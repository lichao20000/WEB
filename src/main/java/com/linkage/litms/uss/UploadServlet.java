...
public class UploadServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		/*
		 * Get message to show when upload is complete. Used only if a success
		 * redirect page is not supplied.
		 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			/*
			 * set content type of response and get handle to output stream in
			 * case we are unable to redirect client
			 */
			response.setContentType("text/plain");
			out = response.getOutputStream();
		} catch (IOException e) {
			// print error message to standard out
			logger.warn("Error getting output stream.");
			logger.warn("Error description: " + e);
			return;
		}
		try {
			...
			if (contentType != null && contentType.indexOf("multipart/form-data") != -1) {
				...
				if (file.indexOf("name=\"Directory\"") > 0) {
					...
					/*
					 * make sure user didn't select a directory higher in the
					 * directory tree
					 */
				...
				saveFile = saveFile.substring(saveFile.lastIndexOf("\\") + 1, saveFile
						.indexOf("\""));
				/*
				 * remove boundary markers and other multipart/form-data tags
				 * from beginning of upload file section
				 */
				...
				pos = file.indexOf("\n", pos) + 1;
				/*
				 * find the location of the next boundary marker (marking the
				 * end of the upload file data)
				 */
				...
				if (checkFile.exists()) {
					/*
					 * file exists, if OverWrite flag is off, give message and
					 * abort
					 */
				...
				/*
				 * create File object to check for existence of Directory
				 */
				...
				if (successPage.equals("")) {
					/*
					 * success HTML page URL not received, respond with generic
					 * success message
					 */
