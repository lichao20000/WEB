...
import com.linkage.module.gtms.stb.report.dao.SheetReportDAO;
 ...
 */
public class SheetReportBIO {
	
	// 日志记录
	private static Logger logger = LoggerFactory.getLogger(SheetReportBIO.class);
	
	private SheetReportDAO dao;
	...
	public List<Map<String, String>> getStatsReport(String cityId, String servTypeId, String startOpenDate, String endOpenDate)
	{
		...
		if("1".equals(servTypeId)){
			...
			for(Map<String,String> one:list){
				logger.warn("result:"+StringUtil.getIntegerValue(StringUtil.getIntegerValue(one.get("result"))));
				logger.warn("num:"+StringUtil.getIntegerValue(StringUtil.getIntegerValue(one.get("num"))));
				if(0==StringUtil.getIntegerValue(one.get("result"))){
					succ = StringUtil.getIntegerValue(one.get("num"));
				}
				else if(1==StringUtil.getIntegerValue(one.get("result"))){
			...
			for(String city:cityList){
				...
				int falut = 0;
				for(Map<String,String> one:list){
					if(city.equals(one.get("city_id")) && 0==StringUtil.getIntegerValue(one.get("result"))){
						succ = StringUtil.getIntegerValue(one.get("num"));
					}
					else if(city.equals(one.get("city_id")) && 1==StringUtil.getIntegerValue(one.get("result"))){
						falut = StringUtil.getIntegerValue(one.get("num"));
					}
				}
				Map<String,String> resOne = new HashMap<String,String>();
		...
		for(Map<String,String> resOne : result){
			...
			resOne.put("all", StringUtil.getStringValue(succ+fault));
		}
	...
	public SheetReportDAO getDao() {
		return dao;
	}

	public void setDao(SheetReportDAO dao) {
		this.dao = dao;
	}
