 ...
 */
public class SheetCustReportDAO extends SuperDAO{

	// 日志记录
	private static Logger logger = LoggerFactory.getLogger(SheetCustReportDAO.class);
	...
	public List<Map<String,String>> getStatsGroupByCity(String cityId, String startOpenDate, String endOpenDate)
	{
		...
		if(StringUtil.IsEmpty(cityId)||"00".equals(cityId)){
			sb.append("select city_id, user_status, count(*) as num from stb_tab_customer where 1=1 ");
		}
		else{
			sb.append("select city_id, user_status, count(*) as num from stb_tab_customer where city_id='"+cityId+"'");
		}

		if (!StringUtil.IsEmpty(startOpenDate))
		{
			sb.append(" and updatetime>=").append(startOpenDate);
		}
		if (false == StringUtil.IsEmpty(endOpenDate))
		{
			sb.append(" and updatetime<=").append(endOpenDate);
		}

		sb.append(" group by city_id,user_status");
		sb.append(" order by city_id,user_status");

		PrepareSQL psql = new PrepareSQL(sb.toString());
		List<Map<String,String>> list = jt.queryForList(psql.getSQL());
		logger.warn(".............getStatsGroupByCity="+list.toString());
		print(list);
	...
	public List<Map<String,String>> getStats(String cityId, String startOpenDate, String endOpenDate)
	{
		...
		StringBuffer sb = new StringBuffer();
		if(StringUtil.IsEmpty(cityId)||"00".equals(cityId)){
			sb.append("select user_status, count(*) as num from stb_tab_customer where 1=1 ");
		}
		else{
			sb.append("select user_status, count(*) as num from stb_tab_customer where city_id='"+cityId+"'");
		}
		...
		sb.append(" group by user_status");
