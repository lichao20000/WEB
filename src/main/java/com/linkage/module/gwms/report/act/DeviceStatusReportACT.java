 ...
 * 
 */
public class DeviceStatusReportACT extends ActionSupport implements ServletRequestAware {
	/**
	 * 序列化
	 */
	 ...
	 * bio
	 */
	I_DeviceStatusReportBIO deviceStatusReportBIO;
	...
	public String getDayReport() throws ParseException {
		...
		if ("1".equals(reportType)) {
			...
			this.reportResult = deviceStatusReportBIO.getReportData(cityId,startDate, endDate);
		...
		if ("3".equals(reportType)) {
			...
			long endDate = simpleDateFormat2.parse(strEndDate.toString())
					.getTime();
			this.reportResult = deviceStatusReportBIO.getReportData(cityId,
					startDate, endDate);
		...
		if("excel".equals(isReport)){
			...
			//报表
			if ("1".equals(reportType)) {
				this.pdfTitle = "终端状态小时统计";
				this.pdfFileName = "deviceStatus";
				this.pdfArrAata = this.reportResult;
			}
			//日报表
			if ("2".equals(reportType)) {
				this.pdfTitle = "终端状态日统计";
				this.pdfFileName = "deviceStatus";
				this.pdfArrAata = this.reportResult;
			}
			//周报表
			if ("3".equals(reportType)) {
				this.pdfTitle = "终端状态周统计";
				this.pdfFileName = "deviceStatus";
				this.pdfArrAata = this.reportResult;
			}
			//月报表
			if ("4".equals(reportType)) {
				this.pdfTitle = "终端状态月统计";
				this.pdfFileName = "deviceStatus";
				this.pdfArrAata = this.reportResult;
			}
	...
	/**
	 * @return the deviceStatusReportBIO
	 */
	public I_DeviceStatusReportBIO getDeviceStatusReportBIO() {
		return deviceStatusReportBIO;
	}

	/**
	 * @param deviceStatusReportBIO the deviceStatusReportBIO to set
	 */
	public void setDeviceStatusReportBIO(
			I_DeviceStatusReportBIO deviceStatusReportBIO) {
		this.deviceStatusReportBIO = deviceStatusReportBIO;
	}
