...
import com.linkage.module.gwms.report.act.interf.I_SheetFailureReasonACT;
...
public class SheetFailureReasonACT extends ActionSupport implements
		ServletRequestAware, I_SheetFailureReasonACT {
	 ...
	 * bio
	 */
	I_SheetFailureReasonBIO sheetFailureReasonBIO;
	...
	public String execute() throws Exception {
		...
		List list = sheetFailureReasonBIO.getFaultCode();	
		...
		for(int i=0;i<list.size();i++){
			...
			bf.append(one.get("fault_code"));
			bf.append("|");
			bf.append(one.get("fault_desc"));
			bf.append("(");
			bf.append(one.get("fault_code"));
			bf.append(")");
		}
	...
	public String getDayReport() throws ParseException {
		...
		if ("1".equals(reportType)) {
			...
			this.reportResult = sheetFailureReasonBIO.getReportData(cityId,startDate, endDate,bookparam);
		...
		if ("3".equals(reportType)) {
			...
			long endDate = simpleDateFormat2.parse(strEndDate.toString())
					.getTime();
			this.reportResult = sheetFailureReasonBIO.getReportData(cityId,
		...
		if("excel".equals(isReport)){
			...
			//报表
			if ("1".equals(reportType)) {
				this.pdfTitle = "业务下发失败原因小时统计";
				this.pdfFileName = "sheetFailure";
				this.pdfArrAata = this.reportResult;
			}
			//日报表
			if ("2".equals(reportType)) {
				this.pdfTitle = "业务下发失败原因日统计";
				this.pdfFileName = "sheetFailure";
				this.pdfArrAata = this.reportResult;
			}
			//周报表
			if ("3".equals(reportType)) {
				this.pdfTitle = "业务下发失败原因周统计";
				this.pdfFileName = "sheetFailure";
				this.pdfArrAata = this.reportResult;
			}
			//月报表
			if ("4".equals(reportType)) {
				this.pdfTitle = "业务下发失败原因月统计";
				this.pdfFileName = "sheetFailure";
				this.pdfArrAata = this.reportResult;
			}
	...
	/**
	 * @return the sheetFailureReasonBIO
	 */
	public I_SheetFailureReasonBIO getSheetFailureReasonBIO() {
		return sheetFailureReasonBIO;
	}

	/**
	 * @param sheetFailureReasonBIO the sheetFailureReasonBIO to set
	 */
	public void setSheetFailureReasonBIO(
			I_SheetFailureReasonBIO sheetFailureReasonBIO) {
		this.sheetFailureReasonBIO = sheetFailureReasonBIO;
	}
