 ...
 * @since 2019-4-15
 */
public class TyGateAgreeCountReportDAO extends SuperDAO{

	Logger logger = LoggerFactory.getLogger(TyGateAgreeCountReportDAO.class);
	...
	/**	if(DBUtil.GetDB()==Global.DB_MYSQL){
		...
		ppSQL.setSQL("select count( distinct a.device_id)  from  ");
		ppSQL.append("  tab_gw_device a");
		...
		if (!StringUtil.IsEmpty(isExcludeUpgrade)) {
			...
			ppSQL.append(" and not exists (select 1 from gw_serv_strategy_soft abc  " +
					"where abc.device_id = a.device_id and abc.start_time >="+time+")  ");
		}
			...
			String startTime1,String endTime1,String recent_start_Time1,String recent_end_Time1)
	{
		...
		if(DBUtil.GetDB()==Global.DB_MYSQL){
			//TODO wait
			//vendor_id,device_model_id,hardwareversion,sum1,sum2,total
			if(Global.JLDX.equals(Global.instAreaShortName)
					|| Global.NXDX.equals(Global.instAreaShortName)){
				sql.append(" select vendor_id,device_model_id,hardwareversion,sum1,sum2,total from " +
						"(select a.vendor_id,a.device_model_id,a.hardwareversion,a.sum1,a.sum2,a.total, case when a.total=0 then '0%' else  concat(cast(round((a.sum1 / a.total * 100), 2) as char),'%')  end  total_percent from ( \n");
			}else{
				sql.append(" select a.vendor_id,a.device_model_id,a.hardwareversion,a.sum1,a.sum2,a.total, case when a.total=0 then '0%' else  concat(cast(round((a.sum1 / a.total * 100), 2) as char),'%')  end  total_percent from ( \n");
			}
		}else{
			if(Global.JLDX.equals(Global.instAreaShortName)
					|| Global.NXDX.equals(Global.instAreaShortName)){
				sql.append("select * from (select a.*, case when a.total=0 then '0%' else  convert(varchar(8),round((a.sum1 / a.total * 100), 2)) +'%'  end  total_percent from ( \n");
			}else{
				sql.append("select a.*,case when a.total=0 then '0%' else round((a.sum1 / a.total * 100), 2) ||'%'  end  total_percent from ( \n");
			}
		}
		...
		sql.append(" ,tab_vendor                   b\n");
		sql.append(" ,gw_device_model              c\n");
		...
		if(!Global.JLDX.equals(Global.instAreaShortName)
			...
			sql.append(") a) a order by a.vendor_id, a.device_model_id, a.hardwareversion \n");
		}
		...
		List<Map<String,String>> list = jt.query(psql.getSQL(), new RowMapper()
		{
			public Object mapRow(ResultSet rs, int arg1) throws SQLException
			{
				...
				map.put("total_percent",
						new BigDecimal(rs.getDouble("sum1")/rs.getDouble("total")*100).setScale(2, RoundingMode.HALF_UP)+"%");
				return map;
			}
		});
		return list;
	...
	public List<Map<String,String>>  queryDataListSDDX(String querydata,String gw_type,String vendorId,String deviceModelId,String isExcludeUpgrade,
		...
		if (flag>0) {
			String sql1="select a.*, v.totalnum ,convert(decimal(10,2), round((a.num / v.totalnum * 100), 2)) total_percent from tab_unificationreport_temp a"
