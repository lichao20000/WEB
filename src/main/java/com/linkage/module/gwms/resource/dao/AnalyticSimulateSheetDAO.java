...
public class AnalyticSimulateSheetDAO extends SuperDAO
{
	private static Logger logger = LoggerFactory.getLogger(AnalyticSimulateSheetDAO.class);
	
	public Map getCityMap()
	{
		PrepareSQL psql = new PrepareSQL();
		if(DBUtil.GetDB()==3){
			psql.append("select city_name,city_id from tab_city");
		}else{
			psql.append("select * from tab_city");
		}
		List <Map>list=new ArrayList<Map>();
		Map map=new HashMap();
		list=jt.queryForList(psql.getSQL());
		for(int i=0;i<list.size();i++){
			map.put(StringUtil.getStringValue(list.get(i).get("city_name")), 
				StringUtil.getStringValue(list.get(i).get("city_id")));
		}
		return map;
	}
	...
	public Map getAccessType()
	{
		PrepareSQL psql = new PrepareSQL();
		if(DBUtil.GetDB()==3){
			psql.append("select type_name,type_id from gw_access_type");
		}else{
			psql.append("select * from gw_access_type");
		}
		List <Map>list=new ArrayList<Map>();
		Map map=new HashMap();
		list=jt.queryForList(psql.getSQL());
		for(int i=0;i<list.size();i++){
			map.put(StringUtil.getStringValue(list.get(i).get("type_name")), 
				StringUtil.getStringValue(list.get(i).get("type_id")));
		}
		return map;
	}
	...
	public int checkHgwcustServInfo(String userId) 
	{
		...
		psql.append("from hgwcust_serv_info where user_id = "+userId+" and serv_type_id = 14");
	...
	public String insertAllTableInfo(String userId, String servTypeId, String operateType, 
			String username,String wanType,String ipaddress,String ipmask,String gateway,
			String adslSer,String vlanid,String vpiid,String vciid,String mgcIp,int mgcPort,
			String standMgcIp,int standMgcPort,String voipPort,String regId,String regIdType,String voipTelepone)
	{
