...
public class GwBindDevQueryDAO extends SuperDAO 
{
	private static Logger logger = LoggerFactory.getLogger(GwBindDevQueryDAO.class);
		...
		if (null != searchTxt) {
			if ("username".equals(gwShare_queryField)) 
			{
				...
				if (LipossGlobals.IsITMS()) {
					table="tab_hgwcustomer";
				}
				pSQL.append("from "+table+" a,tab_gw_device b ");
			...
			{
				if(DBUtil.GetDB()==3){
					pSQL.setSQL("select concat(device_serialnumber,'|',loopback_ip) as device_sn ");
				}else{
					pSQL.setSQL("select top 5 device_serialnumber + '|' + loopback_ip as device_sn ");
				}
	...
	public int getTmpList() {
		...
		if(DBUtil.GetDB()==3){
			...
			sb.append("select count(1) from tab_softwareup_tmp");
		}
	...
	public void insertTmp(String fileName, List<String> dataList,
		...
		if ("username".equals(importQueryField)
			...
			for (int i = 0; i < dataList.size(); i++) {
				psql = new PrepareSQL();
				psql.append("insert into tab_tmp_accounts "
	...
	public List<Map> newDeviceQueryInfo4Simple(String loid, String username, int curPage_splitPage,
			int num_splitPage) 
	{
	...
	/**	if(DBUtil.GetDB()==3){
		...
		sql.append("select b.cityid,b.oper_type,b.loid,b.username,b.old_devsn,b.new_devsn,"
				+ " b.unbinddate,b.binddate,c.vendor_name as old_vendor_name,d.device_model as old_device_model,"
				+ " e.hardwareversion as old_hardwareversion,e.softwareversion as old_softwareversion "
				+ "from tab_dev_bindrecord b,tab_vendor c,gw_device_model d,tab_devicetype_info e "
				+ "where b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
				+ "and b.old_devicetype_id=e.devicetype_id  ");
		PrepareSQL psql = new PrepareSQL(sql.toString());
		if (null != loid && !"null".equals(loid)
				&& !"".equals(loid) && !"-1".equals(loid)) {
			psql.appendAndString("b.loid", PrepareSQL.EQUEAL, loid);
		}
		...
		list = querySP(psql.getSQL(), (curPage_splitPage - 1) * num_splitPage
			...
			public Object mapRow(ResultSet rs, int arg1) throws SQLException {
				...
				String cityId = StringUtil.getStringValue(rs.getString("cityid"));
				map.put("cityId", cityId);
				map.put("cityName", cityIdToFirstCityName(cityId));
	...
	/**	if(DBUtil.GetDB()==3){
		...
		String sql1 = "select c.vendor_name,d.device_model,e.hardwareversion,e.softwareversion "
				+ "from tab_dev_bindrecord b,tab_vendor c,gw_device_model d,tab_devicetype_info e "
				+ "where b.new_vendor_id=c.vendor_id and b.new_model_id=d.device_model_id "
				+ "and b.new_devicetype_id=e.devicetype_id ";

		Map a = null;
		for (Map map : list) {
			PrepareSQL psql1 = new PrepareSQL(sql1.toString());
			psql1.appendAndString("b.loid", PrepareSQL.EQUEAL,
					(String) map.get("loid"));
	...
	public List<Map> newDeviceQueryInfo(String start_time, String end_time,
		...
		if(!StringUtil.IsEmpty(isNewNeed) && "yes".equals(isNewNeed)){
			...
			String sqlstr = "select b.cityid,b.oper_type,b.loid,b.username,b.old_devsn,b.new_devsn,b.unbinddate,b.binddate,"
						+ " c.vendor_name as old_vendor_name,d.device_model as old_device_model,"
						+ " e.hardwareversion as old_hardwareversion,e.softwareversion as old_softwareversion "
					+ "from tab_dev_bindrecord b, tab_vendor c, gw_device_model d,tab_devicetype_info e,"
						+ " tab_device_version_attribute f ,tab_device_version_attribute g "
					+ "where b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
					+ "and b.old_devicetype_id=e.devicetype_id and e.devicetype_id=f.devicetype_id "
			...
			sql.append("select b.cityid,b.oper_type,b.loid,b.username,b.old_devsn,b.new_devsn,b.unbinddate,b.binddate,"
						+ " c.vendor_name as old_vendor_name,d.device_model as old_device_model,"
						+ " e.hardwareversion as old_hardwareversion,e.softwareversion as old_softwareversion "
					+ "from tab_dev_bindrecord b,tab_vendor c,gw_device_model d,tab_devicetype_info e "
					+ "where b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
		...
		if (!StringUtil.IsEmpty(cityId) && !"-1".equals(cityId)) {
			//00 只查省中心 宁夏 光猫更换统计 特殊用法
			if("00".equals(cityId))
			{
				sql.append(" and b.cityid = '" +cityId +"'");
			}
			else 
			{
	...
	public int countNewDeviceQueryInfo4Simple(String loid, String username,
			int curPage_splitPage, int num_splitPage)
	{
		...
		sql.append("from tab_dev_bindrecord b,tab_vendor c,gw_device_model d,tab_devicetype_info e "
				+ "where b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
				+ "and b.old_devicetype_id=e.devicetype_id ");
		PrepareSQL psql = new PrepareSQL(sql.toString());
		if (null != loid && !"null".equals(loid) && !"".equals(loid)
				&& !"-1".equals(loid))
		{
			psql.appendAndString("b.loid", PrepareSQL.EQUEAL, loid);
		}
	...
	public int countNewDeviceQueryInfo(String start_time, String end_time,
		...
		if(!StringUtil.IsEmpty(isNewNeed) && "yes".equals(isNewNeed)){
			...
			sqlstr += "from tab_dev_bindrecord b, tab_vendor c, gw_device_model d,tab_devicetype_info e ,"
						+ " tab_device_version_attribute f ,tab_device_version_attribute g "
					+ "where b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
					+ "and b.old_devicetype_id=e.devicetype_id and e.devicetype_id=f.devicetype_id "
					+ "and b.old_devicetype_id=f.devicetype_id and f.is_tygate=0 "
	...
	public List<Map> NewDeviceQueryExcel(String start_time, String end_time,
		...
		if (!StringUtil.IsEmpty(cityId) && !"-1".equals(cityId) && !"0".equals(cityId)) {
			//00 只查省中心 宁夏 光猫更换统计 特殊用法
			if("00".equals(cityId))
			{
				sql.append(" and b.cityid = '" +cityId +"'");
		...
		if (null != list && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				String cityId1 = StringUtil.getStringValue(list.get(i).get("cityid"));				
				list.get(i).put(
						"cityId",cityId1);
				list.get(i).put(
						"cityName",cityIdToFirstCityName(cityId1));
				list.get(i).put(
						"old_vendor_name",
	...
	public List queryDeviceByImportUsername(String gw_type, long areaId,
		...
		if(DBUtil.GetDB()==3){
			//TODO wait
			pSQL.append("select b.loid,b.old_oui,b.old_devsn,b.new_devsn,b.cityid,b.username,");
		}else{
			pSQL.append("select b.*,");
		}
		pSQL.append("c.vendor_name,d.device_model,e.hardwareversion,e.softwareversion "
				+ "from tab_tmp_accounts a,tab_dev_bindrecord b,tab_vendor c,gw_device_model d,tab_devicetype_info e "
				+ "where a.username=b.username and b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
				+ "and b.old_devicetype_id=e.devicetype_id "
	...
	public List queryDeviceByImportUsername1(int curPage_splitPage,
		...
		pSQL.setSQL("select b.cityid,b.oper_type,b.loid,b.username,b.old_devsn,b.new_devsn,b.unbinddate,b.binddate,"
					+ " c.vendor_name as old_vendor_name,d.device_model as old_device_model,"
					+ " e.hardwareversion as old_hardwareversion,e.softwareversion as old_softwareversion "
				+ "from tab_tmp_accounts a,tab_dev_bindrecord b,tab_vendor c, gw_device_model d,tab_devicetype_info e "
				+ "where a.username=b.username and b.old_vendor_id=c.vendor_id and b.old_model_id=d.device_model_id "
				+ "and b.old_devicetype_id=e.devicetype_id and a.filename = '" + fileName + "'");
	...
	public int queryDeviceBynumber(String gw_type, long areaId, String cityId,
		...
		pSQL.append("from tab_tmp_accounts a,tab_dev_bindrecord b,tab_vendor c,"
	...
	public List queryDeviceByImportUsernameForDoubleParam(String gw_type,
		...
		pSQL.append("c.vendor_name,d.device_model,e.hardwareversion,e.softwareversion "
				+ "from tab_tmp_accounts a,tab_dev_bindrecord b,tab_vendor c, "
					+ "gw_device_model d,tab_devicetype_info e "
	...
	public List queryDeviceByImportKDUsername(String gw_type, long areaId,
		...
		if(DBUtil.GetDB()==3){
			//TODO wait
			pSQL.append("select loid,old_oui,old_devsn,new_devsn,vendor_name,cityid,hardwareversion,");
		}else{
	...
	public int queryDeviceByKDnumber(String gw_type, long areaId,
		...
		pSQL.append(" and g.username=f.username and g.user_id=e.user_id and g.serv_type_id=10 "
	...
	public List queryDeviceByImportDevicesnForDoubleParam(String gw_type,
		...
		if(DBUtil.GetDB()==3){
			//TODO wait
			pSQL.append("select loid,old_oui,old_devsn,new_devsn,vendor_name,cityid,hardwareversion,username,");
		}else{
			pSQL.append("select a.*,f.*,");
		}
		pSQL.append("b.vendor_add,c.device_model,d.softwareversion from tab_gw_device a,tab_vendor b,gw_device_model c,tab_devicetype_info d, tab_seniorquery_tmp f ");
	...
	{
		PrepareSQL pSQL = new PrepareSQL();
		pSQL.append("select d.is_awifi from tab_gw_device a,tab_vendor b,gw_device_model c,tab_devicetype_info d ");
		pSQL.append("where a.device_status =1 and a.vendor_id=b.vendor_id ");
		pSQL.append("and a.device_model_id=c.device_model_id and a.devicetype_id=d.devicetype_id ");
		if (null != queryParam && !"".equals(queryParam)
				&& !"-1".equals(queryParam)) {
			...
			String queryParam, String cityId) 
	{
		...
		if(DBUtil.GetDB()==3){
			...
			pSQL.append("a.office_id,a.complete_time,a.buy_time,a.staff_id,a.remark,a.loopback_ip,");
			pSQL.append("a.interface_id,a.device_status,a.gather_id,a.devicetype_id,");
			pSQL.append("a.maxenvelopes,a.cr_port,a.cr_path,a.cpe_mac,a.gw_type,a.cpe_currentupdatetime,");
			pSQL.append("a.cpe_allocatedstatus,a.cpe_username,a.cpe_passwd,a.acs_username,a.acs_passwd,");
			pSQL.append("a.device_type,a.x_com_username,a.x_com_passwd,a.device_model_id,");
			pSQL.append("a.customer_id,a.device_url,a.x_com_passwd_old,a.vendor_id,");
		}else{
			...
			int num_splitPage, long areaId, String queryParam, String cityId) 
	{
		...
		PrepareSQL pSQL = new PrepareSQL();
		if (DBUtil.GetDB() == 2) {
			pSQL.append("select top " + maxNum+" a.*,");
	...
	{
		PrepareSQL pSQL = new PrepareSQL();
		pSQL.append("select d.is_awifi from tab_gw_device a,tab_devicetype_info d ");
			...
			String bindType, String deviceSerialnumber, String deviceIp) 
	{
		...
		if(DBUtil.GetDB()==3){
			...
			pSQL.setSQL("select count(1) from tab_gw_device a where a.device_status =1");
		}
			...
			String username) 
	{
		...
		pSQL.append("from tab_gw_device a,tab_vendor b,gw_device_model c,tab_devicetype_info d,");
		pSQL.append(tableName);
		pSQL.append(" e where a.device_id=e.device_id and a.device_status=1 and a.vendor_id=b.vendor_id ");
		pSQL.append("and a.device_model_id=c.device_model_id and a.devicetype_id=d.devicetype_id ");
		if (null != cityId && !"null".equals(cityId) && !"".equals(cityId)
				&& !"-1".equals(cityId) && !"00".equals(cityId)) {
			ArrayList cityArray = CityDAO.getAllNextCityIdsByCityPid(cityId);
			...
			int num_splitPage, long areaId, String cityId, String username) 
	{
		...
		if (DBUtil.GetDB() == 2) {
			...
			pSQL.append("c.office_id,c.complete_time,c.buy_time,c.staff_id,c.remark,c.loopback_ip,");
			pSQL.append("c.interface_id,c.device_status,c.gather_id,c.devicetype_id,");
			pSQL.append("c.maxenvelopes,c.cr_port,c.cr_path,c.cpe_mac,c.gw_type,c.cpe_currentupdatetime,");
			pSQL.append("c.cpe_allocatedstatus,c.cpe_username,c.cpe_passwd,c.acs_username,c.acs_passwd,");
			pSQL.append("c.device_type,c.x_com_username,c.x_com_passwd,c.device_model_id,");
			pSQL.append("c.customer_id,c.device_url,c.x_com_passwd_old,c.vendor_id,");
		} else {
			pSQL.append("select a.*,");
		}
		pSQL.append("b.vendor_add,c.device_model,d.softwareversion,d.is_awifi ");
		...
		pSQL.append("from tab_gw_device a,tab_vendor b,gw_device_model c,tab_devicetype_info d,");
		pSQL.append(tableName);
		pSQL.append(" e where a.device_id=e.device_id and a.device_status =1 ");
	...
	public List queryDevice(String gw_type, String city_id, String voipPhoneNum) 
	{
		...
		psql.append("from tab_hgwcustomer a,tab_voip_serv_param b,tab_gw_device c,tab_vendor d,");
			...
			int num_splitPage, String city_id, String voipPhoneNum) 
	{
		...
		PrepareSQL psql = new PrepareSQL();
		if (DBUtil.GetDB() == 2) {
			psql.append("select top " + maxNum+" c.*,");
		} else if(DBUtil.GetDB()==3){
			//TODO wait
			psql.append("select c.device_id,c.oui,c.device_serialnumber,c.device_name,c.city_id,");
			psql.append("c.office_id,c.complete_time,c.buy_time,c.staff_id,c.remark,c.loopback_ip,");
	...
	public int Kd(String gw_type, String cityId, String kdname) 
	{
		...
		psql.append("from tab_hgwcustomer a, hgwcust_serv_info b, tab_gw_device c where a.device_id=c.device_id and a.user_id = b.user_id ");
		// psql.append("   and c.device_status = 1"); // 设备已确认
			...
			ResultSet rs) 
	{
		try 
		{
			...
			//buy_time,staff_id,remark,loopback_ip,interface_id,device_status,gather_id,
			//devicetype_id,softwareversion,maxenvelopes,cr_port,cr_path,cpe_mac,gw_type,
			//cpe_currentupdatetime,cpe_allocatedstatus,cpe_username,cpe_passwd,
			//acs_username,acs_passwd,device_type,x_com_username,x_com_passwd,
			//device_model_id,device_model,customer_id,device_url,x_com_passwd_old,vendor_id,vendor_add,is_awifi,	
	...
	public Map<String, String> resultSet2Map4Wireless(Map<String, String> map,
			ResultSet rs) {
		try {
			//loid,old_oui,old_devsn,new_devsn,vendor_name,device_model,cityid,hardwareversion,username
			...
			map.put("device_model", rs.getString("device_model"));
			map.put("city_name",
					CityDAO.getCityIdCityNameMap().get(rs.getString("cityid")));
	...
	/**
	 * cityId转换为地级市名称(NXDX专用)
	 * @param cityId
	 * @return
	 */
	public String cityIdToFirstCityName(String cityId) {
		String cityName = "";
		try {
			if(cityId != null&&cityId.length()>=3) {
				cityId = cityId.substring(0, 3);
			}
