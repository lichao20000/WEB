 ...
 *
 */
public class HttpTestBatchReportDAO extends SuperDAO
{
	private Logger logger = LoggerFactory.getLogger(HttpTestBatchReportDAO.class);
	...
	public  Map<String, String> qryRouteList(String cityId,int statCaliber,String startTime,String endTime){
		...
		sb.append("from (select row_number() over(partition by loid order by avgsampledtotalvalues+0 desc ) rn,");
		sb.append(" b.avgsampledtotalvalues, b.loid  from tab_http_diag_result_intf_new b ");
	...
	public Map<String, String> qryBridgedList(String cityId,int statCaliber,String startTime,String endTime){
		...
		sb.append("from (select row_number() over(partition by loid order by avgsampledtotalvalues+0 desc ) rn ,");
		sb.append("  b.avgsampledtotalvalues, b.loid ,b.speed from tab_http_diag_result_intf_new b");
		sb.append(" where b.wan_type='PPPoE_Bridged' ");
		if(statCaliber != -1 && statCaliber != 0){
			sb.append(" and b.clienttype="+statCaliber);
		}
		if(!StringUtil.IsEmpty(startTime)){
			...
			int curPage_splitPage,int num_splitPage,boolean isTrun)
	{
		...
		if(DBUtil.GetDB()==3){
			pSQL.append("select loid,vendor_add,device_model,softwareversion,hardwareversion,");
	 ...
	 public List<HashMap<String,String>> qryDetailExcel()
	 {
		PrepareSQL pSQL = new PrepareSQL();
		if(DBUtil.GetDB()==3){
			pSQL.append("select loid,vendor_add,city_name,device_model,hardwareversion");
			pSQL.append("softwareversion,speed,test_rate,wan_type,test_time ");
			pSQL.append("from tab_http_diag_detail_result");
		}else{
			pSQL.append("select * from tab_http_diag_detail_result");
		}
	...
	public  List<String> qryRouteTemp(String cityId,int statCaliber,String startTime,String endTime,String type){
		...
		sb.append("select a.loid,a.test_time,d.downlink,a.rate,a.testusername,f.softwareversion,");
		sb.append("f.hardwareversion,g.device_model,h.vendor_add,m.city_name ,aa.gbbroadband ");
		...
		sb.append("from (select row_number() over(partition by loid order by AVGSAMPLEDTOTALVALUES+0 desc ) rn," +
				" bb.AVGSAMPLEDTOTALVALUES, bb.loid,bb.device_id,bb.testusername,bb.test_time ");
		sb.append("from tab_http_diag_result_intf_new bb ");
		...
		try
		{
			...
			if(null != maps && !maps.isEmpty() && maps.size() > 0){
				for(Map<String,String> map : maps){
					...
					sql.append(","+StringUtil.getSQLString(map.get("rate")));
	...
	public List<String> qryBridgedTemp(String cityId,int statCaliber,String startTime,String endTime,String type)
	{
		...
		StringBuilder sb = new StringBuilder();
		sb.append("select a.loid,a.test_time,a.speed,a.rate,a.testusername,");
