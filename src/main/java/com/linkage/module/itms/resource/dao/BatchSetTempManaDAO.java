...
{
	private static Logger logger = LoggerFactory.getLogger(BatchSetTempManaDAO.class);
	...
	public List<Map> queryTaskList(String task_name_query, int status_query,
		...
		// teledb
		if (DBUtil.GetDB() == 3) {
			psql.append("select t.task_name, t.task_id, t.task_status, t.add_time, temp.name template_name from tab_batchSetTemplate_task t,tab_serv_template temp");
		}
		else {
			psql.append("select t.*,temp.name template_name from tab_batchSetTemplate_task t,tab_serv_template temp");
		}
				...
				+ 1, num_splitPage, new RowMapper()
		{
			public Object mapRow(ResultSet rs, int arg1) throws SQLException
			{
				...
				if(!StringUtil.IsEmpty(taskId)){
					...
					String strategyTab = "tab_batchSetTemplate_dev";
	...
	{
		Map<String,String> map=new HashMap<String,String>();
		String table="tab_batchSetTemplate_task";
		
		PrepareSQL sql = new PrepareSQL();
		sql.append("select t.DONOW,t.TASK_NAME,t.ADD_TIME,t.QUERYTYPE,t.CITY_ID,t.VENDOR_ID,t.DEVICE_MODEL_ID,t.DEVICETYPE_ID,t.ISBIND,t.FILE_NAME,t.TYPE,t.START_TIME,t.END_TIME,a.acc_loginname,temp.NAME,temp.id from tab_serv_template temp,tab_batchSetTemplate_task t left join tab_accounts a on a.acc_oid=t.acc_oid where t.TEMPLATE_ID=temp.id and t.task_id=? ");
	...
	public int queryTaskCount(String task_name_query, int status_query,
			...
			int num_splitPage,String gw_type) {
		PrepareSQL psql = new PrepareSQL();
		psql.append("select count(*) from tab_batchSetTemplate_task t,tab_serv_template temp");
		psql.append(" where temp.id=t.template_id ");
	...
	public List<Map> queryDevList(String task_id, String type, int curPage_splitPage, int num_splitPage)
	{
		...
		if(null != task_id || "".equals(task_id)){
			PrepareSQL psql = new PrepareSQL();
			psql.append("select d.vendor_id,d.city_id,d.device_model_id,d.oui,d.loopback_ip,d.device_serialnumber,d.devicetype_id,s.status from "+strategyTab+" s,tab_gw_device d where d.device_id=s.device_id and s.task_id = '");
			psql.append(task_id);
			psql.append("'");
	...
	public int del(String task_id)
	{
		...
		sqls[0] = "delete from tab_batchSetTemplate_task where task_id="+task_id;
		sqls[1] = "delete from tab_batchSetTemplate_dev where task_id="+task_id;
		sqls[2] = "delete from gw_serv_strategy_batch where task_id='"+task_id+"'";
	...
	{
		PrepareSQL psql = new PrepareSQL();
		psql.append("select count(*) from tab_batchSetTemplate_dev where task_id='"+task_id+"'");//删除未作的策略
	...
	public int update(String task_id,String type)
	{
		...
		String strategyTab = "gw_serv_strategy_batch" ;
		if(LipossGlobals.inArea(Global.YNLT)){
			strategyTab = "gw_serv_strategy";
		}
		//当前状态：1为正常，2为暂停
		if("1".equals(type)){
			sqls[0] = "update tab_batchSetTemplate_task set task_status=2 where task_id="+task_id;
			...
			sqls[0] = "update tab_batchSetTemplate_task set task_status=1 where task_id="+task_id;
	...
	public ArrayList<HashMap<String, String>> continueIds(String task_id)
	{
		...
		if(LipossGlobals.inArea(Global.YNLT)){
			psql.append("select device_id from gw_serv_strategy where status=0 and service_id=5 and task_id='"+task_id+"'");//删除未作的策略
		}else{
			psql.append("select * from gw_serv_strategy_batch where status=0 and task_id='"+task_id+"'");//删除未作的策略
		}
	...
	public int repeatTaskName(String taskName)
	{
		...
		// teledb
		if (DBUtil.GetDB() == 3) {
			pSQL.append("select count(*) from tab_time_softup_task where task_name = '");
		}
		else {
	...
	{
		PrepareSQL pSQL = new PrepareSQL();
		pSQL.append("insert into tab_time_softup_task(task_id,task_name,acc_oid,add_time,service_id,up_begin,up_end,expire_time,status) values (?,?,?,?,?,?,?,?,?)");
