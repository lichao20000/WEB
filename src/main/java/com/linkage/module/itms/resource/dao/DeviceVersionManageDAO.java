 ...
 * @since 2012-12-14
 */
public class DeviceVersionManageDAO extends SuperDAO{

	// 日志记录
	private static Logger logger = LoggerFactory.getLogger(DeviceVersionManageDAO.class);
	 ...
	 * @return
	 */
	public List<Map> queryDeviceVersion(String vendorId, String deviceModelId, String hardVersion, String softVersion, String specVersion,
			String startTime, String endTime, int curPage_splitPage, int num_splitPage)
	{
		logger.debug("queryDeviceVersion({})", new Object[]{vendorId, deviceModelId, hardVersion, softVersion, specVersion, startTime, endTime, curPage_splitPage, num_splitPage});
		
		StringBuffer sb = new StringBuffer();
		// teledb
		if (DBUtil.GetDB() == 3) {
			sb.append("select id, vendor_id, device_model_id, specversion, hardwareversion, softwareversion, file_path, add_time from tab_version_info where 1=1 ");
		}
		else {
		...
		if(!StringUtil.IsEmpty(specVersion))
		{
			sb.append(" and specversion = '").append(specVersion).append("'");
		}
				...
				+ 1, num_splitPage, new RowMapper()
		{
			...
			public Object mapRow(ResultSet rs, int arg1) throws SQLException
			{
				...
				map.put("specversion", StringUtil.getStringValue(rs.getString("specversion")).trim());
	 ...
	 * @return
	 */
	public int countDeviceVersion(String vendorId, String deviceModelId, String hardVersion, String softVersion, String specVersion,
			String startTime, String endTime ,int curPage_splitPage, int num_splitPage)
	{
		logger.debug("countDeviceVersion({})", new Object[]{vendorId, deviceModelId, hardVersion, softVersion, specVersion,
				startTime, endTime, curPage_splitPage, num_splitPage});
		
		StringBuffer sb = new StringBuffer();
		// teledb
		if (DBUtil.GetDB() == 3) {
			sb.append("select count(*) from tab_version_info where 1=1 ");
		}
		else {
	...
	public Map queryForModify(String id) {
		...
		String sql = "select * from tab_version_info where id=?";
		// teledb
		if (DBUtil.GetDB() == 3) {
			sql = "select vendor_id, device_model_id from tab_version_info where id=?";
		}
	...
	public int deleteDeviceVersion(String id) {
		...
		PrepareSQL psql = new PrepareSQL("delete from tab_version_info where 1=1 and id=?");
		psql.setLong(1, Long.parseLong(id));
