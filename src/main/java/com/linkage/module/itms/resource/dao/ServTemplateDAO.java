...
public class ServTemplateDAO extends SuperDAO
{
	...
	// 日志记录
	private static Logger logger = LoggerFactory.getLogger(ServTemplateDAO.class);

	public List<Map> queryTemplateList(String name, String vlan, String serv, int curPage_splitPage, int num_splitPage, String cm)
	{
		StringBuffer sb = new StringBuffer();
		String sql = null;
		sql = "select * from tab_serv_template where gw_type='1' ";
		// teledb
		if (DBUtil.GetDB() == 3) {
			sql = "select id, serv, name, describe, vlanid, vendor_id, device_model_id, devicetype_id, update_time, service_id from tab_serv_template where gw_type='1' ";
		}
		sb.append(sql);
		...
		if ("1".equals(cm)){
			sb.append(" and service_id is not null");
		}
	...
	public List<HashMap<String, String>> queryTemplateList(String name, String vlan, String serv, String cm)
	{
		StringBuffer sb = new StringBuffer();
		String sql = null;
		sql = "select * from tab_serv_template where gw_type='1' ";
		// teledb
		if (DBUtil.GetDB() == 3) {
			sql = "select id, name from tab_serv_template where gw_type='1' ";
		}
		sb.append(sql);
		if (!StringUtil.IsEmpty(vlan)){
			sb.append(" and vlanid='" + vlan + "'");
	...
	{
		String sql = "select * from tab_serv_template where id=?";
		// teledb
		if (DBUtil.GetDB() == 3) {
			sql = "select name, describe, serv, vlanid, update_time, service_id, nserv_svlan_del, sserv_del, sserv_svlan_del, sport_del from tab_serv_template where id=?";
		}
	...
	{
		PrepareSQL pSQL = new PrepareSQL("select * from tab_serv_template_param where template_id=? order by priority");
		pSQL.setInt(1, id);
	...
	public int queryTemplateListCount(String name, String vlan, String serv, int curPage_splitPage, int num_splitPage, String cm)
	{
		StringBuffer sb = new StringBuffer();
		String sql = "select count(*) from tab_serv_template where gw_type='1' ";
		sb.append(sql);
		if (!StringUtil.IsEmpty(vlan)){
			sb.append(" and vlanid='" + vlan + "'");
		}
	...
	public int saveTemplate(List<TreeNode> templateParams, int id, String serv, String vlan, String describe, String name, String nserv_svlan_del, String sserv_del, String sserv_svlan_del, String sport_del, String service_id){
		...
		StringBuffer sf = new StringBuffer("update tab_serv_template set UPDATE_TIME=?,name=?,serv=?,vlanid=?,describe=?");
		if(!StringUtil.IsEmpty(service_id)){
			sf.append(", nserv_svlan_del=?, sserv_del=?, sserv_svlan_del=?, sport_del=?, service_id=?");
		}
		sf.append(" where id=?");
		...
		int index = 5;
		if(!StringUtil.IsEmpty(service_id)){
			sql1.setString(++index, nserv_svlan_del);
			sql1.setString(++index, sserv_del);
			sql1.setString(++index, sserv_svlan_del);
			sql1.setString(++index, sport_del);
			sql1.setString(++index, service_id);
		}
		
		sql1.setInt(++index, id);
	...
	public int addTemplate(List<TreeNode> templateParams, int id, String serv, String vlan, String describe, String name, String nserv_svlan_del, String sserv_del, String sserv_svlan_del, String sport_del, String service_id){
		String[] sqls = new String[2+templateParams.size()];
		//如果service_id不为空，则是零配置模板，不需要插入service_id
		if(!StringUtil.IsEmpty(service_id)){
