...
public class BssSheetByHand4AHDAO extends SuperDAO{

	private static Logger logger = LoggerFactory.getLogger(BssSheetByHand4AHDAO.class);
	...
	public List<HashMap<String, String>> getVoipinfo(String userId){
		...
		List<HashMap<String, String>> resList = DBOperation.getRecords(psql.getSQL());
		if(null != resList){
			for(int i=0;i<resList.size();i++){
				String voipPasswd = StringUtil.getStringValue(resList.get(i),"voip_passwd","");
				if(!StringUtil.IsEmpty(voipPasswd)){
					resList.get(i).put("voip_passwd", EncryptionUtil.decryption(voipPasswd));
				}
			}
		}
	...
	public List<HashMap<String, String>> getServInfo(String loid, String userType)
	{
		...
		PrepareSQL psql = new PrepareSQL();
		psql.append("select b.serv_type_id, b.username, b.passwd, b.vlanid, b.wan_type, b.ipaddress, b.ipmask, b.gateway, b.adsl_ser, b.stand_dns, b.user_id from ");
		psql.append(tabUser);
		psql.append(",");
		psql.append(tabServ);
	...
	public HashMap<String, String> getVoipParaInfo(String userId, String userType,String servType)
	{
		...
		psql.append("select a.protocol, a.voip_phone, a.reg_id, a.reg_id_type, a.line_id, a.voip_username, a.voip_passwd, a.uri, a.user_agent_domain, " +
				"c.prox_serv, c.prox_port, c.stand_prox_serv, c.stand_prox_port, c.regi_serv, c.regi_port, c.stand_regi_serv, c.stand_regi_port, " +
				"c.out_bound_proxy, c.out_bound_port, c.stand_out_bound_proxy, c.stand_out_bound_port from ");
	...
	public List<Map<String,String>> check(String gw_type)
	{
		...
		PrepareSQL psql = new PrepareSQL();// TODO wait (more table related)
		psql.append("select distinct a.line_id from ");
		psql.append(tabParam);
		psql.append(",");
		psql.append(tabServ);
		psql.append(",");
		psql.append(tabSip);
		psql.append(" where 1=1 and a.user_id=b.user_id and a.sip_id = c.sip_id and a.line_id!=-1");
		psql.append(" order by a.line_id asc");
		// 查询
		List<Map<String, String>> map = jt.queryForList(psql.getSQL());
		if (map.isEmpty())
		{
			return null;
		}
	...
	public List<Map<String, String>> getVoipPara(String userId, String userType,String servType)
	{
		logger.debug("isLoidExists({})", new Object[] { userId });
		String tabServ = "hgwcust_serv_info b";
		String tabParam = "tab_voip_serv_param a";
		...
		PrepareSQL psql = new PrepareSQL();
		psql.append("select a.line_id from ");// TODO wait (more table related)
		psql.append(tabParam);
		psql.append(",");
		psql.append(tabServ);
		psql.append(",");
	...
	public List<HashMap<String, String>> getServInfoByVoipPort(String userId, String userType, String lineId,String servType)
	{
		logger.debug("getServInfoByVoipPort({})", new Object[] { userId+ "-"+userType+"-"+lineId});
		String tabServ = "hgwcust_serv_info b";
		String tabParam = "tab_voip_serv_param a";
		String tabSip = "tab_sip_info c ";
		if ("2".equals(userType))
		...
		PrepareSQL psql = new PrepareSQL();// TODO wait (more table related)
		psql.append("select a.voip_username, a.voip_passwd, a.uri, a.user_agent_domain, a.protocol, a.voip_phone, " +
				"c.prox_serv, c.prox_port, c.stand_prox_serv, c.stand_prox_port, c.regi_serv, c.regi_port, " +
				"c.stand_regi_serv, c.stand_regi_port, c.out_bound_proxy, c.out_bound_port, c.stand_out_bound_proxy, c.stand_out_bound_port, " +
				"b.vlanid, b.wan_type, b.ipaddress, b.ipmask, b.gateway, b.adsl_ser from ");
		psql.append(tabParam);
		psql.append(",");
		psql.append(tabServ);
		psql.append(",");
		...
		psql.append(" where 1=1 and a.user_id=b.user_id and a.sip_id = c.sip_id ");
		psql.append(" and b.user_id ="+userId);
		psql.append(" and a.line_id ="+lineId);
		psql.append(" and b.serv_type_id ="+servType);
