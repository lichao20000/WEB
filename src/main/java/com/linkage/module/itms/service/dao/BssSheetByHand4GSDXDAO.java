...
public class BssSheetByHand4GSDXDAO extends SuperDAO{

	private static Logger logger = LoggerFactory.getLogger(BssSheetByHand4GSDXDAO.class);
	...
	public Map<String, String> getUserInfo(String loid, String userType)
	{
		...
		PrepareSQL psql = new PrepareSQL();
		psql.append("select city_id, office_id, zone_id, access_style_id, linkman, linkphone, email, mobile, linkaddress, credno, spec_id ");
	...
	{
		String tabServ = "tab_hgwcustomer a";
		String tabParam = "tab_net_serv_param b";
		if ("2".equals(obj.getUserType()))
		{
			tabServ = "tab_egwcustomer a";
			tabParam = "tab_egw_net_serv_param b";
		}
		PrepareSQL psql = new PrepareSQL();
		psql.append("select b.max_net_num, b.ssid from ");
		psql.append(tabParam);
		psql.append(",");
		psql.append(tabServ);
		psql.append(" where 1=1 and a.user_id=b.user_id and b.serv_type_id="+obj.getNetServTypeId()+" ");
		psql.append(" and a.username='"+obj.getLoid()+"'");
		psql.append(" and b.username='"+obj.getNetUsername()+"'");
		...
		if (list!=null && list.size()>0) {
			 netNum=list.get(0).get("max_net_num")==null?"":list.get(0).get("max_net_num").toString();
			 ssid=list.get(0).get("ssid")==null?"":list.get(0).get("ssid").toString();

		}
		return netNum+"|"+ssid;
	}

	public Map getCloudNetInfoMap(String user_id) {
		String tabServ = "tab_hgwcustomer a";
		String tabParam = "tab_broad_band_param b";
		PrepareSQL psql = new PrepareSQL();
		psql.append("select b.ipoe_upbandwidth,b.ipoe_downbandwidth,b.ipoe_dscp,b.app_type from ");
		psql.append(tabParam);
		psql.append(",");
		psql.append(tabServ);
		psql.append(" where 1=1 and a.user_id=b.user_id  ");
