...
public class BssSheetByHand4SXLTDAO extends SuperDAO
{
  private static Logger logger = LoggerFactory.getLogger(BssSheetByHand4HBLTDAO.class);
 ...
 public List getUserIdFromTabHgwcustomer(String loid, String userType)
  ...
  public Map<String, String> getUserInfo(String loid, String userType)
  {
    ...
    psql.append("select a.*,u.type_id from " +
            "(select user_id,city_id, office_id, zone_id, access_style_id, linkman, linkphone, email, mobile, linkaddress, credno  ");
  ...
  public Map<String, String> getStbUserInfo(String loid)
  {
    ...
    PrepareSQL psql = new PrepareSQL();
    psql.append("select city_id, serv_account, serv_pwd from stb_tab_customer where (cust_stat='1' or cust_stat='2') and loid=? ");
  ...
  public List<HashMap<String, String>> getServInfo(String loid, String userType)
  {
    ...
    PrepareSQL psql = new PrepareSQL();
    psql.append("select l.* from " +
            "(select b.serv_type_id, b.oltfactory, b.username, b.passwd, b.vlanid, b.wan_type, b.bind_port, b.ipaddress, " +
            "b.ipmask, b.gateway, b.adsl_ser, b.ip_type, b.user_id, b.serv_num, b.multicast_vlanid, b.all_route from ");
    psql.append(tabUser);
    psql.append(",");
    psql.append(tabServ);
  ...
  public List<HashMap> getVoipParaInfo(String userId, String userType, String servType,String protocol)
  {
    ...
    psql.append("select a.protocol, a.voip_phone, a.reg_id, a.reg_id_type,  a.voip_port, a.voip_username, a.voip_passwd, a.uri, " +
            "a.user_agent_domain, c.prox_serv, c.prox_port, c.stand_prox_serv, c.stand_prox_port, c.regi_serv, c.regi_port, " +
            "c.stand_regi_serv, c.stand_regi_port, c.out_bound_proxy, c.out_bound_port, c.stand_out_bound_proxy, " +
    ...
    psql.append(" and a.protocol =" + protocol);
  ...
  public List<HashMap<String, String>> getServInfoByUserName(String userId, String userType, String netUsername, String servType)
  {
    logger.debug("getServInfoByUserName({})", new Object[] { userId + "-" + userType + "-" + netUsername });
    String tabServ = "hgwcust_serv_info b";
    if ("2".equals(userType))
    {
      tabServ = "egwcust_serv_info b";
    ...
    PrepareSQL psql = new PrepareSQL();
    psql.append("select b.* from ");
    psql.append(tabServ);
    psql.append(" where 1=1 and b.user_id =" + userId);
    psql.append(" and b.username ='" + netUsername + "' ");
    psql.append(" and b.serv_type_id =" + servType);
    List map = DBOperation.getRecords(psql.getSQL());
    return map;
  }*/

  public void addHandSheetLog(SXLTSheetObj userInfoSheet, UserRes curUser, int servType, int operType, int resultId, String resultDesc)
  {
