 ...
 */
public class ConfigFluxDao_longWarn extends ConfigFluxDao
{
	private static Logger log = LoggerFactory.getLogger(ConfigFluxDao_longWarn.class);
	/*
	 * (non-Javadoc)
	...
	public boolean editFluxPort(String device_id, String[] port_info,int intodb,int gatherflag,
		...
		for(int i=0;i<n;i++){
			...
			+",ifinoct_maxtype="+fmi.getIfinoct_maxtype()
			+",ifoutoct_maxtype="+fmi.getIfoutoct_maxtype()
			+",ifinoct_mintype="+fmi.getIfinoct_mintype()
			+",ifinoctetsbps_min="+fmi.getIfinoctetsbps_min()
			+",ifoutoct_mintype="+fmi.getIfoutoct_mintype()
			+",ifoutoctetsbps_min="+fmi.getIfoutoctetsbps_min()
			+",warningnum_min="+fmi.getWarningnum_min()
			+",warninglevel_min="+fmi.getWarninglevel_min()
			+",reinlevel_min="+fmi.getReinlevel_min()
			+",overmax="+fmi.getOvermax()
			+",overmin="+fmi.getOvermin()
			+",overper_min="+fmi.getOverper_min()
			+",overnum_min="+fmi.getOvernum_min()
			+",overlevel_min="+fmi.getOverlevel_min()
			+",reinoverlevel_min="+fmi.getReinoverlevel_min()
			+",confignum="+fmi.getConfignum()
	...
	public boolean configDevicePortFlux(Map<String,String> param,FluxPortInfo fpi,Flux_Map_Instance alarmParam)
	{
		...
		sb.append("ifinoct_maxtype,ifinoctetsbps_max,ifoutoct_maxtype,ifoutoctetsbps_max,ifindiscardspps_max,ifoutdiscardspps_max,");
		sb.append("ifinerrorspps_max,ifouterrorspps_max,warningnum,warninglevel,reinstatelevel,");
		sb.append("ifinoct_mintype,ifinoctetsbps_min,ifoutoct_mintype,ifoutoctetsbps_min,warningnum_min,warninglevel_min,reinlevel_min,");
		sb.append("overmax,overper,overnum,overlevel,reinoverlevel,com_day,overmin,overper_min,overnum_min,overlevel_min,reinoverlevel_min,");
		sb.append("ifinoctets,inwarninglevel,inreinstatelevel,inoperation,intbflag,ifoutoctets,outwarninglevel,outreinstatelevel,");
		sb.append("outoperation,outtbflag,confignum) values('");
		sb.append(device_id).append("','");
		sb.append(fpi.getIfindex()).append("','");
		sb.append(fpi.getIfdescr().replaceAll("'", "\\$\\$\\$")).append("','");
		sb.append(fpi.getIfname().replaceAll("'", "\\$\\$\\$")).append("','");
		...
		sb.append(alarmParam.getIfinoct_maxtype()).append(",");
		sb.append(alarmParam.getIfinoctetsbps_max()).append(",");
		sb.append(alarmParam.getIfoutoct_maxtype()).append(",");
		...
		sb.append(alarmParam.getIfinoct_mintype()).append(",");
		sb.append(alarmParam.getIfinoctetsbps_min()).append(",");
		sb.append(alarmParam.getIfoutoct_mintype()).append(",");
		sb.append(alarmParam.getIfoutoctetsbps_min()).append(",");
		sb.append(alarmParam.getWarningnum_min()).append(",");
		sb.append(alarmParam.getWarninglevel_min()).append(",");
		sb.append(alarmParam.getReinlevel_min()).append(",");
		sb.append(alarmParam.getOvermax()).append(",");
		...
		sb.append(alarmParam.getOvermin()).append(",");
		sb.append(alarmParam.getOverper_min()).append(",");
		sb.append(alarmParam.getOvernum_min()).append(",");
		sb.append(alarmParam.getOverlevel_min()).append(",");
		sb.append(alarmParam.getReinoverlevel_min()).append(",");
		...
		sb.append(ReadFluxConfigPortInfo.getConfignumByAlarm(alarmParam)).append(")");
	...
	public Map<String, Flux_Map_Instance> loadDeviceAlarm(String device_id)
	{
		...
		String sql = "select ifinoctetsbps_max, ifoutoctetsbps_max, ifindiscardspps_max, ifoutdiscardspps_max, ifinerrorspps_max, " +
				"ifouterrorspps_max, warningnum, warninglevel, reinstatelevel, overper, overnum, overlevel, reinoverlevel, com_day, " +
				"ifinoctets, inwarninglevel, inreinstatelevel, inoperation, intbflag, ifoutoctets, outwarninglevel, outreinstatelevel, " +
				"outoperation, outtbflag, ifinoct_maxtype, ifoutoct_maxtype, ifinoct_mintype, ifoutoct_mintype, ifinoctetsbps_min, " +
				"ifoutoctetsbps_min, warningnum_min, warninglevel_min, reinlevel_min, overmax, overmin, overper_min, overnum_min, " +
				"overlevel_min, reinoverlevel_min, device_id, getway, port_info " +
		...
		for (Map record : list)
		{
			...
			alarmInstance.setIfinoctetsbps_max(Float.parseFloat(String.valueOf(record
					.get("ifinoctetsbps_max"))));
			alarmInstance.setIfoutoctetsbps_max(Float.parseFloat(String.valueOf(record
					.get("ifoutoctetsbps_max"))));
			alarmInstance.setIfindiscardspps_max(Float.parseFloat(String.valueOf(record
					.get("ifindiscardspps_max"))));
			alarmInstance.setIfoutdiscardspps_max(Float.parseFloat(String.valueOf(record
					.get("ifoutdiscardspps_max"))));
			alarmInstance.setIfinerrorspps_max(Float.parseFloat(String.valueOf(record
					.get("ifinerrorspps_max"))));
			alarmInstance.setIfouterrorspps_max(Float.parseFloat(String.valueOf(record
					.get("ifouterrorspps_max"))));
			alarmInstance.setWarningnum(Integer.parseInt(String.valueOf(record
					.get("warningnum"))));
			alarmInstance.setWarninglevel(Integer.parseInt(String.valueOf(record
					.get("warninglevel"))));
			alarmInstance.setReinstatelevel(Integer.parseInt(String.valueOf(record
					.get("reinstatelevel"))));
			alarmInstance.setOverper(Float.parseFloat(String.valueOf(record
					.get("overper"))));
			alarmInstance.setOvernum(Integer.parseInt(String.valueOf(record
					.get("overnum"))));
			alarmInstance.setOverlevel(Integer.parseInt(String.valueOf(record
					.get("overlevel"))));
			alarmInstance.setReinoverlevel(Integer.parseInt(String.valueOf(record
					.get("reinoverlevel"))));
			alarmInstance.setCom_day(Integer.parseInt(String.valueOf(record
					.get("com_day"))));
			alarmInstance.setIfinoctets(Float.parseFloat(String.valueOf(record
					.get("ifinoctets"))));
			alarmInstance.setInwarninglevel(Integer.parseInt(String.valueOf(record
					.get("inwarninglevel"))));
			alarmInstance.setInreinstatelevel(Integer.parseInt(String.valueOf(record
					.get("inreinstatelevel"))));
			alarmInstance.setInoperation(Integer.parseInt(String.valueOf(record
					.get("inoperation"))));
			alarmInstance.setIntbflag(Integer.parseInt(String.valueOf(record
					.get("intbflag"))));
			alarmInstance.setIfoutoctets(Float.parseFloat(String.valueOf(record
					.get("ifoutoctets"))));
			alarmInstance.setOutwarninglevel(Integer.parseInt(String.valueOf(record
					.get("outwarninglevel"))));
			alarmInstance.setOutreinstatelevel(Integer.parseInt(String.valueOf(record
					.get("outreinstatelevel"))));
			alarmInstance.setOutoperation(Integer.parseInt(String.valueOf(record
					.get("outoperation"))));
			alarmInstance.setOuttbflag(Integer.parseInt(String.valueOf(record
					.get("outtbflag"))));
			// 在以前版本上新增的字段
			alarmInstance.setIfinoct_maxtype(Integer.parseInt(String.valueOf(record
					.get("ifinoct_maxtype"))));
			alarmInstance.setIfoutoct_maxtype(Integer.parseInt(String.valueOf(record
					.get("ifoutoct_maxtype"))));
			alarmInstance.setIfinoct_mintype(Integer.parseInt(String.valueOf(record
					.get("ifinoct_mintype"))));
			alarmInstance.setIfoutoct_mintype(Integer.parseInt(String.valueOf(record
					.get("ifoutoct_mintype"))));
			alarmInstance.setIfinoctetsbps_min(Float.parseFloat(String.valueOf(record
					.get("ifinoctetsbps_min"))));
			alarmInstance.setIfoutoctetsbps_min(Float.parseFloat(String.valueOf(record
					.get("ifoutoctetsbps_min"))));
			alarmInstance.setWarningnum_min(Integer.parseInt(String.valueOf(record
					.get("warningnum_min"))));
			alarmInstance.setWarninglevel_min(Integer.parseInt(String.valueOf(record
					.get("warninglevel_min"))));
			alarmInstance.setReinlevel_min(Integer.parseInt(String.valueOf(record
					.get("reinlevel_min"))));
			alarmInstance.setOvermax(Integer.parseInt(String.valueOf(record
					.get("overmax"))));
			alarmInstance.setOvermin(Integer.parseInt(String.valueOf(record
					.get("overmin"))));
			alarmInstance.setOverper_min(Float.parseFloat(String.valueOf(record
					.get("overper_min"))));
			alarmInstance.setOvernum_min(Integer.parseInt(String.valueOf(record
					.get("overnum_min"))));
			alarmInstance.setOverlevel_min(Integer.parseInt(String.valueOf(record
					.get("overlevel_min"))));
			alarmInstance.setReinoverlevel_min(Integer.parseInt(String.valueOf(record
					.get("reinoverlevel_min"))));
	...
	{
		String sql = "select a.port_info,a.ifindex,a.ifdescr,a.ifname,a.ifnamedefined,a.ifportip,a.getway,a.confignum,"
				+ "b.gatherflag,b.ifconfigflag,b.polltime,b.intodb,b.snmpversion from flux_interfacedeviceport a, flux_deviceportconfig b"
		...
		return jt.query(psql.getSQL(), new RowMapper()
		{
			...
			{
				Map map = new HashMap();
				map.put("port_info", changeString(rs.getString("port_info")).replaceAll(
						"\"", "\\$#!").replaceAll("'", "#\\$!"));// 端口信息
