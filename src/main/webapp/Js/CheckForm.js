...
function IsIPAddr(strValue){
	...
	if(IsNull(strValue,msg)){
		...
		/*
		 * do{ if(i>=4) break; if(i != 3 && pos == -1){ alert(msg + "格式不符合");
		 * return false; } i=i+1; if(pos == -1) pos = v.length; tmpStr =
		 * v.substring(0,pos); if(!chkIPArea(tmpStr)) return false; v =
		 * v.substring(pos+1,v.length); pos = v.indexOf("."); }while(pos != -1 ||
		 * i<=3) //if(i==3) return true; //alert(msg + "格式不符合"); //return
		 * false;
		 */
	}
}
...
------------------------------------------------------*/
function IsPassword(strValue,strMsg,loginName,InstArea){
	var bz = true;
	var j=0;
	var k=0;
	var a=0,b=0,c=0,d=0;
	...
	if(true){
		if("cq_dx" == InstArea && strValue.length < 8){
			alert(strMsg + '长度需要至少8位')
			return false;
		}
		if(("nmg_dx" == InstArea || "jl_lt" == InstArea || "hn_lt" == InstArea) && strValue.length < 8){
			alert(strMsg + '长度需要8位及以上')
			return false;
		}
		
		if("hn_lt" != InstArea && "nmg_dx" != InstArea && "hlj_dx" != InstArea && "nx_dx" != InstArea && 
				"nx_lt" != InstArea && "jl_lt" != InstArea && "cq_dx" != InstArea && strValue.length <= 8){
			alert(strMsg + '长度需要8位以上')
			return false;
		}
		
		if(("nx_dx" == InstArea || "nx_lt" == InstArea ) && strValue.length < 8){ 
			alert(strMsg + '长度需要8位及以上')
			return false;
		}
		...
		if("hlj_dx" == InstArea){
			...
			if (strValue.toLowerCase().indexOf(loginName.toLowerCase()) > -1
					|| strValue.toLowerCase().indexOf("ITMS".toLowerCase()) > -1) {
				alert(strMsg + '不能包含敏感信息')
				return false;
			}
		}
		if(strValue.indexOf(loginName)!=-1){
			alert("密码内容不能包含用户名");
			return false;
		}
		...
		if(count < valiSize){
			if("jl_lt" == InstArea) {
				alert(strMsg + "组合必须包含数字、大写字母、小写字母、特殊字符");
			}else{
				alert(strMsg + "组合必须包含数字、大写字母、小写字母、特殊字符中的三种");
			}
			return false;
		}
      ...
      /*
		 * if (!((ch<'0'||ch>'9')&&(ch<'a'||ch>'z')&&(ch<'A'||ch>'Z'))){ if(k<2){
		 * alert(strMsg+'中数字不能少于2位') bz = false; }else{ if(j<3){
		 * alert(strMsg+'中字母不能少于3位') bz = false; } } }
		 */       	    
    }      
...
function IsTime(strValue){
    ...
    /*
	 * if(isNaN(Date.parse(v))){ alert("日期格式不符合，正确格式是：月-日-年"); return false; }
	 */
...
// 校验时间是否在一个月内
function validateOneMonth(start,end){
	var sValue=start.substring(5)+"-"+start.substring(0,4);	
	var s_date=new Date(Date.parse(sValue));
	var s_day=s_date.getDate();
	var s_month=s_date.getMonth()+1;
	var s_year=s_date.getYear();
	
	var eValue=end.substring(5)+"-"+end.substring(0,4);	
