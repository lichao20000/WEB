 ...
 * @Edit by benyp 2007-11-05:增加流量字段的排序，修改IP方式的排序方式
 * @Edit by benyp 2008-03-10:增加告警等级排序:告警等级:0:自动清除 1:正常日志 2:提示告警3:一般告警 4:严重告警 5:紧急告警
 */
...
(function($) {
	...
	//edit by benyp:G:*1000*1000*1000,M:*1000*1000,K:*1000B	
//流量排序

	ts.addParser({
		id: "flux",
		is: function(s) {
			return /^(\+|-)?[0-9]+(\.[0-9]*)?[GgMmKkBb]{1}$/.test(s);
		},
		format: function(s) {
			var sum="";
			s=s.replace(/,/g,"");
			var m=s.charAt(s.length-1);
			if(m=="G" || m == "g"){
			    sum=s.substring(0,s.length-1)*1000*1000*1000;
			}else if(m == "M" || m == "m"){
				sum=s.substring(0,s.length-1)*1000*1000;
			}else if(m == "K" || m=="k"){
				sum=s.substring(0,s.length-1)*1000;
			}else{
				sum=s;
			}
			
			return $.tablesorter.formatFloat(sum);
		},
		type: "numeric"
	});

//edit by benyp:
//告警等级:0:自动清除 1:正常日志 2:提示告警3:一般告警 4:严重告警 5:紧急告警

	ts.addParser({
		id: "warn",
		is: function(s) {
			var flg=/^\u81ea\u52a8\u6e05\u9664$/.test(s) || /^\u6b63\u5e38\u65e5\u5fd7$/.test(s) || /^\u63d0\u793a\u544a\u8b66$/.test(s) || /^\u4e00\u822c\u544a\u8b66$/.test(s) || /^\u4e25\u91cd\u544a\u8b66$/.test(s) || /^\u7d27\u6025\u544a\u8b66$/.test(s);
			return flg;
		},
		format: function(s) {
			var sum="";
