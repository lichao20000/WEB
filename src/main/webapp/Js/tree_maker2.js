/************************************************************************************
*Date	 :	 2020/04/08 山西联通 如果text后面有"."则显示为文件夹 
************************************************************************************/
var Tree_CHILD=-1;
var Tree_LAST=-2;
var Tree_ROOT=-3;
...
function Tree_treeView()
{
	...
	this.value=Tree_treeView_index;
	this.flag=false;
		...
		if(node){
			...
			if(flag)Tree_selectNode(nodeID);
				...
				if(this.showSelect && td)
					td.className=(Tree_node_array[nodeID].text && (Tree_node_array[nodeID].text.indexOf('.') !== -1))?
	...
	this.getSelect=function getSelect(){//����ѡ����
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		else
		{
			...
			if(action.link!="") try{eval(command);}catch(e){alert("�ű����д���\r\n\r\n"+e.description);}
		}
	}
	else if(action.type==Tree_SCRIPT)//run script
	{
		try{eval(action.script);}catch(e){alert("�ű����д���\r\n\r\n"+e.description);}
	}
}
//----------------
function Tree_selectNode(nodeID)
{
	...
	if(tree.selectID!=null)
	{
		...
		if(tree.showSelect && tdOld)
			tdOld.className=(Tree_node_array[tree.selectID].text && (Tree_node_array[tree.selectID].text.indexOf('.') !== -1))?
				tree.folderClass1:tree.fileClass1;//reset to normal class
	}
	tree.selectID=nodeID;
	if(tree.showSelect && td)
		td.className=(Tree_node_array[tree.selectID].text && (Tree_node_array[tree.selectID].text.indexOf('.') !== -1))?
...
function Tree_clickNode(nodeID)
{
	...
	var td=document.getElementById("Tree_td_"+nodeID);
	//------
	if(!node.expanded && node.text && (node.text.indexOf('.') !== -1) && tree.flag==false)
		if(tree.callback_expanding(nodeID)==false)//cancel expand
			return;
	if(node.expanded && node.text && (node.text.indexOf('.') !== -1) && tree.flag==false)
		if(tree.callback_collapsing(nodeID)==false)//cancel collapse
			return;
	node.expanded=!node.expanded && node.text && (node.text.indexOf('.') !== -1)
...
function Tree_imgSrc(node)
{
	...
	return (node.text && (node.text.indexOf('.') !== -1))?(node.expanded?src2:src1):src3;
}
//-------------------------------------------
...
function Tree_td(node)
{
	...
	if(node.text && (node.text.indexOf('.') !== -1))
...
function Tree_mouseout(obj,nodeID)
{
	...
	if(tree.showSelect && nodeID==tree.selectID )
		obj.className = (node.text && (node.text.indexOf('.') !== -1))?tree.folderClass3:tree.fileClass3;
	else if(node.text && (node.text.indexOf('.') !== -1))
