...
String tem_wan_type = request.getParameter("wan_type");
int wan_type = -1;
if (null != tem_wan_type ) {
	try {
		wan_type = Integer.parseInt(tem_wan_type);
		...
		<table border=0 cellspacing=1 cellpadding=2 width="100%" id="idTable"
            ...
            if (fields != null) {
                ...
                while (fields != null) {
                    ...
                    out.println("<td><nobr>" + CityDAO.getCityIdCityNameMap().get(tmp)
                            + "</nobr></td>");
                    out.println("<td><nobr>" + Global.Serv_type_Map.get(Global.Service_Id_Info_Map.get(service_id)[0])
                            + "</nobr></td>");
					out.println("<td><nobr>" + Global.Oper_type_Id_Name_Map.get(Global.Service_Id_Info_Map.get(service_id)[1])
                            + "</nobr></td>");
                    ...
                    if(tmp.equals("1")){
                    	tmp = "执行成功";
                    	if ("1031".equals(service_id)) {
                    		//如果是桥接改路由业务，则执行成功后，再更新用户信息 gsj
                    		sheetManage.updateHgwcustomer(Global.Service_Id_Info_Map.get(service_id)[0],username, passwd,wan_type,vpiid,vciid);
                    	}
