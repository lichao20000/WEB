...
if(null==ip_type || "null".equals(ip_type)){
	ip_type = "";
}
...
if(null==is_speedTest || "null".equals(is_speedTest)){
	is_speedTest = "";
}
String is_ableUser = request.getParameter("is_ableUser");
if(null==is_ableUser || "null".equals(is_ableUser)){
	is_ableUser = "";
}
String is_esurfing=request.getParameter("is_esurfing");
if(null==is_esurfing || "null".equals(is_esurfing)){
	is_esurfing = "";
}

String prt_Page="DeviceListByEditionNXDXE8C_prt.jsp?city_id="+cityId+"&rela_dev_type_id="+ rela_dev_type_id+"&devicetype_id="+devicetypeId+"&gw_type="+gw_type+"&isBind="+isBind+"&vendor_id="+vendorId+"&startTime="+startTime+"&endTime="+endTime+"&is_normal="+is_normal+"&gw_type="+gw_type+"&deviceModelId="+deviceModelId+"&devicetype="+devicetype+"&citynext="+citynext+"&recentStartTime="+recentStartTime+"&recentEndTime="+recentEndTime+"&cityIdSelect="+cityIdSelect+"&is_esurfing="+is_esurfing+"&ip_type="+ip_type+"&gbbroadband="+gbbroadband+"&is_speedTest="+is_speedTest+"&is_ableUser="+is_ableUser;
if("hb_lt".equals(LipossGlobals.getLipossProperty("InstArea.ShortName")) && "1".equals(gw_type)){
	prt_Page = "/itms/itms/resource/deviceInfoExportACT!exportByEdition.action?city_id="+cityId+"&rela_dev_type_id="+ rela_dev_type_id+"&devicetype_id="+devicetypeId+"&gw_type="+gw_type+"&isBind="+isBind+"&vendor_id="+vendorId+"&startTime="+startTime+"&endTime="+endTime+"&is_normal="+is_normal+"&gw_type="+gw_type+"&deviceModelId="+deviceModelId+"&devicetype="+devicetype+"&citynext="+citynext+"&recentStartTime="+recentStartTime+"&recentEndTime="+recentEndTime+"&cityIdSelect="+cityIdSelect+"&is_esurfing="+is_esurfing+"&ip_type="+ip_type+"&gbbroadband="+gbbroadband+"&is_speedTest="+is_speedTest+"&is_ableUser="+is_ableUser;
}
String __debug = request.getParameter("__debug");
List list  = null;
String strData = "";
list = DeviceAct.getDeviceListByVenderNXDXE8C(request);
...
String detailStrHblt = "DetailDeviceHblt('?','"+gw_type+"')";
...
if (fields == null) {
    ...
    while (fields != null) {
		...
		if(!devicetype_id.equals("-1")){
			//由于脏数据，会导致deviceTypeMap中存放的数据缺失,设备表的devicetype_id 在deviceTypeMap中不存在
			//导致tmp=null ，修改之后，将null转化为"",保证数字的一致性
			String tmp = StringUtil.getStringValue(deviceTypeMap.get(devicetype_id));
		...
		{
			strData += "<TD class=column2>" + String.valueOf(fields.get("loid")) + "</TD>";//loid
			strData += "<TD class=column2>" + String.valueOf(fields.get("serv_account")) + "</TD>";//宽带账号
			strData += "<TD class=column2>" + String.valueOf(fields.get("cpe_mac")) + "</TD>";//设备MAC
		}
		...
		if("hb_lt".equals(LipossGlobals.getLipossProperty("InstArea.ShortName"))||"jl_dx".equals(LipossGlobals.getLipossProperty("InstArea.ShortName")))
		{
			strOper = "<a href=javascript:// onclick=\""+ StringUtils.replace(detailStrHblt,"?",device_id) +"\">详细信息</a>";
		}
		else
		{
	...
	if("1".equals(LipossGlobals.getLipossProperty("isReport"))){
		if (!"1".equals(request.getParameter("quality"))) {
...
function DetailDeviceHblt(device_id,gw_type){
	var strpage;
	if("4" == gw_type){
		strpage = "/itms/gtms/stb/resource/gwDeviceQueryStb!queryZeroDeviceDetail.action?gw_type=4&deviceId=" + device_id;
	}else{
