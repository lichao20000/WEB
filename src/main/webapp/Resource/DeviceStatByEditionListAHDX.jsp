...
function detailId(city_id,devicetype_id, gw_type){
	...
	var page="DeviceListByEditionAHDX.jsp?city_id="+city_id+"&devicetype_id="+devicetype_id + "&gw_type=" + gw_type + "&startTime=<%=startTime%>&endTime=<%=endTime%>&use_state=<%=use_state%>";
...
<body>
	<TABLE border=1 cellspacing=0 cellpadding=2 width="100%" style="word-wrap:break-word;">
		<%=ServiceActAhdx.getHtmlDeviceByEditionAHDX(request)%> 
		<TR>
			<TD class=column colspan="<%=count+3%>" height="30"><a
				href="DeviceStatByEdition_prt_AHDX.jsp?isPrt=1&gw_type=<%=gw_type%>&isBind=<%=isBind %>&startTime=<%=startTime%>&endTime=<%=endTime%>&vendorId=<%=vendorId%>&modelId=<%=modelId%>&deviceTypeId=<%=deviceTypeId%>&is_normal=<%=is_normal %>&rela_dev_type_id=<%=rela_dev_type_id%>&is_esurfing=<%=is_esurfing%>&city_id=<%=city_id%>&sqlid=<%=sqlid%>&use_state=<%=use_state%>"
