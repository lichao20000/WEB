...
int size = 0;
size = ServiceSpeedAct.getSpeedList().size() + 1;
...
function detail(city_id,speed){
	window.open("./DeviceListBySpeedHB.jsp?city_id="+city_id+"&gbbroadband=<%=gbbroadband%>"+"&gw_type=1&speed="+speed,"","left=20,top=20,width=800,height=600,resizable=no,scrollbars=yes");
}
	...
	<TABLE border=1 cellspacing=0 cellpadding=2 width="100%" style="word-wrap:break-word;">
		...
		<TR>
			<TD class=column colspan="<%=size%>" height="30">
				<a href="DeviceOnServiceSpeedHB_prt.jsp?isPrt=1" alt="导出当前页数据到Excel中">&nbsp;&nbsp;&nbsp;导出到Excel</a>
			</TD>
		</TR>
	</TABLE>
</body>
