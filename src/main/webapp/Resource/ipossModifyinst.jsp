...
$(document).ready(function(){
    ...
    var dealstaffid = '<%=(String) request.getParameter("dealstaffid")%>';
       
    $("input[@name='username']").val(username);
	$("input[@name='dealstaff']").val(dealstaff);
	$("input[@name='city_id']").val(city_id);
	$("input[@name='dealstaffid']").val(dealstaffid);
	$("div[@id='username']").html(username);
	$("div[@id='dealstaff']").html(dealstaff);
	//$("div[@id='city_id']").html(city_id);
    var url = "<s:url value='/gwms/resource/itmsIpossInst!modifyInstInit.action'/>";
...
function openinst(){
	...
	var dealstaff = $("input[@name='dealstaff']").val();
	var city_id = $("input[@name='city_id']").val();
	location.href="<s:url value='/Resource/ipossUserinst.jsp'/>?city_id="+city_id+"&dealstaff="+dealstaff+"&username="+username;
}

function checkSerialno(){
	var tmp = $.trim($("input[@name='device_serialnumber']").val());
	if (tmp.length < 6){
...
function deviceOnclick(device_id,city_id,oui,device_serialnumber){
	...
	$("div[@id='div_bind']").html("<input type='button' class=jianbian name='save_btn' value=' 开 始 修 障 ' onclick='CheckForm()'/>");
...
function CheckForm(){
	...
	var faultId = document.frm.faultId.value;
	if(faultId=="-1"){
		alert("请选择修障原因！");
		return false;
	}
	...
	document.frm.action="<s:url value='/gwms/resource/itmsIpossInst!modifyInst.action'/>";
	document.frm.submit();	
}
...
<FORM NAME="frm" METHOD="post" ACTION="">
	...
	<input type="hidden" name="dealstaffid" value="" />
	<!-- 设备部分 -->
