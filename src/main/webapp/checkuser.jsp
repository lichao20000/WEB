...
<%
	...
	// 其他省份 可能会报Cannot create a session after the response has been commited
	// 这个只用于新疆
	...
	if (flg)
	{
		...
		if (flg)
		{
			...
			if ("ah_dx".equals(InstArea)) {
				//AHDX_ITMS-REQ-20190924YQW-001（web用户登录改造）超过三个月的重新修改密码
				descMsg = "您已大于三个月未登陆使用";
			...
			if (auth.login())
			{
				...
				else */if (auth.checkPasswordUsedTime())
				{
					...
					LogItem.getInstance().writeLoginLog(request,
							Encoder.toISO(desc));
					//增加session，防止跳过修改密码页面进入首页，同时在DbUserManager.java 修改密码成功后失效session
