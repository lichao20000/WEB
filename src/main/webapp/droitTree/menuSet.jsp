...
<script>
	...
	function loadTree(){
		...
		tree.loadXML("treefromdb.jsp?t="+new Date().getTime());
		status();
	}
...
<%@ include file="./left_bar.jsp"%>
    ...
    <td width="98%"  class="jive-setup-category-header">
      <p >系统目录创建</p>
	...
	<div id="actions">
		<a href="javascript:void(0)" onclick="addNewPeer()">创建相邻目录</a>|<a href="javascript:void(0)" onclick="addNewChild()">创建下级目录</a>|<a href="javascript:void(0)" onclick="deleteNode()">删除目录</a>
		&nbsp;&nbsp;（注：顶层为系统模块分布，若需调整需在<a href="./moduleInfoList.jsp">功能模块列表</a>进行调整）
	</div>
	<table width="100%">
		<tr>
			<!--- tree area --->
			<td width="350px"  class="jive-setup-category-header">
				<div id="treebox" 
					style="	width:350px; 
							height:300px;
			...
			<td id="details" 
				style="padding:5px;visibility:hidden;" valign="top"  class="jive-setup-category-header">
					<form name="detailsForm" action="" target="actionframe" method="post">
						<span style="display:none">Node ID:
						...
						<span>目录名称：</span>
						<input name="tree_name" type="Text" maxlength="50" name="label" style="width:250px;"><br><br>
						<span>说明：</span>
						<textarea name="tree_desc" id="textarea" style="height:100px;width:250px;"></textarea><br>	<br>
						<button type="button" onclick="saveItem()" style="width:200px;">保 存</button>
					</form>
			</td>
		</tr>
	</table>
