 ...
 dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode){
	...
	Nodes[Count]=new dhtmlXTreeItemObject(itemId,itemText,parentObject,this,itemActionHandler,1);
	 ...
	 if(this.XMLloadingWarning){
		// make all of the nodes display is none after create them
		// and make the visual selectively when 'draw' them out in _redrawFrom method
		Nodes[Count].htmlNode.parentNode.parentNode.className="standartTreeRow";
		 ...
		 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
		{
			 ...
			 if((!dhtmlObject.waitUpdateXML)||(dhtmlObject.waitUpdateXML.toString().search(","+cId+",")!=-1))
			{
				 ...
				 if((!dhtmlObject.slowParse)||(dhtmlObject.waitUpdateXML))
				{
			 	...
			 	dhtmlObject._parseXMLTree(dhtmlObject,node.childNodes[i],cId,1);
			 }
		}
		 else{
 ...
 dhtmlXTreeObject.prototype._redrawFrom=function(dhtmlObject,itemObject){
	 ...
	 for(var i=0;i<tempx.childsCount;i++)
	{
		 if(!itemObject){
			...
			//----------------------------------------------------------------------------------------------------------------------
			// add by YYS 2006-11-3
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[0].style.display="";
			// make the top tr of the tree in the standartTreeTopRow css
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0].childNodes[0].className="standartTreeTopRow";
			// hid all of 'td's except the 'Text td' of the top tr of the tree
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[0].style.display="none";
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[1].style.display="none";
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[2].style.display="none";
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[3].style.display="";
			// make the 'Text td' of the top tr of the tree fill the tr
			tempx.childNodes[i].htmlNode.parentNode.parentNode.childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[3].colSpan="4";
			//----------------------------------------------------------------------------------------------------------------------

			
			// make the top tr of the tree visual
...
dhtmlXTreeObject.prototype.onRowClick2=function(){
 ...
 // modify by YYS 2006-11-3
 // add _HideShow to the onRowSelect method to make it show or hid childNodes before response the mouseOnclick event
 dhtmlXTreeObject.prototype.onRowSelect=function(e,htmlObject,mode){
	 
	 if(!htmlObject){htmlObject=this.parentObject.span.parentNode};
	 if(htmlObject.parentNode.className!="standartTreeTopRow"){
	...
	{
		 this.parentObject.treeNod._HideShow(this.parentObject);//show or hid the childNodes before the actionHandler add by YYS 2006-11-3
		 htmlObject.parentObject.actionHandler(htmlObject.parentObject.id,lastId);//the default actionHandler is aFunc() define by user with setOnClickHandler() method
	}
	}
};
...
dhtmlXTreeObject.prototype._createItem=function(acheck,itemObject,mode){
	 ...
	 td1.appendChild(img0);img0.style.padding='5px';img0.style.margin=0;
	 ...
	 img.onclick=this.onRowClick
	 
	 }
	 if(!mode)img.src=this.imPath+this.imageArray[0];
	 td12.appendChild(img);img.style.padding=0;img.style.margin=0;
	/* if(this.timgen)//shield the defautt size defintion to the image and use the image original size modify by YYS 2006-11-3
	...
	{img.style.width="0px";img.style.height="0px";}*/
	 ...
	 // the mouseonclick event define of the 'Text' in the tree
