...
<%
	...
	fupload.setMaxFileSize(1024 * 1024 * 20);
	fupload.upload();
...
//			}
			...
			ArrayList<String> sqlSh = new ArrayList<String>();
			...
			strSQL = "select vendor_id + '|' + device_model_id + '|' + hardwareversion + '|' + softwareversion as dtype,"
					+ "devicetype_id from tab_devicetype_info";
...
//			}
			**/
			...
			for (int k = 0; k < ouiNum; k++) {
				...
				for (int i = 0; i < iDevModelLen; i++) {
					...
					for (int j = 0; j < iHardwareLen; j++) {
						...
						long softwarefile_id=DBOperation.getMaxId("softwarefile_id","tab_ap_software_file")+1;
						System.out.println("softwarefile_id:"+softwarefile_id);
						sqlSh.add("insert  into tab_ap_software_file values("+softwarefile_id+",'"+strRename+"',1,'"+softwareversion+"','"+hardwareversion+"','"+vendor_name+"')");
						sql="insert  into tab_ap_software_file values("+softwarefile_id+",'"+strRename+"',1,'"+softwareversion+"','"+hardwareversion+"','"+vendor_name+"')";
            ...
            for(int z=0;z<sqlfileList.size();z++){
				
				System.out.println("@@@@@@:"+sqlfileList.get(z).toString());
			}
			
			if (sqlList != null && sqlList.size() > 0) {
				icode = DBOperation.executeUpdate(sqlList);
				System.out.println("版本 :"+icode);
				if (icode > 0) {
					if(null != sqlfileList && sqlfileList.size() > 0){
						icode = DBOperation.executeUpdate(sqlfileList);
						System.out.println("file :"+icode);
						if (icode > 0) {
							msg += " 文件入库成功！";
						} else {
							msg += " 文件入库失败！";
