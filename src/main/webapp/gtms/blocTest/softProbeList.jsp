...
function ToExcel() {
	var page="../../gtms/resource/queryDevice!getInfoExcelDevice2.action?"
...
function refreshDev(device_id,deviceSN,oui){
	var url = '<s:url value='/gtms/resource/refDelDev!refresh.action'/>';
	$.post(url,{
		deviceId:device_id,
		deviceSN:deviceSN,
		oui:oui
...
function delDev(device_id,cpe_allocatedstatus,oui,deviceSN){
	...
	var url = "<s:url value='/gtms/resource/refDelDev!delete.action'/>";
...
<title>软探针设备操作</title>
<form name="frm"
	action="<s:url value='/gtms/blocTest/softProbe!deviceOperate.action"'/>"
	method="POST">
...
<table width="98%" border=0 cellspacing=0 cellpadding=0 align="center">
	...
	<TR>
		...
		<TABLE width="100%" height="30" border="0" cellspacing="0"
			...
			<TR>
				<TD width="164" align="center" class="title_bigwhite">软探针设备资源</TD>
					...
					<s:iterator value="data">
						<tr bgcolor="#ffffff">
							<s:if test="infoType == 1">
								...
								<IMG SRC="<s:url value="/images/install.png"/>" 
								BORDER='0' ALT='安装'onclick="alert('安装成功！');"
								style='cursor:hand'>
								<IMG SRC="<s:url value="/images/update.png"/>" 
								BORDER='0' ALT='更新'onclick="alert('更新成功！');"
								style='cursor:hand'>
								<IMG SRC="<s:url value="/images/uninstall.png"/>" 
								BORDER='0' ALT='卸载'onclick="alert('卸载成功！');"
								style='cursor:hand'>
								<IMG SRC="<s:url value="/images/start.png"/>" 
								BORDER='0' ALT='启动'onclick="alert('启动成功！');"
								style='cursor:hand'>
								<IMG SRC="<s:url value="/images/stop.png"/>" 
								BORDER='0' ALT='停止'onclick="alert('已停止！');"
								style='cursor:hand'>
								<IMG SRC="<s:url value="/images/restart.png"/>"
								BORDER='0' ALT='重置'onclick="alert('已重置！');"
								style='cursor:hand'>		
								</td>
							</s:if>
							<s:else>
		...
		<s:if test="data.size()>0">
			<tr>
				<td align="right"><lk:pages
					url="/gtms/blocTest/softProbe!gopageDeviceOperate.action"
