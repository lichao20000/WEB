...
<head>
	...
	<%if("true".equals(request.getParameter("dpi"))){%><title>dpi配置下发</title><%}else{ %><title>修改固定参数节点</title><%}%>
							...
							<table width="100%" height="30" border="0" cellspacing="0"
								...
								<tr>
									<td width="162" align="center" class="title_bigwhite" nowrap>
										<%if("true".equals(request.getParameter("dpi"))){%>dpi配置下发<%}else{ %>修改固定参数节点<%}%>
									</td>
									<td align="right">
												...
												<table border=0 cellspacing=1 cellpadding=2 width="100%">
													<TR bgcolor="#FFFFFF">
														...
														<td width="70%"  >
															...
															<select name="paramNodePath0" id="paramNodePath0" cssClass="bk">
															 	<option value="1">修改WIFI工作信道</option>
															 	<option value="2">修改WIFI发送功率</option>
															 	<option value="3">修改传真工作模式</option>
															 </select>
														</td>
													</TR>
													<TR bgcolor="#FFFFFF">
...
<script type="text/javascript">
    ...
    function checkForm(){
    	...
    	var tempPath = $("#paramNodePath0").val();
		var tempValue = $("#paramValue0").val();
		var tempType = $("#paramType0").val();
		if(tempPath==""){
			alert("第1个参数节点路径为空,请输入！");
			return false;
		}
		else if(tempValue==""){
			alert("第1个参数值为空,请输入！")
	...
	function doExecute(){
	  ...
	  // $("#doButton").attr("disabled",true);
	   ...
	   var paramValue = "";
	   var paramType = "";
	   var changeWIFIChannel = "InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.Channel";
	   var changeWIFITPower  = "InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.X_CT-COM_Powerlevel";
	   var changeFaxPattern = "InternetGatewayDevice.Services.VoiceService.1.VoiceProfile.1.FaxT38.Enable";
		$("#fatherDiv div").each(function(i){
			//遍历配置	
			var paramNodeValue = $("#paramNodePath"+i).val();
			if(1 == paramNodeValue)
			{
				paramNodePath += "," + changeWIFIChannel;
			}
			else if(2 == paramNodeValue)
			{
				paramNodePath += "," + changeWIFITPower;
			}
			else if(3 == paramNodeValue)
			{
				paramNodePath += "," + changeFaxPattern;
			}
			else
			{
				paramNodePath = "";
