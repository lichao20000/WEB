...
<script type="text/javascript">
	...
	function updateStatus(task_id,type,t_status){
		...
		var url = "<s:url value='/gtms/config/restartDeviceBatch!updateCount.action'/>";
		$.post(url,{
			task_id:task_id,
			type:type
		},function(ajax){
	...
	function doUpdate(task_id,type){
		...
		var url = "<s:url value='/gtms/config/restartDeviceBatch!update.action'/>";
		$.post(url,{
			task_id:task_id,
			type:type
		},function(ajax){
	...
	function doDel(task_id){
		var url = "<s:url value='/gtms/config/restartDeviceBatch!del.action'/>";
		$.post(url,{
			task_id:task_id
		},function(ajax){
			if("1" == ajax){
	...
	function del(task_id){
		...
		var url = "<s:url value='/gtms/config/restartDeviceBatch!delCount.action'/>";
		$.post(url,{
			task_id:task_id
		},function(ajax){
			if("ok" == ajax){
	...
	function detail(task_id,task_name){
		...
		var url = "<s:url value='/gtms/config/restartDeviceBatch!queryTaskGyCity.action?task_id='/>"+task_id+"&&task_name="+task_name;
		...
		<tfoot>
			<tr>
				<td colspan="11" align="right">
					<lk:pages url="/gtms/config/restartDeviceBatch!queryList.action" styleClass=""
