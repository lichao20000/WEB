...
<script type="text/javascript">
	...
	function updateStatus(task_id,type,t_status){
		...
		var url = "<s:url value='/gtms/config/stackRefreshTools!updateCount.action'/>";
		$.post(url,{
			task_id:task_id,
			type:type
		},function(ajax){
	...
	function doUpdate(task_id,type){
		...
		var url = "<s:url value='/gtms/config/stackRefreshTools!update.action'/>";
		$.post(url,{
			task_id:task_id,
			type:type
		},function(ajax){
	...
	function doDel(task_id){
		var url = "<s:url value='/gtms/config/stackRefreshTools!del.action'/>";
		$.post(url,{
			task_id:task_id
		},function(ajax){
			if("1" == ajax){
	...
	function del(task_id){
		...
		var url = "<s:url value='/gtms/config/stackRefreshTools!delCount.action'/>";
		$.post(url,{
			task_id:task_id
		},function(ajax){
			if("ok" == ajax){
	...
	function detail(task_id,task_name){
		...
		var url = "<s:url value='/gtms/config/stackRefreshTools!queryTaskGyCity.action?task_id='/>"+task_id+"&&task_name="+task_name;
			...
			<s:if test="taskList.size()>0">
				<s:iterator value="taskList">
					<tr>
						...
						<ms:inArea areaCode="cq_dx" notInMode="false">
						<s:if test="t_status=='doing'">
							<td align="center">加载中，请勿暂停</td>
						</s:if>
						<s:else>
							<s:if test="status!=7">
						...
						<td align="center" width="300px" title='<s:property value="parampath" />'><s:property value="parampath" /></td>
						<td align="center" width="100px" title='<s:property value="paramvalue" />'><s:property value="paramvalue" /></td>
						<td align="center" width="100px" title='<s:property value="paramtype" />'><s:property value="paramtype" /></td>
						...
						<td align="center">
						<ms:inArea areaCode="cq_dx" notInMode="false">
						<s:if test="status!=7"><a href="javascript:updateStatus('<s:property value="task_id" />','7','<s:property value="t_status" />')">暂停</a></s:if>
						<s:else><a href="javascript:updateStatus('<s:property value="task_id" />','1','')">启动</a></s:else>
						</ms:inArea>
		...
		<tfoot>
			<tr>
				<td colspan="11" align="right">
					<lk:pages url="/gtms/config/stackRefreshTools!queryList.action" styleClass=""
