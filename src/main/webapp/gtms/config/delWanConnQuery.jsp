...
function gwShare_queryDevice(){
	...
	var url="<s:url value="/gtms/config/delWanConnToolsAct!queryDeviceList.action"/>?gwShare_queryResultType="+gwShare_queryResultType;
	...
	url=url+"&maxupline="+<%=upMax1%>;
	url=url+"&countMax="+<%=countMax1%>;
	var tips = "";
	var isHaveBind;
...
<input type="hidden" name="upMax" value="<%=upMax1%>" />
<input type="hidden" name="countMax" value="<%=countMax1%>" />
			...
			<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
				...
				<tr id="gwShare_tr32" style="display:none">
					...
					1、需要导入的文件格式限于文本文件，即txt格式 。
					 <br>
					2、文件的第一行为标题行，即"LOID|业务类型（INTERNET、VOIP、ITV、VPDN）|业务vlan"。
					 <br>
					 <br>
					3、文件行数不要超过1000行，以免影响性能。
					</td>
				</tr>
				<tr bgcolor="#FFFFFF">
