...
function gwShare_comfirmDevice(){
	...
	if("全部取消"==$("input[@name='gwShare_selectall']").val()){
		...
		if(confirm("选择的宽带帐号数量为:"+(gwShare_total-deviceSum)+",是否继续?")){
		...
		if(confirm("选择的宽带帐号数量为:"+(deviceSum)+",是否继续?")){
...
<table class="listtable" width="100%" align="center">
	<s:if test="'0'!=gwShare_RecordNum">
		<div style="text-align: center;width: 100%;height: 20px;" class="green_foot">查询结果条数为：<s:property value="total"/>！</div>
	</s:if>
	<thead>
		<tr>
			...
			<th align="center" width="47.5%">宽带帐号</th>
			<th align="center" width="47.5%">上网个数</th>
		</tr>
	</thead>
	<tbody>	
