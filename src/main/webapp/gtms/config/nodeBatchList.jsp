...
<script type="text/javascript">
	...
	function dodownload(filename,dostatus,downloadType){
		if("1" != dostatus){
			alert("暂未执行任务");
			return;
		}
		...
		}else if("xls" == downloadType){
			filename = filename+".xls";
		}
		$("#nameDowmload").val(filename);
			var form = document.getElementById("frm");
				form.action ="<s:url value="/gtms/config/nodeBatchDoFileDown.jsp"/>";
				form.submit();
	}
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="it_table">
					...
					<s:if test="rlist.size()>0">
						<s:iterator value="rlist">
							<tr>
								...
								<td><a href="javascript:dodownload('<s:property value="filename"/>','<s:property value="dostatus"/>','txt');" class="itta_button">txt下载</a>
									<a href="javascript:dodownload('<s:property value="filename"/>','<s:property value="dostatus"/>','xls');" class="itta_button">xls下载</a>
									</td>
							</tr>
						</s:iterator>
					</s:if>
				...
				<tr>
					<td colspan="9" align="right">
						<lk:pages
							url="/gtms/config/getNodeBatchConfigAction!getNodeBatchList.action"
