...
<script type="text/javascript">
	...
	function ExecMod() {
		if (CheckForm()) {
			...
			$("div[@id='div_ping']").html("正在载入诊断结果，请耐心等待....");
			var deviceId = $("input[@name='deviceId']").val();
			$.ajax({
						type : "POST",
						url : "<s:url value='/gtms/stb/diagnostic/systemCmd!restore.action'/>",
						data : {
							"deviceId" : deviceId,
							"gw_type" : gw_type
						},
			...
			<form name="frm" method="POST" action=""
				...
				<table width="100%" class="querytable">
					...
					<TR id="trDeviceResult" style="display: none">
						<td class=title_2 width="25%">设备序列号</td>
						<td id="tdDeviceSn" width="25%"></td>
						<td class=title_2 width="25%">属地</td>
						<td id="tdDeviceCityName" width="25%"></td>
					</tr>
					<tr bgcolor="#FFFFFF">
						<td colspan="4" class=foot>
							<div align="right">
								<button id="reStart_btn" onclick="ExecMod()">恢复出厂设置</button>
							</div>
						</td>
					</tr>
					<TR>
						<TH colspan="4">恢复出厂设置操作结果</TH>
					</TR>
					<TR bgcolor="#FFFFFF">
