...
<html>
	...
	function viewTask(taskId){ 
		var page = "<s:url value='/gtms/stb/resource/openDeviceShowPic!getShowPictureConfigResult.action'/>?taskId="+taskId;
	...
	function deleteTask(taskId){
		if(confirm("删除是不可恢复的，确定要删除吗？") == true){     //如果用户单击了确定按钮 
			var url = "<s:url value='/gtms/stb/resource/openDeviceShowPic!doDelete.action'/>";
			$.post(url,{
				taskId : taskId
			},function(ajax){
				 if(ajax=='1'){
   	...
   	function queryTask()
	{
		...
		var isLocked = $("select[name=isLocked]").val();
		var url = "<s:url value='/gtms/stb/resource/openDeviceShowPic!initImport.action'/>?taskName="+taskName+"&accName="+accName+"&startTime="+startTime+"&endTime="+endTime+"&isLocked="+isLocked;
