...
<SCRIPT LANGUAGE="JavaScript">
	...
	{
		var url = "<s:url value='/gtms/stb/resource/gwDeviceQueryStb!queryDeviceDetail.action?deviceId='/>"+deviceId;
		window.open(url ,"","left=80,top=80,width=800,height=400,resizable=yes,scrollbars=yes");
	}
	function ToExcel() {
		var form = document.getElementById("form");
		form.action = "<s:url value='/gtms/stb/resource/gwDeviceQueryStb!toExcel.action'/>?cityId="
			+'<s:property value="cityId"/>'
			+ "&devicetypeId=" + '<s:property value="devicetypeId"/>'
			+ "&queryType=2&noChildCity="+'<s:property value="noChildCity"/>'
			+ "&completeStartTime=" + '<s:property value="completeStartTime"/>'
			+ "&completeEndTime=" + '<s:property value="completeEndTime"/>'
			+"&vendorId=" + '<s:property value="vendorId"/>'
			+"&deviceModelId=" + '<s:property value="deviceModelId"/>'
			+"&bindType=" + '<s:property value="bindType"/>';
		//alert(form.action);
		form.target = "";
		form.submit();
	}
...
<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center" bgcolor=#999999>
	...
	<s:iterator value="deviceList">
		<tr bgcolor="#FFFFFF" >
			...
			<td class=column1 ><a class='green_link' href="#" onclick="deviceDetail('<s:property value="device_id"/>')"><s:property value="servAccount"/></a></td>
