...
function gwShare_queryDevice(){
	...
	var url="<s:url value="/gtms/stb/share/shareDeviceQuery!queryDeviceList4SoftUp.action"/>?gwShare_queryResultType="+gwShare_queryResultType;
    ...
    if("1"==gwShare_queryType){
        ...
        //江西itv去掉‘在线状态’字段，由于后台使用公用类查询，故传-1补充
	 	var gwShare_onlineStatus = -1;
        ...
        //江西itv能根据IP地址段过滤（目前这个界面仅江西itv使用，故不加省份判断）
        var gwShare_IPType = $("select[@name='gwShare_IPType']").val();
        ...
        url = url + "&gwShare_IPType=" + gwShare_IPType;
	}else {
		return;
	}
...
function gwShare_change_select(type,selectvalue){
	switch (type){
		case "city":
			var url = "<s:url value="/gtms/stb/share/shareDeviceQuery!getAllCityNextChild.action"/>";
			$.post(url,{
			},function(ajax){
...
<input type="hidden" name="gwShare_RecordNum" value="10" /><!-- 默认3条记录，0为查询全部，建议配置大小不超过50 -->
...
<TABLE width="100%" class="querytable" align="center">
	...
	<TR id="gwShare_tr21" STYLE="display:none">
		...
		<TD align="right" class="title_2" width="15%">IP选择</TD>
		<TD width="35%">
			<select name="gwShare_IPType" class="bk">
				<option value="-1">==请选择==</option>
				<option value="0">10开头</option>
				<option value="1">非10开头</option>
			</select>
		</TD>
	</TR>
