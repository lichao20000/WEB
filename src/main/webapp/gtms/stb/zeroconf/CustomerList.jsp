	...
	<table width="100%" class="listtable" align="center"
		...
		<thead>
			<tr>
				...
				<s:if test="%{dto.userStatus==0}"></s:if>
				<s:elseif test="%{dto.userStatus==''}"></s:elseif>
				<s:else>
			...
			<s:if test="dtoList.size()>0">
				<s:iterator var="list" value="dtoList">
					<tr>
						...
						<td><a
							onclick="deviceDetail('<s:property value="#list.deviceId" />')"
							style="cursor: hand"> <s:property value="#list.deviceSN" /></a></td>
						...
						<s:else>
							<td><label
								onclick="javascript:cusomerDetail('<s:property value="#list.customerId" />','<s:property value="#list.deviceId" />','<s:property value="dto.userStatus" />');">
				...
				<tr>
					<td class="foot" colspan="8">
						<div style="float: right">
							<lk:pages
								url="/gtms/stb/zeroconf/zeroConfStatisticsReportQuery!listCustomer.action"
								isGoTo="true" />
						</div>
					</td>
				</tr>
