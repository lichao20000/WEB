...
$(document).ready(function(){
	...
	var selectItem = '<s:property value="logObj.itemSelect" />';
	$("#item").val(selectItem);
...
function checkForm(){
	$("input[name='logObj.itemSelect']").val($("#item").val());
...
function exportLog()
{
	var param = 
	{
		'logObj.account' : $("input[name=logObj.account]").val(),
		'logObj.item' : $("input[name=logObj.item]").val(),
		'logObj.starttime' : $("input[name=logObj.starttime]").val(),
		'logObj.endtime' : $("input[name=logObj.endtime]").val()
	};
	var item = document.getElementsByName("logObj.item")[0].value;
...
function queryDetail(account,itemSelect,starttime,endtime,area,role,areaName,roleName)
{
	var url = "<s:url value='/gtms/system/logManage!queryLog.action'/>";
	var param = "?logObj.itemSelect="+itemSelect+"&logObj.account="+account+"&logObj.starttime="+starttime+"&logObj.endtime="+endtime+"&logObj.area_id="+area+"&logObj.role_id="+role+"&area_name="+areaName+"&gr_name="+roleName;
	window.open(url+param,"","left=80,top=80,width=800,height=400,resizable=yes,scrollbars=yes");
}
</script>
...
<form id="frm" name="frm" method="post"
	action="<s:url value='/gtms/system/logManage!queryLogSAS.action'/>">
...
<table class="querytable" align="center" width="98%" style="margin-top: 15px">
	<tr>
		<td colspan="4" class="title_1">日志统计查询</td>
	</tr>
	<tr>
		...
		<td><select name="logObj.item" size="1" id="item">
		...
		<input type="hidden" name="logObj.itemSelect">
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<INPUT TYPE="hidden" NAME="area_id" value='<s:property value="logObj.area_id" />'><nobr class="BT" onmouseover="this.className='BTOver'" onmouseout="this.className='BT'" onclick="areaSelect();"><IMG SRC="images/search.gif" WIDTH="15" HEIGHT="12" BORDER="0" ALT="查找角色" valign="middle">&nbsp;选择</nobr>
	...
	<tr>
		...
		<td><label><lk:date id="logObj.endtime" name="logObj.endtime" type="all"  /></label></td>
	</tr>
	<tr>
...
<table class="listtable" width="98%" align="center" style="margin-top: 15px">
	...
	<s:iterator value="logList" status="status">
		<tr>
			...
			<td><s:property value="oper_cont" /></td>
			<td>
				<a onclick="queryDetail('<s:property value="acc_loginname" />','<s:property value="logObj.itemSelect" />','<s:property value="logObj.starttime" />','<s:property value="logObj.endtime" />','<s:property value="logObj.area_id" />','<s:property value="logObj.role_id" />','<s:property value="area_name" />','<s:property value="gr_name" />');" href="#" ><s:property value="usedCount" /></a>
			</td>
			<td>
	...
	<tr>
		<td colspan="7">
			 <div class="right">
			 <lk:pages url="/gtms/system/logManage!queryLogSAS.action" styleClass="" showType="" isGoTo="true" changeNum="true" />
			 </div>
		</td>
	</tr>
	</tfoot>
