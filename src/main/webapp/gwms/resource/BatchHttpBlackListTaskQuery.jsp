...
<html>
	...
	function viewTask(taskId){ 
	    ...
	    var url = "<s:url value='/gwms/resource/batchHttpTestBlackListMana!getTaskCount.action'/>";
		$.post(url,{
			taskId:taskId
		},function(ajax){
	...
	function disableTask(taskId){
		if(confirm("确定要恢复限制吗？") == true){     //如果用户单击了恢复限制按钮 
			var url = "<s:url value='/gwms/resource/batchHttpTestBlackListMana!doDelete.action'/>";
			$.post(url,{
				taskId : taskId
			},function(ajax){
				 if(ajax=='1'){
...
/* 	function deleteTask(taskId){
	...
	function commitDesc(){
		...
		var url = "<s:url value='/gwms/resource/batchHttpTestBlackListMana!commitDesc.action'/>";
		$.post(url,{
			taskId : task_id,
			task_desc:desc
		},function(ajax){
   	...
   	function queryTask()
	{
		...
		var url = "<s:url value='/gwms/resource/batchHttpTestBlackListMana!init.action'/>?taskName="+taskName+"&accName="+accName+"&startTime="+startTime+"&endTime="+endTime;
                        ...
                        <tfoot>
                                <tr bgcolor="#FFFFFF">
                                        <td colspan="6" align="right"><lk:pages
                                                url="/gwms/resource/batchHttpTestBlackListMana!init.action" styleClass=""
