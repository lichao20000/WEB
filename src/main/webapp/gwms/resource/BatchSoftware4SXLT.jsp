...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function CheckForm () {
    ...
    var mode = $("#mode").val();
    if (mode == "2") {
      if ((returnValthis[0] == 0 && returnValthis[2].length > max) ||
        (returnValthis[0] != 0 && returnValthis[0] > max)) {
    	...
    	var obj = $("input[@name='softStrategy_type']");
        var check_val = '';
        for(k in obj){
            if(obj[k].checked)
            	check_val = check_val + obj[k].value;
        }
        $("input[@name='type']").val(check_val);
    }
                ...
                <tr>
                    <td>
                        <FORM NAME="mainForm" id="mainForm" METHOD="post"
                              ACTION="<s:url value="/gwms/resource/software!batchUp4sxlt.action"/>"
