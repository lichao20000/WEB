    ...
    url = url + "&gwShare_queryType=" + gwShare_queryType;
  	if("1"==gwShare_queryType){
		...
		if($("input[@name='hblt_SpeedTest_flg']").val()=="true"){
			var url="<s:url value="/gwms/resource/batchHttpTestBlackList!saveUpFile.action"/>";
...
function do_query4BlackList(){
	...
	{
	url="<s:url value="/gwms/resource/batchHttpTestBlackList!getDeviceIfo.action"/>?gwShare_queryResultType="+gwShare_queryResultType;
	var gwShare_queryType = $("input[@name='gwShare_queryType']").val();
    url = url + "&gwShare_queryType=" + gwShare_queryType;
	var gwShare_queryField = $("input[@name='gwShare_queryField'][@checked]").val();
	var gwShare_queryParam = $.trim($("input[@name='gwShare_queryParam']").val());
...
function gwShare_revalue(){
		...
		$("select[@name='gwShare_nextCityId']").attr("value",'-1')
...
function gwShare_queryChange(change){
	...
	switch (change){
		case "2":
			$("th[@id='gwShare_thTitle']").html("高级订置黑名单");
	...
	<tr>
		<td bgcolor=#999999>
			<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
				<tr><th colspan="4" id="gwShare_thTitle" STYLE="display:">简 单 查 询</th></tr>
				<tr bgcolor="#FFFFFF" id="gwShare_tr11" STYLE="display:">
					<td colspan="4" align="center" width="100%">
						<div>
							<input type="input" class="bk" name="gwShare_queryParam" size="64" maxlength="64"/><span style="color:red">黑名单查询请输入完整序列号或是宽带名称</span>
							<br /> 
				...
				<tr bgcolor="#FFFFFF">
					<td colspan="4" align="right" class="green_foot" width="100%">
						<input type="button" onclick="javascript:do_query4BlackList();" class=jianbian 
						name="gwShare_queryButton5" value="黑名单查询 " />					
						...
						<ms:inArea areaCode="js_dx" notInMode="true">
							<input type="button" class=jianbian style="CURSOR:hand" style="display:none " 
							onclick="javascript:gwShare_queryChange('2');" name="gwShare_gaoji" value="高级定制" />
						</ms:inArea>
						<input type="button" class=jianbian style="CURSOR:hand" style="display: none" 
						onclick="javascript:gwShare_queryChange('3');" name="gwShare_import" value="导入批量设置" />						
						<input type="button" class=jianbian style="CURSOR:hand" style="display: none" 
