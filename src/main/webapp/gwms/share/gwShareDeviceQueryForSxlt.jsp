    ...
    <SCRIPT LANGUAGE="JavaScript">
      ...
      function do_test () {
        ...
        //"设备IP"被选中
          ...
          if (gwShare_queryParam != "" && gwShare_queryParam != undefined 
        		  && !reg_verify(gwShare_queryParam)) {
            alert("请输入合法的IP地址！");
            document.gwShare_selectForm.gwShare_queryParam.focus();
            return false;
          }
...
//	gwShare_spellSQL();
	  ...
	  function gwShare_queryField_selected (param_input) {
			// 参数类型
			var param_type = param_input.value;
			if(param_type == 'deviceSn'){
				$('#search_param').html("设备序列号");
			}else if(param_type == 'deviceIp'){
				$('#search_param').html("设备IP");
			}else if(param_type == 'username'){
				$('#search_param').html("唯一标识");
			}else if(param_type == 'kdname'){
				$('#search_param').html("宽带账号");
			}else if(param_type == 'voipPhoneNum'){
				$('#search_param').html("VOIP电话号码");
			}
           	$("input[@name='gwShare_queryField_temp']").val(param_input.value);
      }
      ...
      function gwShare_queryDevice () {
        ...
        if (batchType != 0) {
          ...
          var url = "<s:url value="/gwms/share/gwDeviceQuery!queryDeviceListForSxlt.action"/>?gwShare_queryResultType=" +
        ...
        if ("1" == gwShare_queryType) {
          ...
          // 增加时间查询条件
          var startTime = $dp.$('starttime').value;
          var endTime = $dp.$('endtime').value;
        ...
        if ($("#chbx_isMactchSQL").attr("checked") == true) {
          var gwShare_matchSQL = $("textarea[@name='gwShare_matchSQL1']").val() + " " +
            $("textarea[@name='gwShare_matchSQL']").val();
          ...
          url = "<s:url value='/gwms/share/gwDeviceQuery!queryDeviceListBySQL.action'/>?gwShare_matchSQL=" +
            gwShare_matchSQL + "&gwShare_queryResultType=" + gwShare_queryResultType;
        }
      ...
      function querynumber () {
        var gwShare_queryResultType = $("input[@name='gwShare_queryResultType']").val();
        var url = "<s:url value='/gwms/share/gwDeviceQuery!querynumber.action'/>?gwShare_queryResultType=" +
          gwShare_queryResultType;
        var gwShare_queryType = $("input[@name='gwShare_queryType']").val();
        url = url + "&gwShare_queryType=" + gwShare_queryType;
      ...
      function gwShare_queryChange (change) {
        ...
        switch (change) {
          case "2":
            ...
            $("tr[@id='gwShare_tr13']").css("display", "none");
          ...
          case "3":
            ...
            if (isBatchSpeedTest == "true" &&
              ("hb_lt" == "<%=LipossGlobals.getLipossProperty("InstArea.ShortName") %>")) {
              $("#batch_test").show();
          ...
          case "1":
            ...
            $("tr[@id='gwShare_tr13']").css("display", "");
            $("tr[@id='gwShare_tr25']").css("display", "none");
            $("tr[@id='gwShare_tr26']").css("display", "none");
            $("tr[@id='gwShare_tr21']").css("display", "none");
            $("tr[@id='gwShare_tr22']").css("display", "none");
      ...
      function gwShare_parseMessage (type, ajax, field, selectvalue) {
        ...
        if ("city" == type && lineData.length == 1) {
          // 如果只有一个属地就不可再选
        } else {
        ...
        for (var i = 0; i < lineData.length; i++) {
          ...
          if (selectvalue == xValue || ("city" == type && lineData.length == 1)) {
            flag = false;
...
<form name="gwShare_selectForm" action="<s:url value="/gwms/share/gwDeviceQuery!queryDeviceList.action"/>"
      target="dataForm">
        ...
        <tr>
            <td bgcolor=#999999>
                <table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
                    <tr>
                        <th colspan="6" id="gwShare_thTitle" STYLE="display:">简 单 查 询</th>
                    </tr>
                    <tr bgcolor="#FFFFFF" id="gwShare_tr11" STYLE="display:">
                    	<!-- 查询参数 -->
                        <td class="column" width='15%' align="right" id="search_param">设备序列号</td>
                        <td width='35%' align="left">
                            <input type="input" class="bk" style="width: 220px;" name="gwShare_queryParam" size="64" maxlength="64"/>
                            <br/>
                            <div id="gwShare_msgdiv" STYLE="display:none">
                            </div>
                        </td>
                        <td class="column" width='15%' align="right">参数类型</td>
                        <td align="left" width="100%">
                            <input type="radio" class=jianbian name="gwShare_queryField" value="deviceSn" checked 
								style="position: relative;top: -2px;" onclick="gwShare_queryField_selected(this)"/>设备序列号
                            <input type="radio" class=jianbian name="gwShare_queryField" value="deviceIp" 
								style="position: relative;top: -2px;" onclick="gwShare_queryField_selected(this)"/>设备IP
                            <input type="radio" class=jianbian name="gwShare_queryField" value="username" 
								style="position: relative;top: -2px;" onclick="gwShare_queryField_selected(this)"/>唯一标识
                            <input type="radio" class=jianbian name="gwShare_queryField" value="kdname" 
								style="position: relative;top: -2px;" onclick="gwShare_queryField_selected(this)"/>宽带账号
                            <input type="radio" class=jianbian name="gwShare_queryField" value="voipPhoneNum" 
								style="position: relative;top: -2px;" onclick="gwShare_queryField_selected(this)"/>VOIP电话号码
                        </td>
                    </tr>
                    <tr bgcolor="#FFFFFF" id="gwShare_tr12" STYLE="display:">
                        <td class="column" width='15%' align="right">开始时间</td>
                        <td width='35%' align="left">
                        	<input id='starttime' type="text" name="starttime" class='bk' readonly
                                value="<s:property value='starttime'/>" style="width: 220px;"> 
                            <img onclick="WdatePicker({el:$dp.$('starttime'),dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'whyGreen'})"
                                src="<s:url value='/images/dateButton.png'/>" width="15" height="12"
                                border="0" alt="选择"></td>
                        <td class="column" width='15%' align="right">结束时间</td>
                        <td width='35%' align="left">
                        	<input type="text" id = 'endtime' name="endtime" class='bk' readonly 
                        		value="<s:property value='endtime'/>" style="width: 220px;">
                            <img onclick="WdatePicker({el:$dp.$('endtime'),dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'whyGreen'})"
                                src="<s:url value='/images/dateButton.png'/>" width="15" height="12" border="0" alt="选择">
                        </td>
                    </tr>
                    <tr bgcolor="#FFFFFF" id="gwShare_tr13" STYLE="display:">
                        <td class="column" width='15%' align="right">查询时间类型</td>
                        <td width='35%' align="left" colspan = '3'>
                        	<select id = 'timeType' name="timeType" class="bk" style="width: 220px;">
	                            <option value="-1" selected="selected">==请选择==</option>
	                            <option value="1" >==上报时间==</option>
	                            <option value="2">==绑定时间==</option>
                        	</select>
                        </td>
                    </tr>
                    <TR bgcolor="#FFFFFF" id="gwShare_tr21" STYLE="display:none">
                        ...
                        <TD align="left" width="35%">
                            <div id="commonCity" style="display: ">
                                <div style="float: left;">
                                    <select name="gwShare_cityId" class="bk" style="width: 220px;"
                                            onchange="gwShare_change_select('nextCity','-1')">
                                        <option value="-1">==全部==</option>
                                    </select>
                                </div>
                                <div style="float: left; display: none;" id="nextCityDiv">
                                    <select name="gwShare_nextCityId" class="bk" style="width: 220px;">
                                        <option value="-1">==全部==</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <TD width="35%" colspan="3">
                            <select name="gwShare_onlineStatus" class="bk" style="width: 220px;">
                                <option value="-1">==全部==</option>
                    ...
                    <TR bgcolor="#FFFFFF" id="gwShare_tr22" STYLE="display:none">
                        ...
                        <TD width="35%">
                            <select name="gwShare_vendorId" class="bk" style="width: 220px;"
                        ...
                        <TD align="left" width="35%" colspan="3">
                            <select name="gwShare_deviceModelId" class="bk" style="width: 220px;"
                    ...
                    <TR bgcolor="#FFFFFF" id="gwShare_tr23" STYLE="display:none">
                        ...
                        <TD width="35%">
                            <select name="gwShare_devicetypeId" class="bk" style="width: 220px;">
                            	<option value="-1">请先选择设备型号</option>
                            </select>
                        </TD>
                        <TD align="right" class=column width="15%">是否绑定</TD>
                        <TD width="35%" colspan="3">
                            <select name="gwShare_bindType" class="bk" style="width: 220px;">
                                <option value="-1">==全部==</option>
                    ...
                    <TR bgcolor="#FFFFFF" id="gwShare_tr24" STYLE="display:none">
                        ...
                        <TD width="35%">
                            <input type="text" name="gwShare_deviceSerialnumber" value="" size="25" maxlength="25"
                                   class="bk" style="width: 220px;"/>
                        ...
                        <%} %>
                        <TD align="right" class=column width="15%" id="gwShare_td1" STYLE="display:none">
                            <font>自定义查询</font></TD>
                        <TD width="35%" STYLE="display:none">
                            <input type="checkbox" name="chbx_isMactchSQL" id="chbx_isMactchSQL"
                                   onclick="spellSQL(this);">
                        </TD>
                    </TR>
                        ...
                        <td colspan="5" width="85%">
                            <div id="importUsername">
                                <iframe name="gwShare_loadForm" FRAMEBORDER=0 SCROLLING=NO
                                        src="<s:url value="/gwms/share/FileUpload.jsp"/>" height="30" width="100%">
                                </iframe>
                    ...
                    <tr bgcolor="#FFFFFF">
                        <td colspan="6" align="right" class="green_foot" width="100%">
                            ...
                            <ms:inArea areaCode="js_dx" notInMode="true">
                                <input type="button" class=jianbian style="CURSOR:pointer;display:none "
                                       onclick="javascript:gwShare_queryChange('2');" name="gwShare_gaoji"
                                       value="高级查询"/>
                            </ms:inArea>
                            <input type="button" class=jianbian style="CURSOR:pointer;display:none"
                            ...
                            <input type="button" class=jianbian style="CURSOR:pointer;display: none"
                                   onclick="javascript:gwShare_queryChange('4');" name="gwShare_up_import"
                                   value="升级导入查询"/>
