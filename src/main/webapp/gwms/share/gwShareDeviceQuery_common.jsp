...
function do_query(){
	...
	<%if("js_dx".equals(shortName)){%>
		if($("#location_name").val()=="WirelessOpenBySpecialBuss"){
			gwShare_queryDevice();
			return;
		}
	<%}%>
...
function isawifi()
{
	...
	var url="<s:url value='/gwms/share/gwDeviceQuery!queryisawifi.action'/>";
	var gw_type= <%=gw_type%>;
	$.post(url,{
...
function gwShare_queryDevice(){
	...
	var url ="<s:url value="/gwms/share/gwDeviceQuery!queryDeviceList4Wireless.action"/>?gwShare_queryResultType="+gwShare_queryResultType+ "&currencyType="+currencyType;
	var gwShare_queryType = $("input[@name='gwShare_queryType']").val();
	...
	var gwShare_queryParam = $.trim($("input[@name='gwShare_queryParam']").val());
	var isBindUrl = "<s:url value='/gtms/config/wirelessBusnissCtrl!isBind.action'/>";
...
$(function(){
	...
	var CurrencyType=<%=CurrencyType %>
	if("1"==CurrencyType){
	var appText=document.getElementById("gwShare_tr32_2");
	appText.innerHTML="、WLAN端口（SSID3、SSID4）"
	}
	//针对现网慢的情况，暂时去掉自适应匹配
			...
			<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
				...
				<tr id="gwShare_tr32" style="display:none">
					...
					1、需要导入的文件格式限于Excel、文本文件，即xls、txt格式 。
					 <br>
					2、<font color="blue">导入文件格式包含内容有：用户账号（或设备序列号）,SSID名称，VLANID值，业务优先级 ，无线信道<span id="gwShare_tr32_2"></span></font>(注意：如果导入文件格式为.txt,两个字段间用“,”隔开)
					 <br>
