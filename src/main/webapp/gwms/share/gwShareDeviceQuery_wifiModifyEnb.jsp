...
function gwShare_queryDevice(){
    ...
    if("1"==gwShare_queryType){
		...
		if("hb_lt" == instName && "txt" != gwShare_fileName.split(".")[1]){
			alert("系统暂不支持非txt格式的文件");
			return;
		}else if("xj_dx" == instName){
			url = url + "&limitNum=1";
		}
			...
			<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
				...
				<tr id="gwShare_tr32" style="display:none">
					...
					<td colspan="5" CLASS="green_foot">
					 ...
					 <div>
					    1、需要导入的文件格式限于Excel、文本文件即txt格式 。 <br>
					    2、文件的第一行为标题行，即【用户账号】、或者【设备序列号】。
					 </div>
					 <%}else{ %>
					 1、需要导入的文件格式限于Excel、文本文件即txt格式且以UTF-8编码 。 <br>
					2、文件的第一行为标题行，即【用户账号】、【宽带账号】或者【设备序列号】。
					<%} %>
					 <br>
					3、文件只有一列。
					 <br>
