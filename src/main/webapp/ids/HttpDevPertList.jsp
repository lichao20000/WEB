...
function ListToExcel(taskname,serialnumber,starttime2,endtime2){
	var page = "<s:url value='/ids/httpDownload!toExcel.action'/>?"
			+"taskname="+taskname
				...
				<s:if test="list.size()>0">
					...
					<td ><s:hidden value="task_id" /></td>
						<tr>
							...
							<td ><s:property value="total_bytes_rece" /></td>
							<td ><s:property value="downPert" /></td>
							<%if(!"nx_dx".equals(LipossGlobals.getLipossProperty("InstArea.ShortName"))){%>
							<td ><s:property value="totalDownPert" /></td>
							<%	}	%>
						</tr>
					</s:iterator>
				</s:if>
		...
		<tfoot>
			<tr>
				<td colspan="11"><span style="float:right"><lk:pages
						url="/ids/httpDownload!queryDevBySerialnumber.action" styleClass=""
