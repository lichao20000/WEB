...
function queryDetail(taskid){
	var url = "<s:url value='/ids/httpQualityTest!queryTaskInfo.action'/>?taskid="+taskid;
...
function del(taskid){
	...
	if (confirm(msg)==true){ 
		var url = "<s:url value='/ids/httpQualityTest!delTask.action'/>";
		$.post(url,{
			taskid : taskid
		},function(ajax){
			parent.doQuery();
...
function ListToExcel(taskname,acc_loginname,starttime,endtime){
	var page = "<s:url value='/ids/httpQualityTest!queryTaskExcel.action'/>?"
		+ "taskname="+taskname
		...
		<tfoot>
			<tr>
				<td colspan="7" align="right"><span  style="float:right"><lk:pages
						url="/ids/httpQualityTest!queryTask.action" styleClass=""
						showType="" isGoTo="true" changeNum="true" /></span>
				<IMG SRC="<s:url value="/images/excel.gif"/>" BORDER='0' ALT='导出列表'
						style='cursor: hand;float: left'
						onclick="ListToExcel('<s:property value="taskname"/>','<s:property value="acc_loginname"/>','<s:property value="starttime1"/>','<s:property value="endtime1"/>')">		
				</td>
			</tr>
