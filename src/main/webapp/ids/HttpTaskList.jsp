...
function queryDetail(taskid){
	var url = "<s:url value='/ids/httpDownload!queryTaskInfo.action'/>?taskid="+taskid;
...
function del(taskid){
	...
	if (confirm(msg)==true){ 
		var url = "<s:url value='/ids/httpDownload!delTask.action'/>";
		$.post(url,{
			taskid : taskid
		},function(ajax){
			parent.doQuery();
...
function ListToExcel(taskname,acc_loginname,starttime,endtime){
	var page = "<s:url value='/ids/httpDownload!queryTaskExcel.action'/>?"
		+ "taskname="+taskname
		...
		<tfoot>
			<tr>
				<td colspan="4" align="right"><span  style="float:right"><lk:pages
						url="/ids/httpDownload!queryTask.action" styleClass=""
