...
function queryDetail(taskid){
	var url = "<s:url value='/ids/httpUpload!queryTaskInfo.action'/>?taskid="+taskid;
...
function del(taskid){
	...
	if (confirm(msg)==true){ 
		var url = "<s:url value='/ids/httpUpload!delTask.action'/>";
		$.post(url,{
			taskid : taskid
		},function(ajax){
			parent.doQuery();
...
function ListToExcel(taskname,acc_loginname,starttime,endtime){
	var page = "<s:url value='/ids/httpUpload!queryTaskExcel.action'/>?"
		+ "taskname="+taskname
		...
		<tfoot>
			<tr>
				<td colspan="4" align="right"><span  style="float:right"><lk:pages
						url="/ids/httpUpload!queryTask.action" styleClass=""
