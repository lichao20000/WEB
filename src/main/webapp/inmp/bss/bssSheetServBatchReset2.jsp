...
function reset()
{
	...
	for(var j=1;j<rows.length;j++)
	{
		...
		if(box.checked)
		{
			var input = rows[j].cells[4].lastChild;
	...
	var url = "<s:url value='/inmp/bss/bssSheetServ!callPreProcessBatch.action'/>";
	$.post(url,{
...
function init()
{
	...
	{
		// 业务类型
		cell_servtype = trim(rows[i].cells[0].firstChild.innerHTML);
		
		// 开通状态
		cell_openstatus = trim(rows[i].cells[3].firstChild.innerHTML);
		...
		// 去除A标记
		rows[i].cells[2].firstChild.removeNode();
