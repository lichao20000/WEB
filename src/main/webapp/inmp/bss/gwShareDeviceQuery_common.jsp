...
function gwShare_queryDevice(){
	...
	var url="<s:url value="/inmp/share/gwDeviceQuery!queryDeviceList4Wireless.action"/>?gwShare_queryResultType="+gwShare_queryResultType;
    var gwShare_queryType = $("input[@name='gwShare_queryType']").val();
    url = url + "&gwShare_queryType=" + gwShare_queryType;
    if("1"==gwShare_queryType){
    	var gwShare_queryField = $.trim($("input[@name='gwShare_queryField'][@checked]").val());
	...
	if($("#chbx_isMactchSQL").attr("checked")==true)
    {
		...
		url="<s:url value="/inmp/share/gwDeviceQuery!queryDeviceListBySQL.action"/>?gwShare_matchSQL="+gwShare_matchSQL+"&gwShare_queryResultType="+gwShare_queryResultType;
    }
	var tips = "";
	var isHaveBind;
	var gwShare_queryField = $.trim($("input[@name='gwShare_queryField_temp']").val());
	var gwShare_queryParam = $.trim($("input[@name='gwShare_queryParam']").val());
	var isBindUrl = "<s:url value='/inmp/config/wirelessBusnissCtrl!isBind.action'/>";
	if(user_type == 1){
		$.post(isBindUrl, {
			...
			gwShare_queryParam:gwShare_queryParam
	 		...
	 		if(isHaveBind == 0){
				...
				alert("账号不唯一，请根据LOID查询");
	 			tips = "账号不唯一，请根据LOID查询";
	 			return;
			}else{
				var isAwifiUrl = "<s:url value='/inmp/config/wirelessBusnissCtrl!isAwifi.action'/>";
				$.post(isAwifiUrl, {
					deviceIds : ajax
			     },function(ajax){
			    	 if(ajax == 0){
...
function gwShare_change_select(type,selectvalue){
	switch (type){
		case "city":
			var url = "<s:url value="/inmp/share/gwDeviceQuery!getCityNextChild.action"/>";
			$.post(url,{
			},function(ajax){
		...
		case "deviceModel":
			...
			break;
		case "devicetype":
			var url = "<s:url value="/inmp/share/gwDeviceQuery!getDevicetype.action"/>";
			var vendorId = $("select[@name='gwShare_vendorId']").val();
...
function gwShare_spellSQL(){
	var url = "<s:url value='/inmp/share/gwDeviceQuery!queryDeviceListSQL.action'/>";
	var gwShare_queryResultType = $("input[@name='gwShare_queryResultType']").val();
    var gwShare_queryType = $("input[@name='gwShare_queryType']").val();
	var gwShare_cityId = $.trim($("select[@name='gwShare_cityId']").val());
    var gwShare_onlineStatus = $.trim($("select[@name='gwShare_onlineStatus']").val());
...
<form name="gwShare_selectForm" action="<s:url value="/inmp/share/gwDeviceQuery!queryDeviceList.action"/>" target="dataForm">
			...
			<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
				...
				<tr bgcolor="#FFFFFF" id="gwShare_tr12" STYLE="display:">
					<td colspan="4" align="center" width="100%">
						...
						<input type="radio" class=jianbian name="gwShare_queryField" value="username" onclick="gwShare_queryField_selected(this)"/> LOID &nbsp;&nbsp;
				...
				<tr id="gwShare_tr32" style="display:none">
					...
					1、需要导入的文件格式限于Excel、文本文件，即xls、txt格式 。
					 <br>
					2、<font color="blue">导入文件格式包含内容有：用户账号（或设备序列号）,SSID名称，VLANID值，业务优先级 ，无线信道</font>(注意：如果导入文件格式为.txt,两个字段间用“,”隔开)
					 <br>
					3、文件行数不要超过2000行，以免影响性能。
					</td>
				</tr>
				<tr bgcolor="#FFFFFF">
