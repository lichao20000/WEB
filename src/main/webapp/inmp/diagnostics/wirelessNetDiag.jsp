...
var gw_type = '1';  // 此诊断只有ITMS有，所以ITMS默认为1:ITMS
var wire_type = '2';//无线
...
function checkConnError(){
	if(document.all("connErrorCheckbox").checked == true){
		...
		var diagUrl = '<s:url value="/inmp/diagnostics/deviceDiagnostics!connErrorCheck.action"/>';
		//查询
		$.post(diagUrl,{
			deviceId:device_id,
			userId:user_id,
...
function lanHostCheck(){
	...
	if(document.all("lanHostCheckbox").checked == true){
		...
		var diagUrl = '<s:url value="/inmp/diagnostics/deviceDiagnostics!lanHostCheck.action"/>';
		$.post(diagUrl,{
			deviceId:device_id,
			gw_type:gw_type,
			wireType:wire_type
...
function dhcpCheck(){
	...
	if(document.all("dhcpCheckbox").checked == true){
		...
		var diagUrl = '<s:url value="/inmp/diagnostics/deviceDiagnostics!dhcpCheck.action"/>';
		$.post(diagUrl,{
			deviceId:device_id,
			gw_type:gw_type,
			wireType:wire_type
...
function dnsCheck(){
	...
	if(document.all("dnsCheckbox").checked == true){
		...
		var diagUrl = '<s:url value="/inmp/diagnostics/deviceDiagnostics!dnsCheck.action"/>';
		$.post(diagUrl,{
			deviceId:device_id,
			userId:user_id,
			gw_type:gw_type,
