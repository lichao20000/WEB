...
function ListToExcel(cityId,starttime1,endtime1,userTypeId,userline,isChkBind,is_active) {
	var page="<s:url value='/inmp/report/bindWay!getHgwExcel.action'/>?"
		+ "cityId=" + cityId 
		+ "is_active=" + is_active 
		...
		<s:if test="hgwList.size()>0">
			<s:iterator value="hgwList">
				<tr>
					...
					<td>
						<s:if test='opmode=="1"'>
							<IMG SRC="<s:url value='../../images/inmp/check.gif'/>" BORDER="0"
								ALT="已竣工" style="cursor: hand">
						</s:if>
						<s:else>
							<IMG SRC="<s:url value='./../images/inmp/button_s.gif'/>" BORDER="0"
		...
		<tr>
			<td colspan="7">
				<span style="float: right;"> <lk:pages
						url="/inmp/report/bindWay!getHgw.action" styleClass="" showType=""
