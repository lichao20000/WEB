<%@page import="com.linkage.module.gwms.dao.tabquery.CityDAO"%><%@ include file="../timelater.jsp"%>
...
<script type="text/javascript" src="../../Js/inmp/commFunction.js"/></script>
<script>
<!-- 
var request = false;
   try {
...
///////////////////////////////设置密码/////////////////////////////////////////
	...
	function getOnlineStatus() {
		...
		send_request_OS("testConnectionSubmit.jsp?device_id="+<%=device_id%>+"&tt="+new Date().getTime());
	}
...
function add(){
	...
	var url = "<s:url value='/inmp/midware/midWare!add.action'/>";
	operate(url);
}

function update(){
	$("input[@name='des']").val("change device info");
	var url = "<s:url value='/inmp/midware/midWare!update.action'/>";
	operate(url);
}

function del(){
	$("input[@name='des']").val("delete device");
	var url = "<s:url value='/inmp/midware/midWare!delete.action'/>";
	operate(url);
}

function midareOpen(){
	var deviceId = $.trim($("input[@name='deviceId']").val());
	var url = "<s:url value='/inmp/midware/businessOpen!midMareDevOpen.action'/>";
	$.post(url,{
		deviceId:deviceId
	},function(ajax){
	    alert(ajax);
...
function business_open(){	
	...
	var group = $.trim($("input[@name='group']").val());
	var phone = $.trim($("input[@name='phone']").val());
	var strpage="<s:url value='/inmp/midware/businessOpen.action'/>?"
		+ "&cityId="+cityId
		+ "&deviceModel="+deviceModel
		+ "&deviceId=" + deviceId
		+ "&oui=" + oui
...
<div id="PendingMessage"
	...
	<center>
		<table border="0">
			<tr>
				<td valign="middle">
					<img src="/images/inmp/cursor_hourglas.gif" border="0" WIDTH="30"
						HEIGHT="30">
				</td>
				<td>
					&nbsp;&nbsp;
