...
<%
	...
	LogItem.getInstance().writeItemLog(request, 1, "", "多宽带变更上网方式","in");
 %>
<SCRIPT LANGUAGE="JavaScript">
	...
	function deviceResult(returnVal) {
		var	width = 800;    
		var height = 450;
		var url = "<s:url value='/itms/config/changeConnectionType!getServInfo.action'/>?deviceId=" + returnVal[2][0][0];
		var reVlanid = window.showModalDialog(url , '', 
				'dialogWidth=' + width + 'px;dialogHeight=' + height + 'px;resizable=no;help=no;center=yes;status=no;scroll=yes;edge=raised');    
		if(typeof(reVlanid)=='undefined'){
			return;
		}else{
			servResult(returnVal, reVlanid);
		}
	}
	function servResult(returnVal, reVlanid){
		...
		checkBindAndService(deviceId, reVlanid);
	...
	function checkBindAndService(deviceId, reVlanid){
		document.all("pvcVlanValue").style.display="";
		document.all("pvcVlanValue1").style.display="none";
		$("#pvcVlanValue2").empty();
		var url = "<s:url value='/itms/config/changeConnectionType!checkBindAndService.action'/>";
		$.post(url,{
			deviceId: deviceId,
			gw_type: gw_type,
			vlan: reVlanid
	...
	function showVlan(vlanIds) {
		var v = vlanIds.split("|");
		for (var i = 0; i < v.length - 1; i++) {
			$("#pvcVlanValue2").append("<option value='"+v[i]+"'>"+v[i]+"</option>");
		}
	}
	 //清空options集合
	...
	function usernameOnclick(username,passwd,pvcNow,user_id_a,wan_type,accessTypeName) {
		...
		if(wan_type=="1"){
			...
			document.all("pvcVlanValue1").style.display="";
  		   ...
  		   //获取PVC
  		 	var url = "<s:url value='/itms/config/changeConnectionType!getAllPVCConfig.action'/>";
			$.post(url,{
				deviceId:deviceId
			},function(ajax){
	        	   //alert(ajax);
...
<TABLE border=0 align="center" cellspacing=0 cellpadding=0 width="98%">
	...
	<TR>
		...
		<table width="100%" height="30" border="0" cellpadding="0"
			...
			<tr>
				<td width="162" align="center" class="title_bigwhite">多宽带变更上网方式</td>
	...
	<TR id=tr1 style="display: none">
		...
		<table id="checkBindResult" border=0 cellspacing=1 cellpadding=2
			...
			<TR>
				<TH colspan="4" align="center">多宽带变更上网方式</TH>
			</TR>
			<tr bgcolor="#FFFFFF">
		...
		<table id="changeConnType" border=0 cellspacing=1 cellpadding=2
			...
			<tr bgcolor="#FFFFFF" id="line2" style="display: ">
				...
				<td id="pvcVlanValue" style="display: ">
					<!-- <input type="text" name="pvcVlanValue2" class="bk" readOnly="readonly" value=""> -->
					
					<select name="pvcVlanValue2" id="pvcVlanValue2" style="width:50px;" class="bk">
				</td>
				<td id="pvcVlanValue1" style="display: none"><select name="pvc"
					id="pvcSelect" class="bk">
				</select></td>

				<TD class=column align="right" width="15%" disabled><img
					src="<s:url value="/images/refresh.png" />" border="0" alt="刷新"
					onclick="reInter()"> 绑定端口:</TD>
				<TD width="35%" id="lanInter" disabled>正在获取端口...</TD>
				<!-- 				<td nowrap class=column align="right">
								</td>
								<td nowrap >
								</td>-->
