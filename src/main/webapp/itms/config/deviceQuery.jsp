...
function do_test()
{
	...
	}else{  // 高级查询必须输入设备SN，支持
		var gwShare_cityId = $("select[@name='gwShare_cityId']").val();      // 设备属地
		var gwShare_vendorId = $("select[@name='gwShare_vendorId']").val();  //设备厂商
		var gwShare_deviceModelId = $("select[@name='gwShare_deviceModelId']").val();  // 设备型号
		var gwShare_devicetypeId = $("select[@name='gwShare_devicetypeId']").val();    // 设备版本
...
// 此function针对 设备批量下发数图配置 专门定制改写
function gwShare_queryDevice(){
	var url="<s:url value='/itms/config/digitMapConfig!queryDeviceList.action'/>"; 
	
	var gwShare_queryResultType = $("input[@name='gwShare_queryResultType']").val();  // radio
    var gwShare_queryType = $("input[@name='gwShare_queryType']").val();  // 默认高级查询
    
   	var gwShare_cityId = $("select[@name='gwShare_cityId']").val();   // 设备属地
    var gwShare_onlineStatus = $("select[@name='gwShare_onlineStatus']").val();    // 上线状态
    var gwShare_vendorId = $("select[@name='gwShare_vendorId']").val();            // 设备厂家
    var gwShare_deviceModelId = $("select[@name='gwShare_deviceModelId']").val();  // 设备型号
    var gwShare_devicetypeId = $("select[@name='gwShare_devicetypeId']").val();    // 设备版本
    var gwShare_bindType = $("select[@name='gwShare_bindType']").val();            // 是否绑定
    var gwShare_deviceSerialnumber = $("input[@name='gwShare_deviceSerialnumber']").val();  // 设备序列号
   	...
   	$.post(url,{
		...
		if(typeof(returnVal)=='undefined'){
			...
			deviceResult(returnVal, gwShare_cityId, gwShare_vendorId, gwShare_deviceModelId, gwShare_devicetypeId);
		}
	});
}
			...
			<table border=0 cellspacing=1 cellpadding=2 width="100%" align="center">
				...
				<tr bgcolor="#FFFFFF" id="gwShare_tr12" STYLE="display:">
					<td colspan="4" align="center" width="100%">
						...
						<input type="radio" class=jianbian name="gwShare_queryField" value="username" onclick="gwShare_queryField_selected(this)"/> 用户账号 &nbsp;&nbsp;
