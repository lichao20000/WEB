...
<script type="text/javascript">
	...
	function deviceDetail(type,countNum,task_id){
		var url = "<s:url value='/itms/resource/batchSetTempManaACT!queryDevList.action?countNum='/>"+countNum+"&&type="+type+"&&task_id="+task_id;
	...
	function updateStatus(task_id,type){
		...
		var url = "<s:url value='/itms/resource/batchSetTempManaACT!updateCount.action'/>";
		$.post(url,{
			task_id:task_id,
			type:type
		},function(ajax){
	...
	function doUpdate(task_id,type){
		...
		var url = "<s:url value='/itms/resource/batchSetTempManaACT!update.action'/>";
		$.post(url,{
			task_id:task_id,
			type:type
		},function(ajax){
	...
	function doDel(task_id){
		var url = "<s:url value='/itms/resource/batchSetTempManaACT!del.action'/>";
		$.post(url,{
			task_id:task_id
		},function(ajax){
			if("1" == ajax){
	...
	function del(task_id){
		...
		var url = "<s:url value='/itms/resource/batchSetTempManaACT!delCount.action'/>";
		$.post(url,{
			task_id:task_id
		},function(ajax){
			if("ok" == ajax){
	...
	function detail(task_id){
		var url = "<s:url value='/itms/resource/batchSetTempManaACT!queryTaskDetailById.action?task_id='/>"+task_id;
		...
		<tfoot>
			<tr>
				<td colspan="9" align="right">
					<lk:pages url="/itms/resource/batchSetTempManaACT!queryList.action" styleClass=""
