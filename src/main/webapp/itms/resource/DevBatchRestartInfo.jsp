...
function query() 
{
	...
	mainForm.action = "<s:url value='/itms/resource/DevBatchRestartQuery!devBatchRestartQueryInfo.action' />";
	mainForm.submit();
}
...
{
	var mainForm = document.getElementById("selectForm");
	mainForm.action = "<s:url value='/itms/resource/DevBatchRestartQuery!devBatchRestartQueryExcel.action' />";
	mainForm.submit();
}
	...
	switch (type)
	{
		case "city":
			...
			if("4" == gwType){
				...
				url = "<s:url value="/gwms/share/gwDeviceQuery!getCityNextChild.action"/>";
			}
			$.post(url,{
			},function(ajax){
						...
						<TR>
							<%if(!"1".equals(gw_type)){ %>
								<ms:inArea areaCode="nx_dx" notInMode="true">
									<th colspan="4">机顶盒批量重启统计 </th>
								</ms:inArea>
								<ms:inArea areaCode="nx_dx" notInMode="false">
									<th colspan="4">机顶盒批量重启查询 </th>
								</ms:inArea>
							<%}else{ %>
								<ms:inArea areaCode="nx_dx" notInMode="true">
									<th colspan="4">光猫批量重启统计 </th>
								</ms:inArea>
								<ms:inArea areaCode="nx_dx" notInMode="false">
									<th colspan="4">光猫批量重启查询 </th>
								</ms:inArea>
							<%}%>	
						</tr>
						<TR>
						...
						<ms:inArea areaCode="nx_dx" notInMode="false">
							<TR>
								<%if(!"1".equals(gw_type)){ %>
									...
									<TD width="35%">
										<input name="serv_account" type="text" value="" size=20/>
									</TD>
								<%}else{ %>
									<TD align="right" class=column width="15%">LOID</TD>
									<TD width="35%">
										<input name="loid" type="text" value="" size=20/>
									</TD>
								<%}%>	
								<TD align="right" class="column" width="15%">设备序列号</TD>
								<TD align="left" width="35%">
									<input name="device_serialnumber" type="text" value="" size=20/>
								</TD>
							</TR>
						</ms:inArea>
