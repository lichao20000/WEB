...
<script type="text/javascript">
function CheckForm(){
	var spec_user  = $("select[@name='userSpecId']").val();
	var spec_device  = $("select[@name='deviceSpecId']").val();
	if(spec_user == spec_device && spec_user != -1){
		alert("用户规格和设备规格不能一致！");
	...
	function doQuery() {
		...
		barScript.src = "../../Js/echarts/devUserAgreeBycity.js";
		document.body.appendChild(barScript);
	}
	
	function exportReport() {
		var form = document.getElementById("frm");
		form.action = "<s:url value='/itms/resource/devUserAgreeRate!parseExcel.action'/>";
		form.submit();
	}

	function exportDetail() {
		var form = document.getElementById("frm");
		form.action = "<s:url value='/itms/resource/devUserAgreeRate!parseDetail.action'/>";
		form.submit();
	}
...
<body>
	<div class="it_main">
		<h3 class="main-ttl">终端和用户规格一致率</h3>
		<div class="search-container">
			<form name="frm" id="frm" >
				...
				<div class="search-item">
					<div class="search-label-blue">用户受理时间：</div>
					<div class="search-con">
				...
				<div class="search-item">
					<div class="search-label-blue">用户规格：</div>
					<div class="search-con">
						<s:select list="specList" name="userSpecId" headerKey="-1" headerValue="全部"
						listKey="id" listValue="blue_status" cssClass="select"
							style="width:100px;"  onchange="return CheckForm()"></s:select>
					</div>
				</div>
				
				<div class="search-item">
					<div class="search-label-blue">设备规格：</div>
					<div class="search-con">

					<s:select list="specList" name="deviceSpecId" headerKey="-1" headerValue="全部"
							listKey="id" listValue="blue_status" cssClass="select"
							style="width:100px;"  onchange="return CheckForm()"></s:select>
					</div>
					...
					<div class="searchbtn-box">
						<a href="javascript:doQuery()" class="search-btn" id="doButton"> 查 询 </a>
					</div>
				</div>
			</form>
		</div>
			...
			<div class="chart-container">
				...
				<div id="toExcel" class="chart-toolbar" style="display: none;">
					...
					<a href="javascript:void(0);" onclick="exportDetail()">
						<i class="iconstyle icon-excel">&nbsp;</i>导出规格不匹配清单</a>
				</div>
			</div>
	</div>
</body>
