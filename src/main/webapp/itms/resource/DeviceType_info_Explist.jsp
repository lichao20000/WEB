		...
		<tr>
			<td colspan="9" align="left">
				<IMG SRC="<s:url value="/images/excel.gif"/>" BORDER='0' ALT='导出列表'
					style='cursor: hand'  onclick="javascript:toExcel()">
			</td>
			<td colspan="11" align="right"><lk:pages
				url="/itms/resource/deviceTypeExpInfo!queryList.action" styleClass=""
...
<script>
  ...
  function detailDevice(devicetype_id,specId){
   window.open("<s:url value='/itms/resource/deviceTypeExpInfo!queryDetail.action'/>?deviceTypeId="+devicetype_id+"&detailSpecId="+specId,"","left=20,top=20,height=450,width=700,resizable=yes scrollbars=yes,status=yes,toolbar=no,menubar=no,location=no");
    }
...
function delDevice(id,gw_type)
{
	if(!delWarn())
		return;
	var url = "<s:url value="/itms/resource/deviceTypeExpInfo!deleteDevice.action"/>";
...
{
	var url = "<s:url value="/itms/resource/deviceTypeExpInfo!updateIsCheck.action"/>";
	$.post(url,{
		deviceTypeId:id
	},function(ajax){
		var result = parseInt(ajax);
