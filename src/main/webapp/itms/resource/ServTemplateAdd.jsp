   ...
   */
   function saveTemplate(id){
    ...
    $.ajax({
	    type:"POST",
	    async:false,
	    url:"<s:url value='/itms/resource/servTemplate!addTemplate.action' />?id="+id+"&cm="+$('#cm').val(),
	    dataType:"text",
	    ...
	    success:function (data) {
	    	...
	    	if(data==1){
	 		...
	 		//window.opener=null;
			//window.open('','_self');
			/* console.log("son before judge");
			if (window.opener != undefined) {
				console.log("son window.opener != undefined");
				window.opener.document.getElementById('paraValue').value=data;
				console.log("after set, value="+window.opener.document.getElementById('paraValue').value);
			...
			if (window.opener != undefined) {
		         //for chrome  
		         window.opener.deviceResult(data);
		     }
		     else {
		         window.returnValue = data;
		         window.opener.deviceResult(data);
					...
					<TD bgcolor=#000000>
						...
						<TABLE border=0 cellspacing=1 cellpadding=2 width="100%">
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="nserv_svlan_del_tr">
								...
								<TD colspan="3" class="column text" nowrap>
								<input name="nserv_svlan_del" type="radio" value="1" checked="checked"/>是
								<input name="nserv_svlan_del" type="radio" value="" />否
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="sserv_del_tr">
								<TD class="column text" nowrap align="center">配置项</TD>
								<TD colspan="3" class="column text" nowrap>
								<input name="sserv_del" type="radio" value="1" />是
								<input name="sserv_del" type="radio" value="" checked="checked"/>否
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="sserv_svlan_del_tr">
								<TD class="column text" nowrap align="center">配置项</TD>
								<TD colspan="3" class="column text" nowrap>
								<input name="sserv_svlan_del" type="radio" value="1" checked="checked"/>是
								<input name="sserv_svlan_del" type="radio" value="" />否
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="sport_del_tr">
								<TD class="column text" nowrap align="center">配置项</TD>
								<TD colspan="3" class="column text" nowrap>
								<input name="sport_del" type="radio" value="1" checked="checked"/>是
								<input name="sport_del" type="radio" value="" />否
