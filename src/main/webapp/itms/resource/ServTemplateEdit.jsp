   ...
   function saveTemplate(id){
    ...
    if(sNodes.length==0){
    	...
    	for(var i=0;i<sNodes.length;i++){
    		if(sNodes[i].path == '' || (sNodes[i].value =='' && $("#cm").val()!='1')){
    ...
    $.ajax({
	    type:"POST",
	    async:false,
	    url:"<s:url value='/itms/resource/servTemplate!saveTemplate.action' />?id="+id+"&cm="+$('#cm').val(),
	    dataType:"text",
	    data: {"checkedData":JSON.stringify(sNodes),"name":$("#name").val(),"vlan":$("#vlan").val(),"describe":$("#describe").val(),"serv":$("#serv").val(),"update_time":$("#update_time").val(),
	    	"nserv_svlan_del":$("input[name=nserv_svlan_del]:checked").val(),"sserv_del":$("input[name=sserv_del]:checked").val(),"sserv_svlan_del":$("input[name=sserv_svlan_del]:checked").val(),"sport_del":$("input[name=sport_del]:checked").val(),"service_id":$("#service_id").val()},
   ...
   */
	...
	*/
	function save(){
		...
		else if($("#nodeName").val()=='' || ($("#nodeValue").val() =='' && $("#cm").val()!='1')){
			alert("节点名称、节点值不可为空");
		}
		else{
					...
					<TD bgcolor=#000000>
						...
						<TABLE border=0 cellspacing=1 cellpadding=2 width="100%">
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="nserv_svlan_del_tr">
								...
								<TD colspan="3" class="column text" nowrap>
								<input name="nserv_svlan_del" type="radio" value="1" <s:if test="nserv_svlan_del==1">checked</s:if>/>是
								<input name="nserv_svlan_del" type="radio" value="" <s:if test="nserv_svlan_del!=1">checked</s:if>/>否
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="sserv_del_tr">
								<TD class="column text" nowrap align="center">配置项</TD>
								<TD colspan="3" class="column text" nowrap>
								<input name="sserv_del" type="radio" value="1" <s:if test="sserv_del==1">checked</s:if>/>是
								<input name="sserv_del" type="radio" value="" <s:if test="sserv_del!=1">checked</s:if>/>否
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="sserv_svlan_del_tr">
								<TD class="column text" nowrap align="center">配置项</TD>
								<TD colspan="3" class="column text" nowrap>
								<input name="sserv_svlan_del" type="radio" value="1" <s:if test="sserv_svlan_del==1">checked</s:if>/>是
								<input name="sserv_svlan_del" type="radio" value="" <s:if test="sserv_svlan_del!=1">checked</s:if>/>否
							...
							<tr style="cursor: hand; background-color: #cccccc;display: none" id="sport_del_tr">
								<TD class="column text" nowrap align="center">配置项</TD>
								<TD colspan="3" class="column text" nowrap>
								<input name="sport_del" type="radio" value="1" <s:if test="sport_del==1">checked</s:if>/>是
								<input name="sport_del" type="radio" value="" <s:if test="sport_del!=1">checked</s:if>/>否
