...
<script type="text/javascript">
	...
	function query() {
	    ...
	    var url = "<s:url value='/itms/resource/VoiceFailResonQuery!voiceFailReasonQueryInfo.action' />";
	    $.post(url,{
	    	city_id:cityId,
	...
	function openCity(city_id,reason){
		var startOpenDate = $.trim($("input[@name='startOpenDate']").val());
		var endOpenDate = $.trim($("input[@name='endOpenDate']").val());
		var page="<s:url value='/itms/resource/VoiceFailResonQuery!voiceFailReasonDevQueryInfo.action'/>?"
				+ "city_id=" + city_id+"&startOpenDate="+startOpenDate+ "&endOpenDate=" +endOpenDate+"&reason="+reason;
		window.open(page,"","left=50,top=50,height=550,width=900,toolbar=no,menubar=no,location=no,resizable=yes scrollbars=yes");
	}
	
	function ToExcel(){
		var mainForm = document.getElementById("selectForm");
		mainForm.action = "<s:url value='/itms/resource/VoiceFailResonQuery!voiceFailReasonQueryExcel.action'/>";
		mainForm.submit();
		mainForm.action = "<s:url value='/itms/resource/VoiceFailResonQuery!voiceFailReasonQueryInfo.action'/>";
	}
...
<body>
	<form id="form" name="selectForm"
		action="<s:url value='/itms/resource/VoiceFailResonQuery!voiceFailReasonQueryInfo.action'/>">
		<table>
			<tr>
				<td HEIGHT=20>&nbsp;</td>
			</tr>
