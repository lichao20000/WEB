...
<TABLE border=0 cellspacing=0 cellpadding=0 width="100%">
	...
	<TR>
		...
		<table width="98%" height="30" border="0" align="center" cellpadding="0" cellspacing="0" class="green_gargtd">
			<tr>
				<td width="162"> <div align="center" class="title_bigwhite">软件版本管理</div> </td>
		...
		<TABLE border=0 cellspacing=0 cellpadding=0 width="98%" align="center">
			<tr>
				...
				<table border=0 cellspacing=1 cellpadding=2 width="100%" class="querytable" align="center">
					<tr>
						<th colspan="4" id="gwShare_thTitle">软件版本管理</th>
					</tr>
					...
					<tr bgcolor="#FFFFFF">
						<td colspan="4" align="right" class="green_foot" width="100%">
							...
							<input type="button" class=jianbian onclick="javascript:exportList()" name="exportButto" value=" 导 出 " />
						</td>
					</tr>
				</table>
				</td>
		...
		<TABLE width="98%" border=0 cellspacing=0 cellpadding=0 align="center" id="addTable" style="display: none">
			<TR>
				...
				<TABLE border=0 cellspacing=1 cellpadding=2 width="100%" id="allDatas" class="querytable">
					<TR>
						<TH colspan="4" align="center"><SPAN id="actLabel">添加</SPAN><SPAN id="DeviceTypeLabel"></SPAN>目标软件版本</TH>
					</TR>
					...
					<TR bgcolor="#FFFFFF">
						<TD class=column align="right">目标软件版本</TD>
						<TD colspan=3>
					...
					<TR bgcolor="#FFFFFF">
						...
						<TD colspan = 3 height="23">
							<div id="div_softVersion" >请选择设备型号</div>
						</TD>
					</TR>
					
					<TR bgcolor="#FFFFFF">
						<TD class=column align="right">对应关系类型</TD>
						<TD colspan=3>
					...
					<TR bgcolor="#FFFFFF" id="cityId_addID" style="display:none;">
						...
						<TD colspan = 3 height="23">
							<div id="div_cityId_add" >请选择属地</div>
						</TD>
					</TR>
				</TABLE>
				</TD>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function save(){
		...
		/** 获取选中原软件版本的 devicetype_id */
		$("input[@name='devicetypeIdOld'][@checked]").each(function(){ //由于复选框一般选中的是多个,所以循环
		...
		var cityId_add="";
		$("input[@name='cityId_add'][@checked]").each(function(){ //由于复选框一般选中的是多个,所以循环
			if("" == cityId_add){
				cityId_add = $(this).val();
			}else{
		...
		$.post(url,{
			...
			if("1" == arr[0]){
				alert(arr[1]);
				var form = document.getElementById("mainForm");  // 重新提交查询
				form.action = "<s:url value='/itms/resource/softVerManage!getVersionList.action'/>";
				resetFrm();  /** 提交后清除缓存 */
				form.submit();
			}else{
	...
	function showCity(){
		...
		if("sd_lt" == area||"sd_dx" == area){
			if(tempIdNew=='2'||tempIdNew=='3'){
				...
				$("input[@name='cityId_addAll']").removeAttr("checked");
				document.getElementById("div_cityId_add").innerHTML = "请选择设备型号";
				$("#cityId_addID").hide();
			}
		}
	}
