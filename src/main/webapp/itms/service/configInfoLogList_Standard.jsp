...
function configDetailInfo(strategyId,deviceSN,gw_type){
	//var gw_type = document.getElementsByName("gw_type")[0].value;
	var page = "<s:url value='/itms/service/bssSheetServSDLT!getConfigLogDetail.action'/>?gw_type=" + gw_type+ "&strategyId="+strategyId+"&deviceSN="+deviceSN;
					...
					<s:if test="configLogInfoList.size()>0">
						<s:iterator value="configLogInfoList">
							<tr>
								...
								<td>
									<a href="javascript:configDetailInfo('<s:property value="id" />','<s:property value="deviceSN" />','<s:property value="gw_type" />')">详细信息</a>
								</td>
							</tr>
						</s:iterator>
					</s:if>
