...
function showChild(param)
{
	...
	{		
		...
		page = "showDeviceList.jsp?gather_id="+document.frm.gather_id.value +"&vendor_id="+document.frm.vendor_id.value+ "&devicetype_id=" +document.frm.softwareversion.value+"";
								...
								<TR>
									<TD bgcolor=#999999>
										<TABLE border=0 cellspacing=1 cellpadding=2 width="100%">
											...
											<TR bgcolor="#FFFFFF">
												<td align="right">PPPoe用户名:</td>
												<td><input type="text" name="username"></td>
												<td align="right">PPPoe密码:</td>
...
<!--
	function getStatus() {
     		if(CheckForm()){
		        page = "PPPoeConfigStatus.jsp?device_id=" +did 
		        	+ "&type=" + (document.getElementById("rd1").checked ? "1" : "2")
		        	+ "&refresh=" + new Date().getTime();
				document.all("div_ping").innerHTML = "正在获取设备PPPoe信息，请耐心等待....";
     ...
     function ExecMod(){
     		if(CheckForm()){
		        page = "PPPoeConfigSave.jsp?device_id=" +did 
		        	+ "&type=" + (document.getElementById("rd1").checked ? "1" : "2")
		        	+ "&username=" + document.frm.username.value 
		        	+ "&password=" + document.frm.password.value
		        	+ "&refresh=" + new Date().getTime();
				document.all("div_ping").innerHTML = "正在配置设备PPPoe信息，请耐心等待....";
				document.all("childFrm").src = page;
			}else{
				return false;
			}	
