...
{	
	...
	tree.callback_click=function my_click(nodeID){
		...
		if(area == "gs_dx" || area == "sx_lt"){
			// 甘肃电信 wan节点端口显示 所带的vlan信息，显示到输入框要去掉
...
// 叶子节点
window.nodes={};
//父节点para_name 叶子节点列表 leafNodeList = [{name,path,writable}]
...
// 叶子节点值map
window.nodesValue = {};
// 父节点para_name 叶子节点列表 leafNodesStr = path,value,writable|path,value,writable
	...
	if(leafNodeList == undefined || leafNodeList.length == 0){
		...
		for (var i = 0; i < leafNodeList.length; i++) {
			...
			"<tr>"+
		      ...
		      "</td>" +
			  "<td>" +
			  	"<a onclick=\"del('"+ path +"')\" style=\"cursor:pointer;\">删除</a>" +
			  "</td>" +					     	
		   "</tr>" 
		}
		leafNodeHtml += "</table>"
...
function getAllParam(){
	...
	// 由于list传不到子页面（或者有其他方法，暂时没发现），拼接get请求有时候太长导致请求没反应或失败，所以只拼接必要字符
...
function setAllParam(){
	...
	for (var i = 0; i < leafNodeList.length; i++) {
		...
		var name = leafNodeList[i]["name"];// 节点名称
		var value = leafNodeList[i]["value"];// 节点值
		var newValue = valueArr[i].value; // 更改的值 
	...
	if(newValueListStr != ""){
		nameListStr = nameListStr.substring(0,nameListStr.lastIndexOf('___'));// 去除最后一个逗号
		newValueListStr = newValueListStr.substring(0,newValueListStr.lastIndexOf('___'));// 去除最后一个逗号
		...
		<TR>
			<TD bgcolor=#999999>
				...
				<TR bgcolor="#FFFFFF">
					...
					<!-- 甘肃电信，参数配置显示在右侧 -->
					<s:if test='#request.area=="gs_dx" || #request.area=="sx_lt"'>
						<TD class=column align="right" width=5%>参数配置</TD>
						<TD bgcolor="#F1F1F1">
						<div style="overflow:auto;width:100%;height:260;padding:2pt 2pt 2pt 2pt">
							<table  border=0 cellspacing=10 cellpadding=0 width="100%">
							    ...
							    <tr>
							    	<td>
							    		叶子节点配置（点击“获取参数值”按钮，获取最新数据）：
							    	</td>
							    </tr>
							    <tr>
							    	<td>
									  	<INPUT TYPE="button" disabled="disabled" NAME="getAllValue" value="获取参数值" class=btn  onclick="getAllParam()">
									  	<INPUT TYPE="button" disabled="disabled" NAME="setAllValue" value="配置参数值" class=btn  onclick="setAllParam()">
									</td>
							    </tr>
							    <tr>
									<td>
