...
{	
	...
	if(nodeStr==""){
		...
		if(<%=checkType%>==0)
		{
		   this.location = "paramCMDConfig.jsp?gather_id=<%=gather_id%>&vendor_id=<%=vendor_id%>&devicetype_id=<%=devicetype_id%>&softwareversion=<%=softwareversion%>&checkType=<%=checkType%>";
		}
		else
		{
		   this.location = "paramCMDConfig.jsp?<%=param%>&checkType=<%=checkType%>";
		}
...
function getParam(){
		...
		var page="para_configForm.jsp?param_name=" 
			+new_param+ "&gather_id=<%=gather_id%>&device_id=<%=device_id%>&ior=<%=ior%>&action=getValue&refresh=" + Math.random();
...
function setParam(){
		...
		showMsgDlg();
		var page="para_configForm.jsp?param_name=" 
			+new_param+ "&gather_id=<%=gather_id%>&device_id=<%=device_id%>&ior=<%=ior%>&action=setValue&para_value=" + document.frm.para_value.value+"&refresh=" + Math.random();
		document.all("childFrm").src = page;
	...
	<TABLE width="90%" border=0 cellspacing=0 cellpadding=0 align="center">
		...
		<TR>
			<TD bgcolor=#999999>
				...
				<TR>
					<TH  colspan="2" align="center"><B>配置参数实例</B></TH>
				</TR>
				<TR bgcolor="#FFFFFF">
					<TD class=column align="right" >参数名称</TD>
					<TD><INPUT TYPE="text" NAME="para_name" id="para_name" maxlength=255 class=bk size="60" readOnly>&nbsp;<font color="#FF0000">*</font></TD>
					<input type="hidden" id="hid_para_name" name="hid_para_name" value="">
				</TR>
				
				<TR bgcolor="#FFFFFF">
					<TD class=column align="right" >参数值</TD>
					<TD><INPUT TYPE="text" NAME="para_value" maxlength=255 class=bk size=60></TD>
				</TR>
				
				<TR class=green_foot >
					<TD colspan="2" class=foot align=center>
						<INPUT TYPE="button" name="getValue" id="getValue" value="获取参数值" class=btn  onclick="getParam()">&nbsp;&nbsp;				
						<INPUT TYPE="button" name="setValue" id="setValue" value="重新配置" class=btn  onclick="setParam()">&nbsp;&nbsp;				
					</TD>
				</TR>
			</TABLE>
			</TD>
