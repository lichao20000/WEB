...
function CheckForm(){
	...
	if(typeof(oselect.length)=="undefined"){
		if(oselect.checked){
			num = 1;
			document.all("dev_serial").value = oselect.dev_serial;
		}
 	}else{
		for(var i=0;i<oselect.length;i++){
			if(oselect[i].checked){
				document.all("dev_serial").value = oselect[i].dev_serial;
				num++;
				break;
			}
		}
...
function showChild(param)
{
	...
	if(param == "gather_id")
	{
		...
		page = "showDeviceVersion.jsp?vendor_id="+document.frm.vendor_id.value + "&devicetype_id="+document.frm.devicetype_id.value;
...
<TABLE border=0 cellspacing=0 cellpadding=0 width="100%">
	...
	<TR>
		<TD>
			<FORM NAME="frm" METHOD="post" ACTION="handSheetFrame.jsp" onsubmit="return CheckForm()">
										...
										<TABLE border=0 cellspacing=1 cellpadding=2 width="100%">
											...
											<TR bgcolor="#FFFFFF">
												<TD colspan="4" align="right" class="green_foot">												
													<INPUT TYPE="submit" value=" 选 择 " class=btn >
													<input type=hidden name="dev_serial" value="">
												</TD>
											</TR>
										</TABLE>
									</TD>
