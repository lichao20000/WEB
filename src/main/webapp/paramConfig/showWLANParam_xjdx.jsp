		...
		<td >
			...
			<%
				for(int i=0;i<wlanObj.length;i++)
				{
					...
					wepEncrLevel=wlanObj[i].getWepEncrLevel();
					wpaEncrMode=wlanObj[i].getWpaEncrMode();
					basicAuthMode=wlanObj[i].getBasicAuthMode();
				...
				<tr  bgcolor="#FFFFFF">
					...
					<td align="center" >
						...
						<label style="cursor:hand;"
						 	onclick="delWlan_conn(this,'<%=device_id_first%>|<%=temp_lan_id%>|<%=temp_lan_wlan_id%>')">
						 	删除
						</label>
					</td>
				</tr>
				...
				<tr bgcolor="#FFFFFF">
					<td align="right" colspan="8">
						<label style="cursor:hand;"
						 onclick="add('<%=device_id_first%>','<%=wepEncrLevel%>')">新增</label>
					</td>
				</tr>
			</table>
		</td>
