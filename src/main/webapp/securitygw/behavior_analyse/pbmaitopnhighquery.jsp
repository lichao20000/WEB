...
<%-- 
	/**
 	 * 高级查询:获取员工mail浏览topN报表页面
 	 * 
 	 * @author suixz(5253) 2008-5-7
 	 * @version 1.0
 	 * @category securitygw
 	 */
--%>
<meta http-equiv="Content-Type" content="text/html; charset=gbk">
<title>mailTopN高级查询</title>
...
function queryData(){
	...
	$("#load").show();
	//加载报表图形
	var url = "<s:url value="/securitygw/pBMaiHighQuery!getMailTopNChart.action"><s:param name="deviceId" value="deviceId"/></s:url>";
	$("#chartData").attr("src",url+"&startTime="+$("#startDate").val()+"&endTime="+$("#endDate").val());
	//动态获取报表信息
	url = "<s:url value="/securitygw/pBMaiHighQuery!getMailTopNTab.action"><s:param name="deviceId" value="deviceId"/></s:url>";
	$.post(
		url,
		{
...
<form>
	<table width="99%"  border="0" cellpadding="0" cellspacing="0" class="table">
		<tr class="tab_title">
			<td class="title_white"><span>员工MAIL浏览Top<s:property value="topN"/></span>&nbsp;&nbsp;
