...
function query(time)
{
	var url ="<s:url value="/securitygw/UserTopReport!filterTopDayChart.action"><s:param name="device_id" value="device_id"/></s:url>";
 	$("#chartDay").attr("src",url+"&strTime="+time);
 	var turl ="<s:url value="/securitygw/UserTopReport!filterTopDayTable.action"></s:url>";
	$.post(
		turl,
		{
	...
	var wurl = "<s:url value="/securitygw/UserTopReport!filterTopWeekChart.action"><s:param name="device_id" value="device_id"/></s:url>";
	 $("#chartWeek").attr("src",wurl+"&strTime="+time);
	 	var wturl ="<s:url value="/securitygw/UserTopReport!filterTopWeekTable.action"></s:url>";
	 	$.post(
		wturl,
		{
			device_id:"<s:property value="device_id" />",
	...
	var murl = "<s:url value="/securitygw/UserTopReport!filterTopMonthChart.action"><s:param name="device_id" value="device_id"/></s:url>";
	 $("#chartMonth").attr("src",murl+"&strTime="+time);
	 	var mturl ="<s:url value="/securitygw/UserTopReport!filterTopMonthTable.action"></s:url>";
	 	$.post(
		mturl,
		{
			device_id:"<s:property value="device_id" />",
...
function adQuery()
{
$.open("<s:url value="/securitygw/UserTopReport!filterAD.action"><s:param name="device_id" value="device_id"/></s:url>","740px","400px","200px","200px","true");
}
</script>
<style type="text/css">
<!--
...
<table width=98% style="margin-left: 5px; margin-top: 10px;"
	...
	<tr class="tab_title">
		<td colspan="2" class="title_white">过滤用户排名 <INPUT TYPE="text"
			id="start" class=bk value="<s:property value="nowDateString" />">
		<INPUT TYPE="button" value="▼" class=jianbian
			onclick="showCalendar('day',event)"> <input name="Submit"
			type="button" onclick="Query()" class="jianbian" value="查询">&nbsp;<input type="button" class="jianbian" value="高级查询" onclick="adQuery()"/>
		</td>
	</tr>
	<tr>
