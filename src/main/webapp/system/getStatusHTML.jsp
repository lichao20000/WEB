...
Map fields = null;
String maxTime = "";
if (!"".equals(hostIP) && !"-1".equals(hostIP)){
	cursor = DataSetBean.getCursor("select max(gettime) as maxtime from tab_performance where hostip = '"
									+ hostIP + "'");
	...
	String sql = "select a.*,b.warmvalue from tab_performance a left join tab_attrwarconf b on a.gather_id = b.gather_id and a.hostip = b.hostip and a.attr_id = b.attr_id where a.hostip = '" 
									+ hostIP + "' and a.gettime = " + maxTime + " and a.gather_id = '" + gather_id + "'";
	// teledb
	if (DBUtil.GetDB() == 3) {
		sql = "select a.attr_id, a.value1, a.value2, a.instance, b.warmvalue " +
				"from tab_performance a left join tab_attrwarconf b on a.gather_id = b.gather_id and a.hostip = b.hostip and a.attr_id = b.attr_id where a.hostip = '"
				+ hostIP + "' and a.gettime = " + maxTime + " and a.gather_id = '" + gather_id + "'";
	}
