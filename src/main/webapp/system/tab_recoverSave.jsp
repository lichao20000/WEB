...
//入库成功
int recover_id = sysManage.tabRecover(request);

if(recover_id < 0){
	response.sendRedirect("backup_fail.jsp?redirect=tab_recover.jsp");
	return;
}
...
String strSql = "";
short s1 = 2;
int _result = sysManagerClient.dbManager(recover_id,s1);

strSql = "select * from tab_db_recover where recover_id=" + recover_id;
// teledb
if (DBUtil.GetDB() == 3) {
	strSql = "select exec_status, tab_name, tab_name_bk, execute_time, complet_time" +
			" from tab_db_recover where recover_id=" + recover_id;
}
