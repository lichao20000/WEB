...
int iStart = Integer.parseInt(start) + iHour*3600;
int iEnd   = iStart + 3600;
...
<TR><TD align=center>
	<DIV id="idLayer" style="overflow:auto;height:'400px'" align=center>
	  ...
	  <TR>
	  	<TD bgcolor=#999999>
	    	  <TABLE border=0 cellspacing=1 cellpadding=2 width="100%">
                <TR> 
                  <TH width='25%'>索引</TH>
                  <TH width='25%'>最大值(<%=ex_unit%>)</TH>
                  <TH width='25%'>最小值(<%=ex_unit%>)</TH>
                  <TH width='25%'>平均值(<%=ex_unit%>)</TH>
                </TR>
				<%
				//打印数据
				if(fields!=null){
					...
					out.println("<TH>&nbsp;</TH>");
					out.println("<TH>采集值</TH>");
					out.println("<TH>采集时间</TH>");
					out.println("</TR>");
					//根据时间格式化原始数据表名
//					tblname = "pm_raw_"+year+"_"+month;
//					//格式化查询SQL
//					strSQL = "select * from "+ tblname +" where gathertime>="+ iStart +" and gathertime<"+ iEnd +" and id in("+ str_iid +") order by id,gathertime";
					cursor = netPM.getGeneralChartData();
					fields = cursor.getNext();
					if(fields == null){
						out.println("<TR bgcolor=#ffffff><TD colspan=4>原始采集数据没有入库</TD></TR>");
					}
					else{
						String old_id="",cur_id;
						String value,gathertime;
						String tmp;
						long lms;
						//一个实例一个Cursor
						...
						while(fields != null){
							...
							out.println("<TD class=column>"+value+"</TD>");
