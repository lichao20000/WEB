...
<%@page import="java.util.Calendar"%><%@ include file="../timelater.jsp"%>
...
DateTimeUtil dtUtil = new DateTimeUtil((long)iStart*1000);
Calendar calendar = Calendar.getInstance();
calendar.set(dtUtil.getYear(),dtUtil.getMonth()-1,1);
long startTime = calendar.getTimeInMillis()/1000;
calendar.set(dtUtil.getYear(),dtUtil.getMonth(),1);
long iEnd   = calendar.getTimeInMillis()/1000;
...
//out.println(strSQL);
//执行SQL，获取数据
GeneralNetPerf netPM = new GeneralNetPerf(startTime, iEnd, type, arr_iid);
	  	...
	  	<TD bgcolor=#999999>
	    	  <TABLE border=0 cellspacing=1 cellpadding=2 width="100%">
				...
				if(fields!=null){
					...
					GeneralNetPerf netPmMore = new GeneralNetPerf(startTime, iEnd, 5, arr_iid);
					cursor = netPmMore.getGeneralChartData();
					fields = cursor.getNext();
//					tblname = "pm_hour_stats_"+year+"_"+month;
//					//格式化查询SQL
//					strSQL = "select * from "+ tblname +" where gathertime>="+ iStart +" and gathertime<"+ iEnd +" and id in("+ str_iid +") order by id,gathertime";
					//out.println(strSQL);
//					cursor = DataSetBean.getCursor(strSQL);
//					fields = cursor.getNext();
					if(fields == null){
						out.println("<TR bgcolor=#ffffff><TD colspan=5>日报表数据没有入库</TD></TR>");
					}
					else{
